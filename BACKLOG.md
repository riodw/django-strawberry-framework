...
- **R4 — this card's spec must record where the deferred G3 tests live.** Two `TODO(BACKLOG polymorphic_interface_connections - the abstract-return optimizer entry card)` anchors already stand at `tests/optimizer/test_walker.py` and `tests/optimizer/test_extension.py`, each restating a spec test name verbatim. On DRY alone they are deletable duplicates of [spec-035][spec-035]'s deferred test plan — **except that each also carries a file-placement judgement that exists nowhere else**, and both legs were measured on 2026-09-01: spec-035's G3 heading names **no** file (unlike its `### Slice 1 — G1 (tests/optimizer/test_extension.py)` and `### Slice 2 — G2 (tests/optimizer/test_walker.py + tests/optimizer/test_extension.py, extend)` siblings), and the phrase `real extension execution` — the `test_extension.py` anchor's stated reason for existing there rather than in the walker file — occurs **0** times in spec-035 and **0** times in its rationale companion. The spec-035 cycle deliberately did not write that placement in, because inventing this card's test-file layout is this card's spec's decision. So: **this card's spec records the placement, and only then do those two anchors become deletable.** Deleting them before that drops the judgement.
- **The live-coverage half is a fifth anchor, and it is still unretargeted.** `examples/fakeshop/test_query/test_library_api.py:3680` carries the P3a live matching-type test as `TODO(spec-035)` — the last `TODO(spec-035` occurrence in the tree — because that file was dirty with concurrent work throughout the spec-035 cycle. The head swap is carried by `TODO-ALPHA-053-0.0.15`; do not delete the anchor, and read P3a as no-regression coverage of the eventual classifier's `INLINE` branch, never as a behavioural proof of G3.
