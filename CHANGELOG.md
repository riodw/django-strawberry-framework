...
- `TypeRegistry.set_primary(model, type_cls)` and `TypeRegistry.unregister(type_cls)` public helpers. The first promotes an already-registered type to the primary slot for its model (idempotent; raises on a different existing primary; honours `_check_mutable`). The second removes all traces of a type from the registry (types list, model index, primary slot, definition, and pending relations sourced from the type); intentionally callable post-finalize so schema-audit tests can simulate a missing registration. Test fixtures that previously poked private registry maps now go through these helpers.
