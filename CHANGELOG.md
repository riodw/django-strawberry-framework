...
### Fixed
- **Ordering a list by a to-many relation no longer multiplies parent rows.** [`OrderSet`][glossary-orderset]`.apply_sync` / `apply_async` now order a to-many [`RelatedOrder`][glossary-relatedorder] path (reverse FK, forward / reverse M2M) by a `Min` (ASC) / `Max` (DESC) **aggregate annotation** — `.annotate(<alias>=Min("rel__col")).order_by(<alias>)` — instead of a raw `order_by("rel__col")` fan-out JOIN that multiplied the parent row once per related child. The aggregate forces a `GROUP BY` on the parent, so the queryset returns one row per parent (NULLs preserved, scalar / to-one terms unchanged, mixed scalar + to-many terms compose). This corrects behavior shipped in `0.0.8`: a `DjangoListField` ordered by a to-many relation previously returned duplicated parents — a branch with three shelves appeared three times — and now returns distinct, aggregate-ordered parents. The same root-cause fix makes the new [`DjangoConnectionField`][glossary-djangoconnectionfield] correct over a to-many `orderBy` (no duplicate / skipped nodes under the positional cursors; `totalCount` counts distinct parents, not the multiplied parent×child rows).
