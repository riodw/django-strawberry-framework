...
- **In-flight design work** — new features use the `docs/spec-<NNN>-<topic>-<0_0_X>.md` convention (NNN is the 3-digit KANBAN card number; see `docs/builder/BUILD.md` "Spec filename pattern"). Once a slice ships, its behavior is folded into `docs/GLOSSARY.md` or `docs/TREE.md` and the spec defaults to staying at its working location as the historical record; opt-in archival moves it to `docs/SPECS/` (e.g. `docs/SPECS/spec-017-deferred_scalars-0_0_6.md`, `docs/SPECS/spec-015-relay_interfaces-0_0_5.md`).
