...
| Card | Spec file |
| --- | --- |
| `WIP-ALPHA-050-0.0.15` - `DjangoListField` argument surface: `offset` / `limit` and `orderBy` | No dedicated spec |
...
- This card's spec is not written. It takes `docs/SPECS/spec-051-parity_gaps-0_0_15.md`, which is free only after the renumber sweep moves the DRY-squeeze spec off that stem.
