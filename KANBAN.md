...
- `spec-009-rich_schema_architecture-0_0_4.md #"### Layer 3: Finalization trigger"` still presents hybrid auto-finalization as the preferred direction after the spec-008 reconciliation established that the direction was REJECTED - an explicit `finalize_django_types()` call is the sole trigger and nothing auto-finalizes. The pointer half this bullet used to carry was discharged by the spec-010 residual cycle (2026-08-15): spec-010's two inbound citations into spec-009 are now `#"..."`-form and correctly aimed, the auto-trigger sentence citing the Layer-3 heading as the not-adopted direction and the error-format requirement citing `spec-009 #"### Decision 6: fail loudly"` directly. What remains is the cause, not the symptoms: a spec-009 residual reconciliation cycle, which is the disposition every residual cycle from spec-002 onward has taken and needs no card of its own. A concurrent session had spec-009's reconciliation in flight as of 2026-08-15; verify its state before dispatching.
