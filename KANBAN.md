...
- Seal the `FilterSet.apply_*` return at both fields the way the `OrderSet.apply_*` return is now sealed at both (`django_strawberry_framework/utils/querysets.py::apply_orderset_sync` / `::apply_orderset_async`, called from `list_field.py` and `connection.py`). The spec-050 rationale's Decision 20 recorded the connection field's missing snapshot-plus-re-seal as a deferral to a card that was never opened; the `OrderSet` half is discharged and this row is the `FilterSet` half. Same frozen routing intent, same `_ORDERSET_RESULT_POLICY` axes (lazy, model rows of the captured model, unsliced, uncombined, same alias), one shared helper pair, live rows on a shipped connection and a shipped list field, plus a sharded routing rejection. Adjacent asymmetry to close in the same change: `connection.py::_pipeline_sync` / `::_pipeline_async` seal `get_queryset` with `_DEFAULT_SEAL_POLICY` while the list field's argument path uses `_LIST_ARGUMENT_VISIBILITY_POLICY` (`reject_combined`).
