...
**67.6% complete** toward `1.0.0` - 50 of 74 cards done (65.1% size-weighted). Across all non-backlog cards (incl. post-`1.0.0`), 50 of 75 (66.7%, 64.0% size-weighted). Past the 50% mark. Backlog excluded; size-weighted by relative size (XS=1 .. XL=5).
...
| [`TODO-BETA-077-0.1.7`](#generated_input_field_names_follow_the_schema_naming_config_auto_camel_case_nameconverter) | Generated input field names follow the schema naming config (`auto_camel_case`, `NameConverter`) | To Do - Beta (1.0.0) |
...
- The package's `strawberry.Schema(...)` docstring examples disagree with each other about `config=strawberry_config()`, and the file that settles the argument is about to be deleted. **Re-derived 2026-09-22: exactly five multi-line construction examples exist in package docstrings, and three carry the registration** - `extensions/debug.py` (module docstring) **yes**; `extensions/resource_policy.py::DjangoResourcePolicyExtension` (class docstring, the `DjangoSchema(...)` example and the plain-schema fallback) **yes** twice; `optimizer/extension.py` (module docstring) no; `optimizer/extension.py` (the singleton-in-a-factory method docstring) no. Population boundary, because the greppable token over-collects: a package-wide grep for `(strawberry\.Schema|DjangoSchema)\(` returns 35 hits, of which 30 are inline prose in the ``strawberry.Schema(...)`` form, the `class DjangoSchema(strawberry.Schema):` definition, or an error-message string - only those five open a constructor block. `scalars.py`'s `[strawberry_config](docs/GLOSSARY.md#strawberry_config)` docstring names the pattern inline without building an example and is adjacent, not a member. **The precedent-setting file is the one that goes away**: the debug-extraction card's Slice 2 deletes `extensions/debug.py`, and that card lands ahead of this one on this card's own `0.0.15` line, so by the time this card opens the population is two non-compliant sites in `optimizer/extension.py` against the two compliant `resource_policy.py` examples - which is the sequencing that puts it here rather than leaving it to drift. **Homed, not decided**: whether a topic-scoped illustration (an optimizer example, a resource-policy example) should carry an unrelated `config=` line is a doc-style call for the maintainer, not a defect a spec cycle may settle. Either answer is cheap; five examples answering it two ways is not. Recorded by the spec-025 residual cycle at its reopening (`docs/builder/DONE/build-025-scalar_map_helper-0_0_7.md`), which also recorded the instrument trap that travels with it: **count on `config=strawberry_config`, never on `config=strawberry_config()`** - the empty parens miss `extra_scalar_map=` call sites. Scope note carried forward from that cycle: this card's WP batches name `extensions/resource_policy.py` but not `optimizer/extension.py`, so one of the two target files is not established by the board text.
...
- `django_strawberry_framework/mutations/sets.py` carries two single-file literal duplications this card's WP-B write-side batch already opens the file for. The flavor label is hard-spelled as the `"DjangoMutation"` bare literal **6 times** plus **12** more as the leading token of an f-string error message (43 occurrences of the bare word overall, so match on the two spellings and not on the word); the error text is pinned byte-identically by tests, so this is a rename-the-literal change and never a reword. Separately, `::_materialize_merged_input` re-spells the operation-to-override-attribute mapping that `django_strawberry_framework/mutations/operations.py::_OPERATION_INPUT_OVERRIDE_ATTR` already owns, differing only in its key. Confirmed single-file, not cross-slice, by the spec-036 residual cycle's cross-file literal comparison (2026-09-01).
- `django_strawberry_framework/types/finalizer.py::finalize_django_types` is **326 lines / 29 branch nodes** with phase 2.5 inlined in the body - the largest entry in the file by a wide margin. **Explicitly not a defect**: the phases are ordered and each is correct. Take the decomposition only if a WP batch is already opening the function (this card's WP-C finalizer error-formatter item does open the file), and treat a standalone finalizer-decomposition spec as a separate maintainer call rather than something this card must schedule. Measured 2026-09-01 by the spec-036 residual cycle.
- **Contract-level, and the answer decides a WP-A `import-linter` contract:** should `FieldError` / `NON_FIELD_ERROR_KEY` live in `django_strawberry_framework/mutations/inputs.py` at all? The neutral `utils/` layer depends UPWARD on one flavor's package at **3 sites**, one of them a function-local import written specifically to dodge an import cycle - the exact cross-boundary shape WP-A's contracts fence. Against moving it: `FieldError` is a public root export whose module identity a consumer may have pinned, so a move owes a re-export. Measured 2026-09-01 by the spec-036 residual cycle.
- **Contract-level, and it is WP-B's `sets_mixins.py` question asked on the write side:** should `django_strawberry_framework/mutations/sets.py` own the cross-flavor write substrate? **25 symbols** are imported out of it by the three write flavors, and each of **6 named helpers** has **zero caller inside `mutations/`** - they exist only for `forms/` and `rest_framework/`. `sets_mixins.py` is the standing precedent for a flavor-neutral home and is the destination spec-036 Decision 4 predicted. Measured 2026-09-01 by the spec-036 residual cycle.
- **Retire the stranded ordinal citations sitting in this card's own WP files: 26 sites in 10 package modules, four spec vocabularies.** Measured 2026-09-03 by the spec-038 residual cycle, whose per-token derivation is in `docs/builder/DONE/build-038-form_mutations-0_0_12.md` `## Final test-run gate (folded in from the retired final report)`. Same fold-into-the-WP-batch convention as this card's `_optimizer_field_map` and `[spec-011]` items; the sites in `tests/` and `examples/` are `TODO-ALPHA-056-0.0.17`'s repo-wide sweep, not this card's. **spec-039, 19 sites and the largest of them** - `Md1` x3 (`forms/inputs.py`, `rest_framework/inputs.py`, `utils/inputs.py`), `Md2` x3 (`mutations/resolvers.py`, `rest_framework/resolvers.py`, `utils/querysets.py`), `Md3` x2, `Md4` x2 (both `utils/querysets.py`), `Md5` x1 (`rest_framework/inputs.py`), `Md7` x3 (`forms/sets.py`, `mutations/sets.py::construction_kwargs`, `rest_framework/sets.py`), `M1a` x4 (`forms/resolvers.py`, `mutations/resolvers.py` x2, `rest_framework/resolvers.py`) and `H4` x1 (`rest_framework/resolvers.py` #"for a serializer-only relation - spec-039 H4"). **`spec-039` carries 0 occurrences of every one of them and has no rationale companion, so nothing resolves and these were never labels in that spec** - they are false independently of any later cycle, and `H4` additionally reads as a review-severity label, which `START.md` bars from standing prose, so its fix is content restatement rather than a spelling repair. **spec-036, 3 sites:** `L3-1` in `mutations/inputs.py` #"never indexed as a decode-able FK (spec-036 L3-1)" and in `utils/relations.py::is_forward_concrete_relation`, `M3-1` in `mutations/sets.py` #"FK-to-field-name reversal, spec-036 M3-1" - all 0 in spec-036 and in its companion, whose live vocabulary is `AR-H4` / `AR-M1` / `Major-2` / `Medium-1`, and `L<n>` / `M<n>` are Low / Medium review-round labels. **spec-033, 2 sites**, both in `optimizer/nested_planner.py` #"covering the general page (the P2-3 false-coverage defect);" and #"naming a duplicated column (the P2-3 defect)." - the module cites spec-033 eleven times but not on either of those lines, so no one-line instrument sees them. **spec-030, 2 sites** in `orders/sets.py` #"(``spec-030-connection_field-0_0_9`` P1-B). Scalar columns" and #"multiplied (``spec-030-connection_field-0_0_9`` P1-B); ``None``" - the other five sites of that vocabulary are on `056`, and **the two halves must land in the same release or spec-030's retirement cannot be proved at 0.** Every site is a comment or docstring; none is a behavior defect. **Re-measured 2026-09-10 by the spec-039 residual cycle: the spec-039 sub-population is DISCHARGED, and the premise above was wrong on its second clause.** All 19 sites are retired - a Python census over the git-tracked tree for `Md1`-`Md7` / `M1a` / `H4` under both-side word boundaries returns 0 in `django_strawberry_framework/`, `tests/` and `examples/` - and they were retired as content restatements rather than spelling repairs, which is the remedy this bullet prescribed. `spec-039` DOES have a rationale companion now (`docs/SPECS/appx/spec-039-serializer_mutations-0_0_13-rationale.md`), so the nothing-resolves reasoning held only for the window before that cycle's Slice 0 wrote it; the labels still resolved to nothing in either file, so the conclusion survives its premise. One further site outside this bullet's own list went with them: `django_strawberry_framework/rest_framework/serializer_converter.py` carried a `spec-039 SR-3` citation at HEAD (`4c483b6b`) that no vocabulary in this bullet names, and it is now 0 as well - the paired `tests/` site is on `TODO-ALPHA-056-0.0.17`. **This card's residual is 8 sites, not 7:** spec-036 4, spec-033 2, spec-030 2. The spec-036 clause below names 3 and there are 4 - `L3-1` x2 (`django_strawberry_framework/mutations/inputs.py`, `django_strawberry_framework/utils/relations.py::is_forward_concrete_relation`) and `M3-1` x2 (`django_strawberry_framework/mutations/sets.py`, plus an unlisted fourth at `django_strawberry_framework/mutations/resolvers.py` #"M3-1). Scalar attrs"). **The instrument warning is now load-bearing twice over:** re-deriving this population with a left-anchored `(^|[^A-Za-z0-9_-])TOKEN` alternation under `ugrep` returns 0 on the live 4-occurrence `P2-3` / `P1-B` population, so that spelling is a silent false-clean; use a lookaround in Python, or an unbounded grep read by eye.
- **Consolidate the `SerializerMutation ` message prefix, and fold the fourth provisional-name derivation into the same edit.** Measured 2026-09-10 by the spec-039 residual cycle in Python over the git-tracked package tree, counting occurrences and not lines: **73** occurrences of the source text `"SerializerMutation ` across five modules - `django_strawberry_framework/rest_framework/resolvers.py` 43, `django_strawberry_framework/rest_framework/inputs.py` 14, `django_strawberry_framework/rest_framework/sets.py` 14, `django_strawberry_framework/rest_framework/__init__.py` 1 (the djangorestframework-absent install hint) and `django_strawberry_framework/utils/inputs.py` 1 (a docstring naming the label as a `noun` parameter value, so the parameterized substrate this consolidation would target already exists) - of which **59** are `f"SerializerMutation {`, at 40 / 5 / 14 / 0 / 0. **State the five-module population:** an earlier reading of 71 globbed only the three `rest_framework/` modules and is the same too-narrow-population error this card's `"DjangoMutation"` sibling bullet warns about. A fourth spelling sits outside that literal - `django_strawberry_framework/utils/permissions.py::request_from_info` is called with `family_label="SerializerMutation"` from `django_strawberry_framework/rest_framework/resolvers.py`. Several occurrences are `pytest.raises(match=...)` anchors, so every message renders byte-identically under this card's existing error-string policy; this is a rename-the-literal change and never a reword. **Riding the same edit because it is another spelling of the same derivation:** `django_strawberry_framework/rest_framework/resolvers.py::_assert_field_agreement` rebuilds the partial-input name inline from the runtime operation instead of from the `Meta` serializer class - a third spelling of the partial test and an inline rebuild of `NON_DELETE_OPERATION_INPUT_KIND` - and the correct fix needs an import edge that `docs/SPECS/spec-039-serializer_mutations-0_0_13.md`'s `### Import manifest` does not grant `resolvers.py`, so the manifest row moves in the same change and this half is a contract change rather than a consolidation. The three other provisional-name sites were already re-pointed at `django_strawberry_framework/utils/inputs.py::generated_input_type_name`; this is the fourth. Homed here 2026-09-10 by the spec-039 residual cycle.
- **Contract-level, and the answer decides a WP-A `import-linter` contract: which population should the DRY import ratchet hold, and does the forms flavor get one?** `tests/rest_framework/test_dry_import_ratchet.py` pins by OBJECT IDENTITY that the serializer flavor imports its shared substrate rather than redefining it - the instrument is identity and not a source grep, so it catches the redefinition and the deletion alike - and it was built from imports measured at HEAD because `docs/SPECS/spec-039-serializer_mutations-0_0_13.md`'s `### Import manifest` is stale in all four rows and a ratchet built from it would `AttributeError` on correct code. Three candidate populations, each losing something different: the spec's **named promotions** (what shipped, and what the file holds today), the **whole measured shared substrate**, or the `### Import manifest` population. Two concrete rows for the answer to weigh, neither added by the cycle that raised this: `django_strawberry_framework/utils/inputs.py::generated_input_type_name` is load-bearing at four `django_strawberry_framework/rest_framework/inputs.py` call sites and is absent from the ratchet's `SHARED_BINDINGS`, and the three `django_strawberry_framework/rest_framework/inputs.py` helpers that `django_strawberry_framework/rest_framework/sets.py` now imports are not named in that module's manifest column. **Second axis, and the reason it is a contract and not a test chore:** the forms flavor has no ratchet at all, while `django_strawberry_framework/forms/converter.py::FormFieldConversion` subclasses the same `django_strawberry_framework/utils/inputs.py::FieldConversionBase` the serializer ratchet pins - so whatever boundary WP-A declares for one flavor is owed to the other. Homed here 2026-09-10 by the spec-039 residual cycle; carried exactly, not re-decided.
- **Decide whether a duck-typed `_mutation_meta` is supported at the field-agreement seam.** `django_strawberry_framework/rest_framework/resolvers.py::_assert_field_agreement` now raises `ConfigurationError` for both incoherent spellings of an absent mutation-meta snapshot (attribute missing, attribute `None`), replacing a fail-open `getattr` default that statement coverage could not see; that narrowing is shipped and correct, and its `operation` + `optional_fields` snapshot requirement is written into the spec as a contract sentence. What is still open is the tighter `isinstance(meta, _ValidatedMutationMeta)` form, which is a seam-support question rather than a hardening one: `django_strawberry_framework/auth/mutations.py::_AuthMutationMetaSnapshot` is deliberately a duck-typed snapshot, so an `isinstance` gate at that seam would decide for the whole write side whether the snapshot is a protocol or a class. WP-A owns the question because it is the same boundary the `import-linter` contracts formalize. Homed here 2026-09-10 by the spec-039 residual cycle.
- **Settle the owner-name asymmetry in the two `Meta.nested_fields` rejections BEFORE the WP-B consolidation opens those files, because this card's byte-preservation policy freezes whatever text it finds.** The same `SerializerMutation <name>` slot renders the MUTATION class name when `django_strawberry_framework/rest_framework/sets.py` rejects and the SERIALIZER class name when `django_strawberry_framework/rest_framework/inputs.py` rejects, so which name a consumer sees depends on which depth of the `django_strawberry_framework/rest_framework/inputs.py::NestedSerializerConfig` opt-in tree refused. It is pre-existing and was preserved byte-for-byte by the spec-039 residual cycle's consolidation, on the rule that changing a message inside a refactor is a behavior change wearing a refactor's diff - which is exactly this card's policy, and exactly why the decision has to precede the batch rather than ride it. Homed here 2026-09-10 by that cycle.
- **Two test-file items in the mirrored `tests/rest_framework/` tier, both for the pass that next opens those files - the same fold-in convention this card already uses for its `_optimizer_field_map` and `[spec-011]` items.** (a) `tests/rest_framework/test_dry_import_ratchet.py`'s opening docstring wraps raggedly across its first lines; cosmetic, no gate sees it, and no consolidation pass remained in the cycle that wrote the file. (b) The inline-schema idiom in `tests/rest_framework/test_resolvers.py` is a consolidation candidate that was explicitly NOT held by the pass that raised it: re-measured 2026-09-10 with the instrument stated, **19** `class CategoryT` / `class ItemT` declarations, **12** `DjangoSchema(` calls and **14** `finalize_django_types()` calls. Both resolutions are legitimate - leave it (a test file where explicit local declaration aids readability) or extract a parametrizable schema builder - so this is a decision the batch makes when it is already in the file, never a defect to schedule on its own. Homed here 2026-09-10 by the spec-039 residual cycle.
- Slice 5 GLOSSARY flip - the three spec-050 anchors this flip must carry by name: `#listargumenterror` ([`ListArgumentError`](docs/GLOSSARY.md#listargumenterror)), `#async-queryset-completion-adapter` ([Async queryset completion adapter](docs/GLOSSARY.md#async-queryset-completion-adapter)) and `#list-offset-order-precondition` ([List offset order precondition](docs/GLOSSARY.md#list-offset-order-precondition)) all read `planned for 0.0.15` in `docs/GLOSSARY.md` while their code ships at `django_strawberry_framework/list_field.py`, so the generic every-card flip in this card's DoD is not enough of a pointer: flip each of the three `GlossaryTerm` rows to shipped in the same ORM edit and `scripts/build_glossary_md.py` run as the other glossary halves on this card. Never hand-edit the rendered file.
- **The subprocess-isolation idiom is spelled inline at every site - 8 call sites in 8 files spanning five `tests/` subdirectories plus the `tests/` root, with no shared helper.** Measured 2026-09-11 by the spec-040 residual reconciliation cycle over the git-tracked `tests/` tree, counting occurrences with the population named: the bare `sys.executable` token occurs **9** times in **8** files - `tests/auth/test_mutations.py`, `tests/auth/test_sessions.py`, `tests/base/test_init.py`, `tests/filters/test_finalizer.py`, `tests/filters/test_sets.py`, `tests/orders/test_inputs.py`, `tests/rest_framework/test_soft_dependency.py` (one each) and `tests/test_scalars.py` (two) - of which one of the two in `tests/test_scalars.py` is a docstring mention inside `tests/test_scalars.py::test_package_import_does_not_emit_strawberry_deprecation_warning`, so the **call-site** figure is **8** and the **occurrence** figure is **9**; a reading that states one number without naming which it is has already lost the population, and both spellings of that error are on the record. **State the instrument:** a first-argument match on `subprocess.run(\s*\[\s*sys\.executable` returns only **7** files, because `tests/base/test_init.py` builds its argv into a `cmd` local and calls `subprocess.run(cmd, ...)` - count the bare token instead. Control, files calling `subprocess.run(` at all: **10**; the two extras (`tests/test_bug_hunt.py`, `tests/test_ci_governance.py`) invoke `git` rather than an interpreter, read at source to confirm it. **A working prototype already exists, file-local:** `tests/auth/test_mutations.py::_auth_free_subprocess` builds a fakeshop-`sys.path` + `DJANGO_SETTINGS_MODULE` + `django.setup()` prologue, appends the caller's body verbatim and asserts the child's return code - and `tests/auth/test_sessions.py`, `tests/filters/test_finalizer.py`, `tests/filters/test_sets.py` and `tests/orders/test_inputs.py` each re-spell that same prologue inline - `tests/filters/test_finalizer.py` and `tests/orders/test_inputs.py` near-identically to the helper, `tests/auth/test_sessions.py` compressed onto a single `import django, os, sys;` clause, and `tests/filters/test_sets.py` newline-joined inside a `child` local - so the helper plus four inline re-spellings is five spellings of one prologue. **The open design question is what deferred this, and it is why the item is a decision rather than a chore. (a) Which conftest.** No single `tests/` subdirectory contains the population, so a subdirectory `conftest.py` cannot serve it; a root `tests/conftest.py` helper widens a contract every package test inherits; and the existing shared test module `tests/_soft_dependency.py` is the precedent for a non-conftest home, but its stated contract is in-process `sys.modules` absence simulation, not launching a child interpreter. **(b) Whether ONE contract can serve all three probe kinds,** which differ in the child's argv and not only in its body: the **import-isolation** probes (`tests/auth/test_mutations.py`, `tests/auth/test_sessions.py`, `tests/base/test_init.py`, `tests/filters/test_finalizer.py`, `tests/filters/test_sets.py`, `tests/orders/test_inputs.py`) assert a submodule is absent from a fresh `sys.modules` and five of the six configure Django in the child (only `tests/base/test_init.py` does not); the **soft-dependency** probe (`tests/rest_framework/test_soft_dependency.py`) must run with djangorestframework absent and asserts an install hint; the **warnings** probe (`tests/test_scalars.py`) needs `-W error::DeprecationWarning` on the child argv ahead of `-c`. If the answer to (b) is no, the right shape is two or three named helpers rather than one, and saying so with the reason is the deliverable. Homed here 2026-09-11 by the spec-040 residual reconciliation cycle, whose final gate carries the measurement; that cycle had already extracted the file-local helper named above out of the file's one pre-existing inline copy, so its second probe rides the helper instead of adding a fifth re-spelling.
- **The three soft-dependency floors each promise a Django range the package no longer advertises, and only one of the three can be closed by a bump.** Measured 2026-09-11 against installed metadata and PyPI: `pyproject.toml` advertises `Framework :: Django :: 5.2` / `6.0` / `6.1`, while `channels` at the floor every install hint interpolates (`django_strawberry_framework/utils/imports.py` #"CHANNELS_FLOOR = ") classifies 4.2 / 5.1 / 5.2 / 6.0 and **no released Channels version classifies 6.1 at all** - that floor is also the latest release - so on the channels side the floor cannot be made to reach the advertised ceiling and the prose is the only lever. The debug-toolbar floor is the opposite case: `7.0.0` classifies 5.2 + 6.0, but `7.1.1` - already the version the dev group resolves to - classifies 5.2 / 6.0 / 6.1, so moving the `[dependency-groups].dev` row, the hint floor constant and `uv.lock` together would make that one floor cover the whole advertised range. Decide both together, because the three install hints are deliberately byte-parallel and a bump on one side leaves the other two as the lone narrower promises. Nothing is asserting a falsehood while this waits: the spec prose on both sides now states only what is verified (`spec-041` by its own reconciliation cycle, `spec-042` and the [`DjangoGraphQLProtocolRouter`](docs/GLOSSARY.md#djangographqlprotocolrouter) glossary entry on 2026-09-11), so what is owed is the packaging call and not a text repair. One `pyproject.toml` site rides the same answer: the comment above the debug-toolbar row stops at #"Framework :: Django :: 6.0 classifier the package itself advertises." and so records only the half of the advertised range `7.0.0` covers. This card already owns the `[dependency-groups]` and extras surface through its WP-A packaging item; the beta release card excludes it by name (`docs/SPECS/spec-057-beta_release-0_1_0.md` #"rows or version floors is not this card"). **The classifier half of the question is nearly closed, and the reason is not in the metadata.** Qualifying or dropping the `Framework :: Django :: 6.1` classifier would make the channels range statement true by withdrawing a claim about the package's own tested surface: `.github/workflows/django.yml` runs four `6.1.*` matrix rows - Python 3.12 and 3.13, each single-DB and multi-DB - so 6.1 is durably exercised, and withdrawing the classifier to repair one soft dependency's wording would understate support for every consumer who never installs `channels` at all. Treat the classifier as fixed and the floors as the movable side unless that CI coverage changes. **The floor's ungated restatements ride the same bump, and the sweep that finds them is over the package NAME.** Measured 2026-09-12 by the spec-042 reconciliation cycle: `django-debug-toolbar>=7.0.0` is restated - correct today, gated by nothing - in `docs/GLOSSARY.md` #"django-debug-toolbar>=7.0.0" (DB-backed: an ORM edit plus a regenerate, never a hand-edit), `docs/README.md`, `CHANGELOG.md` #"Soft-dependency feature floors", the `docs/SPECS/appx/spec-042-debug_toolbar-0_0_14-terms.csv` soft-dependency row, and the body of `docs/SPECS/spec-042-debug_toolbar-0_0_14.md` itself. The terms.csv site is a **pair**: `import_spec_terms` loaded the same `notes` cell into `glossary_glossaryspecmention.notes` in `examples/fakeshop/db.sqlite3`, so fixing the CSV alone leaves the DB copy live. Sweep for the package name and read each hit rather than for the `django-debug-toolbar>=` specifier: the `CHANGELOG.md` site separates the name from the constraint with a space and a backtick, so the tight needle misses it. No absolute count is carried - it moves by more than twenty depending on whether `__pycache__`, the tracked DB and `temp-tests/` are inside the corpus, and the reproducible half is the delta between the two needles.
- **The DRF soft-dependency twin counts a spec Decision among its three places-that-must-agree, and its `pyproject.toml` side is the one floor with no governance row - close both in the edit that hoists the toolbar floor constant.** Measured 2026-09-12 by the spec-042 reconciliation cycle. `django_strawberry_framework/rest_framework/__init__.py` #"three-places-that-must-agree" - mirrored at `tests/rest_framework/test_soft_dependency.py` #"three-places-that-must-agree" - names the `[dependency-groups].dev` pin, the install hint, and the spec-039 soft-dependency Decision; the third is prose, not a gate, so the comment advertises a population one wider than anything that can fail. The toolbar twin, `django_strawberry_framework/middleware/debug_toolbar.py` #"named here rather", is the shape to copy: it separates GATED sites from a census and names its divergences rather than counting them. Unlike `channels` and `strawberry-graphql`, DRF has no `tests/test_ci_governance.py` row comparing its hint floor against the `pyproject.toml` row - `test_channels_floor_constant_matches_the_pyproject_dependency_row` and `test_strawberry_floor_constant_matches_the_pyproject_dependency_row` are the two that exist. The same edit closes the floor-constant asymmetry: `django_strawberry_framework/utils/imports.py` declares `CHANNELS_FLOOR` and `STRAWBERRY_FLOOR` and no toolbar floor, so `7.0.0` is typed literally into `_DEBUG_TOOLBAR_INSTALL_HINT` in the middleware - hoist it beside the other two, interpolate it into the hint, and add the third governance row. That refinement's stated trigger was a third regex-over-`pyproject.toml` governance row, which is exactly what this bullet asks for, so it lands as one change rather than two.
- **No gate fails an undeclared raw `.post(` / `.generic(` on a Django client in `examples/fakeshop/test_query/`, and the switchover's first three failures are already known.** Measured 2026-09-13 by the spec-043 reconciliation cycle. `docs/SPECS/spec-043-test_client-0_0_14.md` Decision 11 lets a live test keep a raw post only when the wire envelope is its subject, declared by a one-line comment naming the exemption - and nothing in `scripts/` or `.pre-commit-config.yaml` inspects live-tier HTTP call shapes, which is how five files drifted in two months (`START.md` "Rule w/o gate rots"). The cycle converted nine sites in four files (`test_error_policy_api.py`, `test_products_visibility_api.py`, `test_relations_async_api.py`, `test_resource_policy_api.py`); what remains is the gate and the three files it would fail on its first run: `examples/fakeshop/test_query/test_transport_api.py` and `examples/fakeshop/test_query/test_auth_api.py` declare the `graphql_client.py` raw-envelope exemption (the right regime for suites whose subject IS the envelope, but not a spec-043 declaration), and `examples/fakeshop/test_query/test_list_field_async_api.py` #"intentionally exempt from" declares the sync-only exemption over its one raw `AsyncClient` post (#"await http_client.post("), a file the spec-050 cycle owns. Count `.post(` AND `.generic(` - a `client.generic("POST", ...)` was invisible to the cycle's first `.post(` census - and leave `.get(` out: `test_debug_toolbar_api.py`'s five raw GETs fetch GraphiQL and toolbar panels, not GraphQL operations, and sit outside Decision 11's population. This card already owns `.pre-commit-config.yaml` and the CI workflow through its `lint-imports` item; the gate lands beside them.
- **Decide whether the live tier gets a shared async post helper, then rewrite the `test_query/README.md` `Async` bullet once - it is falsified today and its correct text depends on the decision.** Measured 2026-09-13 by the spec-043 reconciliation cycle. Five near-identical `await AsyncTestClient().query(..., assert_no_errors=False, url=...)` bodies now sit across four live modules (`test_error_policy_api.py`, `test_products_visibility_api.py`, `test_relations_async_api.py`, `test_resource_policy_api.py`), a sixth async site of a different shape is committed at `examples/fakeshop/test_query/test_kanban_mutations_api.py` #"client = AsyncTestClient()" (native async test, `async with client.login(user)`), and a seventh is the unconverted raw post in `test_list_field_async_api.py`. Extracting a shared async helper is a contract choice, not a DRY chore: `examples/fakeshop/graphql_client.py` is synchronous by construction and `test_list_field_async_api.py` #"that helper is synchronous by construction" cites exactly that as its exemption, so a helper reverses the regime the exemption vocabulary is built on. Both options and their costs are in the cycle's retired live-conversion artifact, retrievable with `git show 974d25b4:docs/builder/bld-043-review-4-live_conversion.md` - the commit that introduced it, unmodified before its retirement. Either way the README bullet at `examples/fakeshop/test_query/README.md` #"declare the helper exemption" is wrong now: it names `django.test.AsyncClient` and a helper exemption for BOTH `test_list_field_async_api.py` and `test_relations_async_api.py`, while the second drives `AsyncTestClient` and declares no exemption, and it names none of the other converted files. Corrected wording that keeps the still-true half (`test_list_field_async_api.py` really does drive `AsyncClient` under the sync-only exemption) is in that same retired artifact; write it after the decision so the bullet is not rewritten twice. If the decision is "no helper", the bullet fix alone moves to `TODO-ALPHA-056-0.0.17` (Alpha documentation-debt discharge).
- The package spells "a control signal is not a cleanup diagnostic" two ways: `consumers.py` enumerates `asyncio.CancelledError` / `KeyboardInterrupt` / `SystemExit` in 31 re-raise guard clauses ahead of broad handlers, while `django_strawberry_framework/resource_policy.py::_is_cleanup_diagnostic` asks whether the error is an `Exception` at all - the stricter form, since it also covers `GeneratorExit` and anything added to the hierarchy later. Fold the 31 `consumers.py` sites onto one shared predicate so the rule has a single spelling; no wire change.
- `django_strawberry_framework/resource_policy.py::clear_resource_context` is a public export (one of the module's 16 `__all__` names) with zero package callers: `grep -rn --include='*.py' clear_resource_context` returns four lines at `63a132be` - the definition, the `__all__` entry, and one import plus one call in `tests/test_resource_policy.py::test_clearing_the_context_restores_the_default_policy`. `django_strawberry_framework/extensions/resource_policy.py::DjangoResourcePolicyExtension` now brackets each operation with `django_strawberry_framework/utils/context.py::restored_context_keys`, which restores a pre-existing outer value rather than clearing, so no request path reaches the clear. Measured 2026-09-14 by the spec-047 reconciliation cycle. Same shape as this card's dead cascade-edge helpers bullet, with one difference that puts a maintainer ruling ahead of the delete: the name sits on a published `__all__`, so retiring it is a contract change rather than a DRY cut. Ruling owed - keep (the test row is then its only consumer and the spec's Slice 1 row stays true as written) or retire (then `docs/SPECS/spec-047-resource_policy-0_0_14.md`'s Implementation-plan Slice 1 row, its `__all__` arithmetic at #"Two names on this module's surface that no slice row above carries", the rationale's post-release change register, and `CHANGELOG.md`'s Removed section move with it). If retired, fold the deletion into the WP batch that opens `resource_policy.py`; do not open the module for this alone.
- Seal the `FilterSet.apply_*` return the way the `OrderSet.apply_*` return is sealed: one shared post-sidecar helper pair (`django_strawberry_framework/utils/querysets.py::apply_filterset_sync` / `::apply_filterset_async`, `::apply_orderset_sync` / `::apply_orderset_async`, all over `::_apply_sidecar_sync` / `::_apply_sidecar_async`), the same frozen routing intent and the one `_SIDECAR_RESULT_POLICY` (lazy, model rows of the captured model, unsliced, uncombined, same alias), called from both `connection.py` pipelines; the list field publishes no `filter:`. The spec-050 rationale's Decision 20 recorded the connection field's missing snapshot-plus-re-seal as a deferral to a card that was never opened; both halves are now discharged. The adjacent asymmetry is closed with it: a connection request carrying a sidecar input seals `get_queryset` under `_LIST_ARGUMENT_VISIBILITY_POLICY` (`connection.py::_sidecar_visibility_policy`), as the list field's argument path does. Package rows in `tests/test_connection.py`, live rows on the shipped genre connection in `examples/fakeshop/test_query/test_library_api.py`.
...
- Related: Amendment source: the `spec-037` residual cycle graded the missing file-column read-side `Meta.exclude` row Low and unplanned because that path is shared with the scalar case; this card's bundle migration is what un-shares it, so the row belongs here rather than on the spec-037 line. -> `DONE-037-0.0.11` - Upload scalar and file / image field mapping
...
- Extend `scripts/check_citations.py` to resolve `spec-<NNN> Decision <N>` occurring in first-party `.py` against the `### Decision <N>` headings of `docs/SPECS/spec-<NNN>-*.md`, alongside this card's source-symbol, unused-link-definition, cited-substring-uniqueness and generated-target checker siblings. **Record the caveat with the item, or the gate creates a false sense of coverage: it would have caught NONE of the four instances that motivated it.** The general class is that a prose reference from source into a spec Decision is invisible to every gate this repository has - `check_citations.py` resolves `path::Symbol` and deliberately excludes `docs/`, so it sees neither an ordinal INSIDE a Decision's item list, nor a citation by a Decision's heading text, nor a citation to a Decision that exists but does not state the claim. The extension catches only citations to a NON-EXISTENT Decision. The durable instrument is the spec-side convention already landed in `spec-033` Decision 6's introduction - a citation naming the arm by CONTENT carries its own claim, so target and claim are checkable in one read. Measured 2026-08-27 by the spec-033 residual reconciliation cycle (`docs/builder/DONE/build-033-connection_optimizer-0_0_9.md`, whose folded-in deferred-work catalog carries the full measurement). **Widened 2026-09-03 by the spec-038 residual cycle, which is the second cycle this missing gate cost a review round: scope the checker to any ordinal token after a `spec-<NNN>`, not to `Decision <N>` alone.** That cycle stripped 88 `P1`/`P2`/`P3` labels from `docs/SPECS/spec-038-form_mutations-0_0_12.md` - correct, since every spec whose residual cycle has run carries zero - and thereby falsified 32 ordinal citations in shipped `.py` source, which nothing in the repository could see; it was caught by an integration pass, not by a gate. So a `Decision <N>`-only extension is too narrow by construction: the live stranded vocabularies are `P1-B` / `P3a` / `P2-3` / `Md<n>` / `M1a` / `L3-1` / `FV-1`, none of which it would match. **Two provisions that cycle derived by reading, without which the gate publishes false positives of its own:** the range comparison must be EN-DASH-TOLERANT (`spec-044 D4-D5` and `spec-044 Test plan 1-7` read as unresolved against an ASCII-only `.py` citer purely on the dash), and resolution must reach a NUMBERED-LIST item and an ABBREVIATION rather than searching for a literal string (`spec-028 DoD 4(c)`, `spec-043 scenario 4` resolve against a `## Test plan` list; `spec-048 D1` against `### Decision 1`, and `D<N>` is a measured repo-wide shorthand - 26 such citations exist and 20 resolve). **The policy call is the maintainer's and blocks the gate:** which ordinal vocabularies stay legitimate (the readings so far say `Decision N` / `DoD N` / `TODO(spec-NNN slice N)` should), and whether `docs/` enters scope at all - `check_citations.py` excludes it by design, so a citation FROM a spec is out of reach even after this extension.
...
- **The `0.0.14` mutation write hardening has no owning spec, and `spec-036` is not it.** Nine specs carry the `0_0_14` filename segment and **none** scopes the completion-spanning transaction, the `DjangoSchema` requirement, `Meta.select_for_update`'s lock semantics, single-write-alias pinning, the retryable `conflict` envelope, the immutable authorized-pk snapshot, the phased alias guard, the strict-`bool` authorization contract, or the point-in-time authorization rule; the corpus-wide grep's top hit is `spec-039` (10 occurrences), the serializer card at `0.0.13`, and `require_managed_write` greps to **0** across the whole board. The spec-036 residual cycle deferred **10 rows** on the maintainer's CORRECT-CLAIMS-ONLY ruling, because folding them in would make `spec-036` retroactively the spec for a pipeline it never scoped; that spec's corrected Decisions 8, 10 and 15 are the BOUNDARY of what `036` claims, not an outline for the missing spec. Decide as ONE ruling, the same shape as this card's keyset-cursor item. **Third sighting of this class** - a shipped surface with no owning spec silently inverts the specs describing it - previously homed on `051` / `052` / `064` by the spec-033 cycle. Measured 2026-09-01.
- **A failed generated mutation field nulls the whole response, hiding earlier fields that already committed.** Generated mutation payloads are non-null (`CreateItemPayload!`), so an error on a top-level generated mutation field - a completion failure, or a window whose connection was closed inside it (`django_strawberry_framework/schema.py::DjangoMutationExecutionContext._refuse_uncommitted_window`) - propagates to the `Mutation` root and the response carries `data: null`, although each serial field before it committed its own transaction. This is spec-correct non-null propagation, stated in `docs/README.md`'s `DjangoSchema` paragraph; the only fix is nullable generated payloads, a public-schema change for every consumer. Decide it with the owning spec for the `0.0.14` write hardening (the item above). Measured 2026-09-25.
...
- **Retire the stranded ordinal citations in `tests/` and `examples/`: 13 sites in 7 files, five spec vocabularies plus three that name no spec at all.** Measured 2026-09-03 by the spec-038 residual cycle, whose per-token derivation is in `docs/builder/DONE/build-038-form_mutations-0_0_12.md` `## Final test-run gate (folded in from the retired final report)`; the sibling population inside `TODO-ALPHA-053-0.0.15`'s WP files is homed on that card, and the split is this card's existing rule for the spec-016 spellings - a site in a file no WP batch opens belongs to this card's repo-wide sweep. Sites: **spec-030** `P1-B` x3 in `examples/fakeshop/test_query/test_library_api.py`, `P3a` x1 in `tests/test_connection.py` #"(``spec-030-connection_field-0_0_9`` P3a)", `P3b` x1 in `tests/test_registry.py` (spec-030 carries 0 of all three; its rationale companion carries 5 / 4 / 3, which is where the labels went). **spec-032** `the P1 bug` and `the P2 bug` x2 in `tests/test_relay_node_field.py` (0 in the spec, 47 in the companion). **spec-036** `FV-1` x1 in `tests/mutations/test_resolvers.py`. **spec-039** `SR-3` x1 in `tests/rest_framework/test_converter.py`. **spec-016** `Decision 4` x1 in `test_library_api.py` #"Pins the end-to-end contract (spec-016 Decision 4," - and `docs/SPECS/spec-016-fieldmeta_consolidation-0_0_6.md` carries the string `Decision` **0** times, so either the ordinal names nothing or the target is another spec and this is the renumber-artifact treatment instead. **Three name no spec anywhere in their file and need the authoring cycle to decode, which no reader can recover:** `tests/test_lateral_pg_parity.py` #"P2-4: two callers of ONE extension", `examples/fakeshop/apps/library/tests/test_generic_connection_sharded.py` #"Pins the P1 fix:", and `test_library_api.py` #"the P0 served it 1". **Coordination: `test_library_api.py` alone carries three catalog rows** (the spec-030 x3, the spec-016 x1 and one undecodable), so open it once. **The population must move per spec, never per file:** spec-030's seven sites split 5 here / 2 on `053`, and retiring one half leaves that vocabulary divergently rather than uniformly wrong - the same half-fix argument this card's `[spec-011]` item already records. **Do not trust a single census glob:** five instruments were pointed at this class and the graded population moved 11 -> 41 -> 16 -> 38 -> 37 -> 36, each blind where the last was not (`\bP[123]\b` cannot see a suffixed label; `\bP[0-9]+([.\-][0-9A-Za-z]+)?\b` cannot see `P3a` or a non-`P` vocabulary; a per-line resolver cannot see a citation wrapped across two comment lines; and all of them over-report). The durable fix for the class is this card's `check_citations.py` ordinal extension, not a sixth manual sweep. **Re-measured 2026-09-10 by the spec-039 residual cycle: the spec-039 `SR-3` site is DISCHARGED and this card's population is 12, not 13.** HEAD (`4c483b6b`) carried two `SR-3` occurrences, not one - `tests/rest_framework/test_converter.py` and a package-side sibling in `django_strawberry_framework/rest_framework/serializer_converter.py` that was `TODO-ALPHA-053-0.0.15`'s - and both are now 0, so the two halves of that vocabulary retired together and neither card is left holding a divergent half. Do not re-raise it. The same cycle's instrument note belongs beside this bullet's own do-not-trust-a-single-census-glob warning: a left-anchored `(^|[^A-Za-z0-9_-])TOKEN` alternation returns 0 under `ugrep` even where the token is live, which is a seventh blind spot on top of the six graded above.
- **Two instrument hazards in the builder corpus, both needing a corpus-ratchet edit that names the bytes it retires.** (a) **Bare `/tmp` in three recipes across two standing docs** that `START.md` itself forbids: `docs/builder/BUILD.md`'s floor-venv recipe and its failability-proof recipe, plus a second copy of the floor recipe in `START.md` under #"Floor is executed, never reasoned" - so repairing only `BUILD.md` leaves `START.md` prescribing it. `scripts/prove_failability.py` is a docstring-only fourth site whose code already honors `TMPDIR`. The confirming datum, measured 2026-09-10 by the spec-039 residual cycle: the `dsf-floor` path two recorded floor runs name has lost its `pyvenv.cfg` while a sibling `dsf-floor-pass2` is intact, so the decay is real and a floor record naming only a scratch path is not auditable afterwards. (b) **No recipe asks a floor run to prove its directory is a venv** - `pyvenv.cfg` and `sys.prefix` return 0 hits across `docs/builder/BUILD.md`, `docs/builder/ARTIFACT.md` and all four `docs/builder/worker-*.md` - and the recorded `uv venv ... | tail -3 && ...` shape reports the PIPE's exit status, so a refused venv followed by two refused installs reads as exit 0. No floor claim recorded by that cycle is falsified by either hazard; both are gaps in what a later reader can audit. Homed here 2026-09-10 by that cycle, which could not take them under a spec-and-`.py` fence.
- **`BACKLOG.md`'s error-envelope spec entry is the last `rev6`-vocabulary citer of `spec-039` in the tree, and it is a stranded ordinal.** It reads `BACKLOG.md` #"spec-036 Decision 7 + spec-039 rev6", naming a numbered revision vocabulary that the spec-039 residual cycle retired on 2026-09-10 - `docs/SPECS/spec-039-serializer_mutations-0_0_13.md` now carries 0 occurrences of `rev6`, so both the token and the two ordinals behind it resolve to nothing. One substitution, recorded by that cycle so the next pass does not re-derive it: replace the parenthetical's `spec-039 rev6 #4/#13` with a content restatement - `spec-039`'s error-envelope `codes` / `path` improvements - which is the same restate-by-content remedy this card's stranded-ordinal bullets prescribe, and never a renumber. Homed here 2026-09-10 by that cycle; `BACKLOG.md` was outside its spec-and-`.py` fence.
...
- **A second stale card-id population sits beside the `062` one and has never been enumerated on this board: the migration-and-adoption-guides card.** Measured 2026-09-11 by the `spec-041` residual reconciliation cycle over the whole tracked tree, counting occurrences rather than lines and taken **before this row existed**: `TODO-BETA-068-0.1.8` occurs **11 times across 5 tracked files** - `docs/SPECS/spec-041-channels_router-0_0_14.md` 3, `docs/SPECS/spec-042-debug_toolbar-0_0_14.md` 2, `docs/SPECS/spec-043-test_client-0_0_14.md` 3, `BACKLOG.md` 1, and `docs/builder/DONE/build-027-filters-0_0_8.md` 2 - so **8 of the 11 sit under `docs/SPECS/`**, where the six-file `spec-03[4-9]` glob the `062` row warns about reaches none of them. The id is wrong in both halves at once, which is why no +1 repairs it: the live guides card sits at `0.1.8` under a higher number, while the numeral `068` today names the structural-optimization card at `0.1.6`, so shifting only the numeral manufactures a version that contradicts the card it then names - read each site against its live referent exactly as this card's bare-numeral row prescribes. Two of the 11 are decided non-edits on grounds this card already establishes: `BACKLOG.md` #"relocated 2026-07-20 from the migration-guides card" dates its own spelling and names the target in prose, and the `docs/builder/DONE/build-027-filters-0_0_8.md` pair are closed-cycle records rather than live claims. **This row's own single occurrence of the id is board-side and excluded from that total**, per the counting rule the `062` row establishes; it reaches `KANBAN.md` and `KANBAN.html` only as two renders of this one `CardItem`, so a whole-tree byte sweep taken after this row lands will score 14 where 11 are sweepable. Discharge it in the same pass as the `062` population: both were created by the same 2026-08-29 board inserts, three of the five files carry both ids, and a pass that renumbers one and not the other leaves each spec disagreeing with itself.
- **`docs/GLOSSARY.md`'s [Debug-toolbar middleware](docs/GLOSSARY.md#debug-toolbar-middleware) entry enumerates two robustness divergences where the module names three, and the spec's `-terms.csv` row still describes the pre-flip status.** Measured 2026-09-12 by the spec-042 reconciliation cycle at `docs/GLOSSARY.md` #"Two deliberate robustness divergences": the entry names the `isinstance(view, type)` guard and the `_get_payload` response-shape bail, while `django_strawberry_framework/middleware/debug_toolbar.py` #"named here rather" names a third - `_postprocess` returns an already-encoded response untouched, since appending the bridge script to a compressed body corrupts it - and names them rather than counting them, which is the shape the entry should copy. The entry's status line already reads shipped (`0.0.14`) while `docs/SPECS/appx/spec-042-debug_toolbar-0_0_14-terms.csv` still says the GLOSSARY status is planned for `0.0.14` until the joint cut flips it; that `notes` cell is read by no gate, so it is corrected here or not at all. DB-backed: an ORM edit plus a regenerate, never a hand-edit.
- **Two citation residues in archived specs, each measured at ONE site rather than the population the spec-042 cycle assumed.** Measured 2026-09-12. (a) Decision 1's "This spec lives at `docs/spec-...`" template sentence: `docs/SPECS/spec-043-test_client-0_0_14.md` #"This spec lives at" still gives the pre-archive `docs/` path, while `spec-044` and `spec-046` already carry `docs/SPECS/` and `spec-023`'s occurrence states the in-flight rule with an `<NNN>` placeholder and is not a defect - so this is one edit, not one per archived spec. (b) `docs/SPECS/spec-041-channels_router-0_0_14.md` cites `utils/imports.py::require_optional_module` package-relative where `AGENTS.md` rule 27 wants the repo-relative form; the two sibling sites in `docs/SPECS/spec-042-debug_toolbar-0_0_14.md` were corrected by that spec's reconciliation cycle and this is the last one in the tree. Neither is gated - `scripts/check_citations.py` reads first-party `.py` files and `KANBAN.md`, never `docs/` - which is why both belong beside this card's citation-checker items.
- `docs/SPECS/appx/spec-042-debug_toolbar-0_0_14-rationale.md` #"three sentences in" claims three sentences in `spec-043` cite "spec-042 Revision 8" by name. Measured 2026-09-13 by the spec-043 reconciliation cycle, after that cycle moved spec-043's deliberative layer into its own companion: the spec body carries **0** occurrences and `docs/SPECS/appx/spec-043-test_client-0_0_14-rationale.md` carries **1**. The sentence's substance survives - the citations did find the destination it predicted - and only its arithmetic is stale; same shape as this card's `spec-012` rationale bullet, one measured site, one-line fix, and a candidate row for the spec/rationale consistency checker this card scopes (a count stated in one companion about another companion's body).
- `docs/SPECS/appx/spec-047-resource_policy-0_0_14-terms.csv` line 30 reads `Joint version cut,joint-version-cut,the rule this card is NOT subject to`, and both documents beside it now say the opposite: `docs/SPECS/spec-047-resource_policy-0_0_14.md` #"the release rule this card is subject" and `docs/SPECS/appx/spec-047-resource_policy-0_0_14-rationale.md` #"is now the" record that card 047 IS subject to the joint version cut. Measured 2026-09-14 by the spec-047 reconciliation cycle (`sed -n 30p` on the CSV; `grep -c 'spec-047-resource_policy-0_0_14-terms.csv' KANBAN.md` returned 0, so this instance was named on no card and would have died unlisted). Another member of the population this card's #"Decide whether the `notes` column of a spec's `-terms.csv` is contract text" ruling item governs: `scripts/check_spec_glossary.py::load_terms` reads `term,anchor` only, so the cell fails no gate and `check_spec_glossary.py` reports `OK` over it. Fix under that ruling, not before it: flip the cell to name the rule the card is subject to if the column is contract text, or blank it if the column is ruled scratch.
- `docs/GLOSSARY.md`'s `## Value-budget walker` entry states the classification rule spec-047 Decision 4 retired. It reads #"Families are classified by the argument's GraphQL INPUT TYPE, not by name" and "a list of `ID` inside a mutation is a relation-id set", while `django_strawberry_framework/extensions/resource_policy.py::_ValueBudget` classifies a mutation's list by the field's bind-time spec first (#"spec.kind == RELATION_MULTI", read through `::_mutation_input_specs`) and falls back to the `ID`-scalar rule only for an id list the package did not generate - so a raw-pk `[Int!]` relation list is charged as relation ids, where the entry's rule would price it as a membership list. Verified 2026-09-14 by the spec-047 reconciliation cycle against the reconciled spec at `63a132be`. The same entry is silent on two charges the spec now states: an operation variable definition's own default is folded into the variable map and charged when the request omits the variable (`docs/SPECS/spec-047-resource_policy-0_0_14.md` #"variable definition's own default"), and a bytes-like scalar (`bytes` / `bytearray` / `memoryview`) is charged by `nbytes` / `len` against `max_scalar_bytes`, where the entry names only "scalar UTF-8 byte size". The cycle's positive-controlled sweep of the glossary for eight retired absolutes returned 0 and could not see this, because the drift is a superseded rule stated in different words rather than a retired phrase. DB-generated - edit the fakeshop glossary app's `GlossaryTerm.body` and re-render with `scripts/build_glossary_md.py`, never hand-edit.
...
- Async-path constraint. Every async path today wraps its sync body in `sync_to_async(..., thread_sensitive=True)` — `FilterSet.apply_async`, `OrderSet.apply_async`, `aapply_cascade_permissions`, `resolve_mutation_async` — which serializes them onto the calling context's one thread-sensitive worker (asgiref's process-wide worker when no `ThreadSensitiveContext` is active). A generated mutation is the exception: `django_strawberry_framework/schema.py::DjangoMutationExecutionContext` gives each mutation field's transaction window a private thread and connection, so `resolve_mutation_async` runs there and never on the shared worker. That is a deliberate connection / consumer-hook safety choice, not a bug, but it is the constraint the async `acompute` gather must design around: the gather must run genuinely independent units on their own connections and never re-enter the shared sensitive thread.
...
- Four test-quality / DRY items in `examples/fakeshop/test_query/test_products_api.py`, all deliberately deferred by the spec-036 residual cycle (2026-09-01) to the pass that next opens that file for its own reasons - which is this card. **(a)** The denial assertion is spelled at **9** sites and wants an `_assert_denied(response)` helper; the existence challenge is clear on 9 callers, but **3** of those sites carry a site-specific comment explaining that the denial nulls the whole `data` rather than producing an in-band `FieldError` - the very distinction those rows exist to pin - so the item is not the helper, it is deciding where that explanation lives once the helper hides it. **(b)** The grant-a-codename block is a three-way near-copy; split `_grant_perms()` out of `::_login_with_perm` rather than adding a fourth copy. **(c)** GlobalID type-name literals are unnamed - `"products.category"` **46** times and `"products.item"` **32** - and want two module constants, not a factory. **(d)** `::test_create_item_via_serializer_multipart_upload_to_attachment` builds a local `createItemViaSerializer` document while the module-level `_CREATE_ITEM_VIA_SERIALIZER` already spells that field. The selections genuinely differ today so it is **not** a duplicate - recorded because the file's own convention is "each wire contract spelled once", and two spellings is how that convention erodes.
...
- Document the Channels ASGI router migration row in the `strawberry-graphql-django` guide's symbol-equivalents table, with the three reasons it is not a drop-in rename. `strawberry_django.routers.AuthGraphQLProtocolTypeRouter` maps to `django_strawberry_framework/routers.py::DjangoGraphQLProtocolRouter`, but the constructor differs in ways a migrant hits on the first run: `django_application` is **required** here and second-positional, where upstream defaults it to `None` and builds its own; the websocket route argument is `websocket_url_pattern` with default `r"^graphql/?$"`, where upstream spells it `url_pattern` with default `"^graphql"`; and upstream's router also serves GraphQL over HTTP from the same object, while this one dispatches `"http"` straight to the project's own ASGI handler - so the HTTP half is a separate URLconf entry for `django_strawberry_framework/views.py::DjangoGraphQLView` rather than something the router provides. The card that shipped the router hands this card the row content rather than the symbol mapping alone; the mapping itself already arrives as that card's related reference.
...
- **The bridge asset's guards are pinned by text and by no execution, and whether that stays permanent is a contract call the spec-042 cycle parked for the maintainer.** `django_strawberry_framework/templates/django_strawberry_framework/debug_toolbar.html` is covered by a 57-row predicate table in `tests/middleware/test_debug_toolbar.py`, every row a text-identity check over the file on disk: a green table proves each guard is present, single-sited and correctly ordered, and says nothing about whether the JavaScript runs - while three of the four documented divergence families exist specifically to survive runtime inputs (null-prototype objects, absent DOM nodes, a shadow root). The gap is narrowed in one direction and measured rather than argued: all eight absence needles occur exactly once in upstream's asset and never in the port, so the rows do detect a verbatim paste of upstream's text, and a rewrite of the same hole in different words passes every one of them. Three resolution paths, from the original escalation: (a) accept text-identity pinning permanently and say so in the spec - already half-taken, `docs/SPECS/spec-042-debug_toolbar-0_0_14.md` #"The bridge asset's guards are pinned by text, never by execution." states the limit and names the maintainer as the risk's owner; (b) add a minimal DOM harness for this asset alone; (c) narrow the asset so less of it needs guarding. Path (b)'s cost is the substance of the decision and was measured at the cycle's gate: `node` v24.10.0 is installed, but a bare `node` carries no `CSS` global and no `document`, so even `CSS.escape(panelId)` with its empty-identifier skip cannot be exercised as it stands - (b) means jsdom or equivalent as a new dev dependency, for one 89-line asset. Full reasoning in `docs/builder/DONE/build-042-debug_toolbar-0_0_14.md` `## The JS-runtime question`.
- **`scripts/prove_failability.py`'s verdict arithmetic has no row-isolation case, so a proof that succeeded reads as a finding.** Measured 2026-09-12 by the spec-042 reconciliation cycle. The tool labels a one-row result `WEAKLY PINNED - revision-needed` because its arithmetic is written for boundary proofs, where one failing row means one assertion holds the guard. A row-isolation proof - does this row produce a failing node id no other row produces - seeks EXACTLY one row, and a second would falsify it, so the tool's verdict line inverts the reading. Twice in that one cycle a pass had to write a paragraph explaining that the verdict does not apply. Either teach the tool the proof kind (a manifest flag naming the expected row count) or have it print the count without a verdict when the manifest does not declare one. Sits beside this card's failability-manifest re-anchoring item, which opens the same tool.
- `tests/base/test_conf.py` has no row pinning that `django_strawberry_framework/conf.py::testing_endpoint_setting` returns a non-`str` setting value verbatim - every existing row asserts a `str` (`/custom/`, `/graphql/`). Measured 2026-09-13 by the spec-043 reconciliation cycle, which restated the docstring to say the helper does not validate or coerce and left the row out by decision (spec-043 rationale Decision 7: the claim the row would pin is about Django's own `str()` coercion at `urlsplit(str(path))`, already established by reading every cached wheel `5.2.0` -> `6.1.0`). The docstring claim is therefore pinned by nothing executable; a one-row adversarial pin (a `Path`, an `int`) belongs with this card's other missing-pin rows. `tests/base/` may grow rows but no files.
...
<a id="generated_input_field_names_follow_the_schema_naming_config_auto_camel_case_nameconverter"></a>
### [TODO-BETA-077-0.1.7 - Generated input field names follow the schema naming config (`auto_camel_case`, `NameConverter`)](KANBAN.html#generated_input_field_names_follow_the_schema_naming_config_auto_camel_case_nameconverter)

- Priority: Medium
- Status: To Do
- Relative size: L
- Labels: `config`, `filters`, `mutations`, `ordering`, `public-api`

#### Predicted files

- [`django_strawberry_framework/extensions/resource_policy.py`](django_strawberry_framework/extensions/resource_policy.py)
- [`django_strawberry_framework/filters/inputs.py`](django_strawberry_framework/filters/inputs.py)
- [`django_strawberry_framework/forms/inputs.py`](django_strawberry_framework/forms/inputs.py)
- [`django_strawberry_framework/mutations/inputs.py`](django_strawberry_framework/mutations/inputs.py)
- [`django_strawberry_framework/orders/inputs.py`](django_strawberry_framework/orders/inputs.py)
- [`django_strawberry_framework/rest_framework/inputs.py`](django_strawberry_framework/rest_framework/inputs.py)
- [`django_strawberry_framework/rest_framework/serializer_converter.py`](django_strawberry_framework/rest_framework/serializer_converter.py)
- [`django_strawberry_framework/scalars.py`](django_strawberry_framework/scalars.py)
- [`django_strawberry_framework/utils/inputs.py`](django_strawberry_framework/utils/inputs.py)
- [`django_strawberry_framework/utils/write_values.py`](django_strawberry_framework/utils/write_values.py)

#### Planning note

Under `StrawberryConfig(auto_camel_case=False)` or a custom `NameConverter`, output fields and list arguments follow the config (`order_by`, `page_info`) while every package-generated filter, order and mutation input field stays pinned camelCase (`createdDate`, `iContains`, `categoryId`), so one schema mixes both. Upstream strawberry-graphql-django leaves input fields unpinned (`fields/filter_types.py::BaseFilterLookup`, `ordering.py::OrderSequence.get_graphql_name`) and follows the config. Surfaced by the 0.0.15 REVIEW run (utils/strings.py, Mechanics); a probe under `auto_camel_case=False` over the products app printed the mixed SDL.
...
- Stop pinning `name=` on package-generated input fields except where a Python keyword forces it (`in_`, `and_`, `or_`, `not_`); mark them so the schema's converter can name them.
- `django_strawberry_framework/scalars.py::strawberry_config` (and `DjangoSchema`) install a converter wrapping the consumer's: for marked fields `django_strawberry_framework/utils/strings.py::graphql_camel_name` under the default config (keeps `field_2` and `field2` distinct), the consumer's `apply_naming_config` otherwise.
- `django_strawberry_framework/filters/inputs.py::LOOKUP_NAME_MAP` entries follow the config (`i_contains`, `is_null` under snake case).
- Runtime readers of the pinned name read the name the built schema published instead, the way `django_strawberry_framework/list_field.py::_published_wire_name` does: `django_strawberry_framework/extensions/resource_policy.py::_nested_specs_map` (the resource budget; a wrong name silently skips the limit), the mutation, form and serializer resolvers, and `utils/write_values.py`.
- Decide a plain `strawberry.Schema` built without `strawberry_config`: document it or refuse it.

#### Definition of done

- [ ] Under `auto_camel_case=False` every package-generated input field, lookup and logic key is snake_case in SDL and filters, orders and mutates correctly end to end; under the default config the SDL is byte-for-byte unchanged.
- [ ] A custom `NameConverter` names generated input fields the way it names output fields.
- [ ] The resource budget and write-error field names match the published wire names under both configs (package tests building schemas with each config).
- [ ] `docs/README.md` and the filter, order and mutation glossary rows state that generated input names follow the schema naming config.

#### Why it matters

- `auto_camel_case=False` is a supported, documented knob (`docs/GLOSSARY.md` shows `strawberry_config(auto_camel_case=False, ...)`; list arguments already follow it), yet a client of such a schema must write `order_by: [{createdDate: ASC}]`: two naming conventions in one operation.

#### Dependencies

- Filtering subsystem: `LOOKUP_NAME_MAP` and the logic keys this card unpins.
- [Ordering](docs/GLOSSARY.md#ordering) subsystem: the order input fields this card unpins.
- [`DjangoListField`](docs/GLOSSARY.md#djangolistfield) argument surface: list arguments already follow the naming config; this card brings the inputs in line.

#### Open question

- Input classes are process-global and built before any schema exists (`finalize_django_types()`, mutation binding), so one class can serve schemas with different configs; the wrapper must name per schema, not per class.

#### Card references

- Dependency: Filtering subsystem: `LOOKUP_NAME_MAP` and the logic keys this card unpins. -> `DONE-027-0.0.8` - Filtering subsystem
- Dependency: Ordering subsystem: the order input fields this card unpins. -> `DONE-028-0.0.8` - Ordering subsystem
- Dependency: `DjangoListField` argument surface: list arguments already follow the naming config; this card brings the inputs in line. -> `DONE-050-0.0.15` - `DjangoListField` argument surface: `offset` / `limit` and `orderBy`
- Related: Configurable filter/logic key namespace also moves import-time wire names to schema-build time; land the two resolution points together. -> `TODO-BETA-070-0.1.7` - Configurable filter/logic key namespace (`FILTER_KEY`/`AND_KEY`/`OR_KEY`/`NOT_KEY`)
...
- [x] Define the shared `errors: list[FieldError]` envelope type for typed validation errors at the package boundary; `DONE-038-0.0.12`, `DONE-039-0.0.13` and `DONE-040-0.0.13` return that same class rather than a per-flavor mirror, so a client's error branch is written once. Shape mirrors graphene-django's `ErrorType` - `field` (a model field name, or the `"__all__"` sentinel for a model-wide or multi-field-constraint error) plus `messages`. The type is ADDITIVE, not frozen: `codes` and `path` were added at `0.0.13` alongside `DONE-039-0.0.13` without disturbing the shared-return contract, so it carries **four** members today.
...
- Related: Define the shared `errors: list[FieldError]` envelope type for typed validation errors at the package boundary; `DONE-038-0.0.12`, `DONE-039-0.0.13` and `DONE-040-0.0.13` return that same class rather than a per-flavor mirror, so a client's error branch is written once. Shape mirrors graphene-django's `ErrorType` - `field` (a model field name, or the `"__all__"` sentinel for a model-wide or multi-field-constraint error) plus `messages`. The type is ADDITIVE, not frozen: `codes` and `path` were added at `0.0.13` alongside `DONE-039-0.0.13` without disturbing the shared-return contract, so it carries **four** members today. -> `DONE-038-0.0.12` - Form-based mutations (Django Forms / ModelForms)
- Related: Define the shared `errors: list[FieldError]` envelope type for typed validation errors at the package boundary; `DONE-038-0.0.12`, `DONE-039-0.0.13` and `DONE-040-0.0.13` return that same class rather than a per-flavor mirror, so a client's error branch is written once. Shape mirrors graphene-django's `ErrorType` - `field` (a model field name, or the `"__all__"` sentinel for a model-wide or multi-field-constraint error) plus `messages`. The type is ADDITIVE, not frozen: `codes` and `path` were added at `0.0.13` alongside `DONE-039-0.0.13` without disturbing the shared-return contract, so it carries **four** members today. -> `DONE-039-0.0.13` - DRF serializer mutations (`SerializerMutation`)
- Related: Define the shared `errors: list[FieldError]` envelope type for typed validation errors at the package boundary; `DONE-038-0.0.12`, `DONE-039-0.0.13` and `DONE-040-0.0.13` return that same class rather than a per-flavor mirror, so a client's error branch is written once. Shape mirrors graphene-django's `ErrorType` - `field` (a model field name, or the `"__all__"` sentinel for a model-wide or multi-field-constraint error) plus `messages`. The type is ADDITIVE, not frozen: `codes` and `path` were added at `0.0.13` alongside `DONE-039-0.0.13` without disturbing the shared-return contract, so it carries **four** members today. -> `DONE-040-0.0.13` - Auth mutations (login / logout / register)
