...
- `tests/types/test_base.py:1278` carries a comment naming `convert_relation`, a symbol with zero occurrences in `django_strawberry_framework/` - relation annotations resolve through `types/converters.py::resolved_relation_annotation`. Fold into whichever WP batch legitimately opens `test_base.py`; it is the same shape as the `_optimizer_field_map` item above and takes the same boundary - the present-tense survivals in shipped specs (`spec-008`, `spec-009`, `spec-010`, `spec-019` all still name `convert_relation`) are correct as history and are not in the sweep.
...
- The spec rationale companion file is on its third hand-reproduced instance: `spec-001-django_types-0_0_1`, `spec-002-optimizer-0_0_2`, and `spec-003-optimizer_nested_prefetch_chains-0_0_2` each rebuild the same shape by hand - the deliberative-companion opener, `## How to read this file`, `## Provenance of this record`, `## Entries keyed to the spec`, `## Standing notes`, and the link-definition scaffold at `docs/SPECS/appx/` depth. Decide whether it becomes a documented template; the natural home is beside the spec/rationale consistency checker this card already scopes.
