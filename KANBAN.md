...
- `spec-010-foundation-0_0_4.md`'s two inbound citations into `spec-009-rich_schema_architecture-0_0_4.md` both resolve to something other than the claim they are cited for, and both are stale at HEAD independently of the spec-008 residual cycle that measured them. `spec-010:65` cites `spec-009 (670-687)` and `spec-010:408` cites `spec-009 (1076-1077)`; the second lands on spec-009's multiple-`DjangoType`s question while the error requirement it is cited for sits eight lines earlier at `spec-009 #"### Decision 6: fail loudly"`. Both point into `spec-009 #"### Layer 3: Finalization trigger"`, which is the larger defect: that section still presents hybrid auto-finalization as the preferred direction, and the spec-008 reconciliation established that the direction was REJECTED - an explicit `finalize_django_types()` call is the sole trigger and nothing auto-finalizes. So the citations are a symptom and the section is the cause; the fix is a spec-009 residual reconciliation cycle rather than two pointer edits, which is the disposition every residual cycle from spec-002 onward has taken and which needs no card of its own. This bullet carries the pointer half so the measurement does not die with that cycle's artifacts.
- `spec-010-foundation-0_0_4.md` carries the board's largest single rule-27 debt, and closing it is a section retirement rather than a sweep. Measured 2026-08-14: 42 raw `path:NN` occurrences on 30 lines, of which 20 occurrences on 15 lines are in-repo citations that `AGENTS.md` rule 27 forbids and 22 on 15 lines are pinned third-party prior art at a named upstream version, which the rule does not reach. Two of the in-repo refs (`spec-010:299`, `:383`) sit inside pseudo-code comments, where a symbol-qualified form has to be written rather than substituted. The conversion is not the blocker: `spec-010 #"## Note on source line references"` institutionalizes the practice the conversion removes, so that section has to be retired in the same change, and that is an edit to a shipped spec outside the sites its own decisions name. It therefore needs the maintainer to authorize the spec-010 edit before any pass may start. Settle it together with this card's note item on whether rule 27's exemption widens to `docs/builder/build-*.md` - both are the same question about what the rule reaches, and answering them apart is how one answer comes to contradict the other.
