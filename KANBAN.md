...
- Six test-module docstrings cite a spec at `docs/spec-NNN-...` where the file lives at `docs/SPECS/`: `tests/test_connection.py`, `tests/test_list_field.py`, `tests/test_relay_node_field.py`, `tests/test_relay_connection.py`, `tests/optimizer/test_multi_db.py`, and `tests/testing/test_relay.py`. All six are stale path claims - the specs were archived by a later cycle's `docs/SPECS/NEXT.md` Step 8 sweep and the docstrings were not re-relativized. **The population is 12 occurrences and only these 6 are edits.** The other 6 are decided non-edits: `scripts/check_spec_glossary.py`'s 4 are usage examples in a docstring whose own prose then discusses what an archived spec keeps, so the pre-archive form is the point; and `examples/fakeshop/test_query/test_glossary_api.py`'s 2 (`spec_path=` and the `specPath: { exact: ... }` assertion) are fixture DATA matching a glossary DB row, not a path claim - rewriting them breaks the test. A sweep handed an undifferentiated "12 stale spec paths" would break a passing test and destroy a deliberate example. Measured by the spec-009 residual cycle (`docs/builder/bld-009-final.md` catalog item 22, which recorded only the `test_connection.py` site); population enumerated 2026-08-16. The board-DB half of this same defect is carried by `TODO-ALPHA-053-0.1.0`.
...
- `AGENTS.md` rule 31 - bump `pyproject.toml` `[project].version` and `django_strawberry_framework/__init__.py` `__version__` together, both must match - is enforced by no gate this board runs, and this card's version-quintet box is where one belongs. A mechanical comparison does exist and is easy to mistake for the gate: `scripts/bug_hunt.py::_package_release` reads both files and raises `#"bump them together"` on a mismatch. Three measurements taken 2026-08-15 show it is not one. It appears in neither `.github/workflows/` nor `.pre-commit-config.yaml`, so it fires only when a maintainer hand-runs the bug-hunt agentflow; `scripts/bug_hunt.py::main` reaches it only when `--target-release` is omitted, and `tests/test_bug_hunt.py::test_target_release_overrides_mismatched_package_versions` pins that bypass as deliberate; and both of its tests build synthetic `tmp_path` trees, so nothing in the suite has ever compared this repository's own two files. The third surface is independent of both - `tests/base/test_init.py::test_version` asserts `__version__` against a bare literal and reads no other file - so the release string is written in three places and compared in none of them under CI, pre-commit, or the suite. The smallest honest fix is a test beside `::test_version` that reads the real `pyproject.toml` `[project].version` and asserts equality, which also turns that literal from a third independent copy into the pin's fixed point. `uv.lock`'s `django-strawberry-framework` root entry is the fourth surface and is the more arguable half - reading it means parsing a generated lockfile - so decide it deliberately rather than folding it in. Raised by the spec-012 residual cycle (`docs/builder/bld-012-final.md`), whose own statement of it - that the pairing has no executable pin at all - is the over-statement corrected here; it had no card to route the item to, and this card owns the `0.1.0` quintet with `django_strawberry_framework/__init__.py` and `tests/base/test_init.py` already among its predicted files.
...
- Two further deliberate no-ops, recorded so a sweep does not "fix" them. `docs/README.md`'s counted-body section was examined for a clause about body-reading project middleware and ruled no-change: the decision does not require it, `views.py::_run_after_csrf_check` already carries it in bold for a code reader, and the counted half of that section states its own honest boundary, so the code docstring is the authoritative statement. Separately, the revalidation DRY note prices the WebSocket delegates by `await` count in a way that is true on the natural reading but not literally - each handler body has two `await` expressions and the adapter's `send_json` has two `await`s and two `super()` references.
...
- [x] Spec docs/spec-045-visibility_boundary-0_0_14.md authored with its companion *-terms.csv.
