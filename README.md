...
**Newest shipped (`0.0.14` + the security-hardening program on `main`, cards `046`–`049`).** The `0.0.14` cut is four surfaces; the hardening program landed on `main` after it, and its two **intentional alpha breaking changes** carry migration notes in [`docs/README.md`][readme]:
...
- `0.0.8` — the filtering (`FilterSet` / `RelatedFilter` / `Meta.filterset_class`) and ordering (`OrderSet` / `RelatedOrder` / `Meta.orderset_class`) subsystems, with active-input-only `check_<field>_permission` gates.
