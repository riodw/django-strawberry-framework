...
- TODO anchors. When a spec slice is staged but not yet implemented, drop a `TODO(spec-<doc>.md <slice-id>)` comment at the exact source site the slice will land in, and pair it with `NotImplementedError` if the call path needs to fail loudly. The optimizer rebuild already uses this — `types.py` and `optimizer.py` carry anchors for every `spec-optimizer.md` O-slice. Update or remove the anchor in the same change that ships the slice.
