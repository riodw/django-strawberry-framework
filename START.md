...
- Build in slices. Each slice ships with tests in the same change. Completed design docs should be folded into `docs/FEATURES.md`, `docs/TREE.md`, and `KANBAN.md`, then archived. Future specs should follow the same shape, and don't be afraid to fork a subsystem into its own spec mid-stream when a slice grows past ~one module.
