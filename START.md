...
| **DRY** | [docs/dry/DRY.md][dry] + `worker-0..2.md` | design principles every builder pass reads + consolidation cycle: responsibility families over a file-inventory backstop; mandatory probing matrix; change challenge counts authoritative definitions only (consumers, oracles, projections listed, not counted); owner chosen by responsibility; finding record w/ behavioral proof, structural gate + freshness fingerprints; cycle-owned edits ledgered, attributed by content; rejections carry contract difference + trigger, recorded at the owner; `docs/dry/export_dry_review.py plan|audit|check` = plan + orientation, never findings | `docs/dry/dry-<ver>.md` (plan + responsibility index + owned changes + outcomes = durable record), `dry-file|rule|folder-*.md`, `dry-project.md`, `temp-tests/`, `worker-memory/` |
