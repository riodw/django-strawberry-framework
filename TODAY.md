...
The eventual `1.0.0` shape for products — Relay `DjangoConnectionField`s with `filterset_class` / `orderset_class` / `aggregate_class` / `fields_class` / `search_fields` / `apply_cascade_permissions` — is tracked in `KANBAN.md` under the Layer-3 cards (`DONE-021-0.0.8` filters, `WIP-ALPHA-022-0.0.8` orders, `TODO-BETA-040-0.1.3` aggregates, `TODO-ALPHA-027-0.0.10` permissions, etc.). The list-based schema below is what's there today; the Relay shape grows in as those cards land.
...
- `DjangoConnectionField`
- ordering, aggregates, and fieldsets
- search fields
