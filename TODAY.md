...
- **Session-auth mutations** (shipped `0.0.13`) — the opt-in `django_strawberry_framework.auth` factories (`login_mutation` / `logout_mutation` / `register_mutation` / `current_user`) over `django.contrib.auth`, on the shared `FieldError` envelope with the allow-any default (the documented auth inversion of deny-by-default). Products' catalog surface has no auth fields; the schema-only **`accounts` app owns the live demonstration** — its `UserType` over `auth.User` plus the `login` / `logout` / `register` / `me` fields, exercised by the seeded test users in `examples/fakeshop/test_query/test_auth_api.py` (the full register → login → `me` → logout round trip, the enumeration-guard envelope, hashed-only storage). See [`docs/GLOSSARY.md#auth-mutations`][glossary-auth-mutations].
