...
The eventual `1.0.0` shape for products — Relay `DjangoConnectionField`s with `filterset_class` / `orderset_class` / `aggregate_class` / `fields_class` / `search_fields` / `apply_cascade_permissions` — is tracked in `KANBAN.md` under the Layer-3 cards (`TODO-ALPHA-020` filters, `TODO-ALPHA-021` orders, `TODO-BETA-038` aggregates, `TODO-ALPHA-025` permissions, etc.). The list-based schema below is what's there today; the Relay shape grows in as those cards land.
