...
- ``_connection_type_for(target_type, definition)`` - resolves and caches the connection
...
- ``DjangoConnectionField(target_type, *, resolver=None, ...)`` - the PascalCase
  ...
  ``orderBy:``), and returns ``relay.connection(_connection_type_for(target_type, definition),
...
def _keyset_connection_context(cls: type) -> DeclaredCursorState | None:
    ...
    connection over it KEYSET-MODE - value cursors minted/decoded through the
    canonical codec, offset cursors rejected; the ``False`` sentinel keeps the
    shipped offset behavior. ``_generate_connection_class`` derives the state
    from the field's ONE captured definition and stashes it in the class
    namespace, so the vocabulary a request mints with is the one the class was
    built with; nothing here reads the node type at resolve time. A class not
    generated by this module carries no slot and resolves as offset-mode.
    """
...
def _window_edge_class(cls: type) -> Any:
    ...
    ``Edge`` subclass via the bounded ``unwrap_container_type`` (-
...
def _generate_connection_class(
    ...
    *,
    ...
    ``definition`` is the caller's captured read, never a fresh one: the name
    this class is generated under and the sidecar arguments the field publishes
    beside it come from the same object, so a stateful target cannot answer a
    ...
    def _populate(namespace: dict) -> None:
        ...
        # generated with. ``False`` is the offset-mode sentinel.
        namespace["_dst_keyset_state"] = declared_cursor_state_for_definition(definition) or False
...
def _build_total_count_connection(target_type: type, definition: Any) -> type:
    ...
    generated = _generate_connection_class(target_type, definition, _populate, description=None)
    return generated
...
def _finalize_queryset(
    ...
    *,
    ...
    ``definition`` is the field's captured construction-time read. The model the
    total order is derived over and the ``cursor_field`` that selects it come
    from that one object, so the order this resolve-time tail applies and the
    order the planner planned cannot be answered from two different reads.
    """
...
def _pipeline_sync(
    ...
    *,
    ...
    captured-model default seed). A ``Manager`` is coerced to a ``QuerySet``;
...
def _build_relation_connection_resolver(
    ...
    the fast path). Divergent aliases land one window PER RESPONSE
    ...
    ``definition`` is the TARGET's definition, resolved once by the finalizer
    from the registry and handed in. The connection class the generated field
    publishes, the sidecar arguments this resolver's signature carries, the ones
    its pipeline applies, and the model its visibility seals validate against
    ...
    def _resolve(root: Any, info: Info, **kwargs: Any) -> Any:
        ...
        if (
            ...
            return _WindowedConnectionRows(
                rows=window_rows,
                fallback=_build_windowed_fallback(target_type, source, info, definition),
            )
...
def DjangoConnectionField(  # noqa: N802  # PascalCase for graphene-django parity - consumer usage is `DjangoConnectionField(GenreType)`
    ...
    *,
    ...
    ``relay.connection(_connection_type_for(target_type, definition), resolver=<synthesized>,
    ...
    # fields). Its ``_is_relay_shaped`` check accepts
    ...
    definition = _validate_relay_djangotype_target(
        target_type,
        resolver,
