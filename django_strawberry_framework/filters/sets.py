...
class FilterSet(ClassBasedTypeNameMixin, filterset.BaseFilterSet, metaclass=FilterSetMetaclass):
    ...
    # TODO(docs/row-preserving-predicates-part1-plan.md Slice C.2): add an
    # immutable per-class mapping from FINAL form-filter name to
    # row-preserving-candidate metadata. Read it through
    # ``type(self).__dict__.get(...)`` so a subclass can never inherit its
    # parent's classification. Build/replace it in ``get_filters`` under the
    # exact expansion-cache gate; a name absent from the mapping is ineligible.
    ...
    def get_filters(cls) -> OrderedDict:
        ...
        def _build() -> OrderedDict:
            ...
            # - For every final ``all_filters`` entry, classify its final
            #   ``field_name`` against ``cls._meta.model`` via Slice A. Record a
            #   frozen row containing that path plan, origin (direct generated,
            #   expanded generated, or declared/custom), the pre-framework
            #   distinct provenance stamped by ``filter_for_field``, and an
            #   explicit inner-invocation eligibility bit.
            # - Fail closed. Eligible means: generated by the framework at the
            #   direct or expanded surface; crosses a many-side hop; has no
            #   consumer ``method`` or consumer-origin ``distinct``; and uses an
            #   enumerated audited class (upstream generated filters plus the
            #   package's IntegerInFilter, IntegerRangeFilter, GlobalIDFilter,
            #   GlobalIDMultipleChoiceFilter, ArrayFilter, and ListFilter).
            #   Declared filters, ``Meta.filter_overrides`` products, and unknown
            #   subclasses stay on their original invocation path.
            # - Store the mapping only when ``should_cache_expansion`` succeeds,
            #   beside ``_expanded_filters``/``base_filters``. Use a direct
            #   per-class assignment and ensure registry/input-namespace reset
            #   cannot expose stale metadata before a completed rebuild.
    ...
    def filter_for_field(
        ...
        # TODO(docs/row-preserving-predicates-part1-plan.md Slice C.2): persist
        # ``default.distinct`` on the generated filter BEFORE the framework's
        # to-many stamp below. Use a private provenance attribute whose value is
        # copied by ``_expand_related_filter`` and BaseFilterSet instance
        # deepcopies. The final owning FilterSet's metadata build consumes that
        # captured bit; it must never infer consumer-origin distinctness from the
        # post-stamp ``default.distinct`` value.
    ...
    def filter_queryset(self, queryset: models.QuerySet) -> models.QuerySet:
        ...
        #   original ``filter(inner_root, value)``. Do not reconstruct lookup
