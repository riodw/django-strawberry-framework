...
class FilterSet(ClassBasedTypeNameMixin, filterset.BaseFilterSet, metaclass=FilterSetMetaclass):
    ...
    # TODO(docs/row-preserving-predicates-part1-plan.md Slice C.2): publish
    # filters + row-preserving-candidate metadata as ONE immutable expansion
    # snapshot, its storage slot(s) registered in ``SetLifecycleAttrs`` so
    # ``registry.clear()`` resets both together (a free-floating fourth class
    # attribute would survive the clear that deletes ``_expanded_filters``).
    # Read through ``type(self).__dict__.get(...)`` so a subclass can never
    # inherit its parent's classification. Build inside ``get_filters`` under
    # the exact expansion-cache gate and publish only after a successful
    # build; a name absent from the snapshot's mapping is ineligible.
    ...
    def get_filters(cls) -> OrderedDict:
        ...
        def _build() -> OrderedDict:
            ...
            # TODO(docs/row-preserving-predicates-part1-plan.md Slice C.2):
            # build candidate metadata in THIS expansion pass, while final
            # owning-model paths and expanded-field provenance are both known.
            #
            # Pseudo:
            # - Track every name added by each ``_expand_related_filter`` call;
            #   do not attempt to rediscover expanded origin from a prefixed
            #   string after the loop.
            # - Determine origin BEFORE any strict classification, from the
            #   frozen generation-provenance record stamped on each filter
            #   instance at its construction site (``filter_for_field`` and the
            #   package's GlobalID replacement branches). Declared/custom and
            #   ``Meta.filter_overrides`` leaves are immediately ineligible and
            #   are NEVER fed to the strict classifier: their ``field_name`` may
            #   legitimately be an annotation/method-owned non-model path, and
            #   raising there would break a working declaration. An expanded
            #   leaf inherits the CHILD's origin.
            # - Only proven framework-generated (direct or expanded) leaves get
            #   a strict Slice A classification of the final ``field_name``
            #   against ``cls._meta.model``; a resolution failure THERE is a
            #   framework/configuration defect and raises. Record a frozen row:
            #   path plan, provenance record (origin + framework-added-distinct
            #   bit), and inner-invocation eligibility.
            # - Fail closed via construction provenance, never a class
            #   allowlist (upstream synthesizes dynamic ConcreteInFilter /
            #   ConcreteRangeFilter subclasses, so exact-class lists drift).
            #   Eligible means: framework-generated per the provenance record;
            #   crosses a many-side hop; no consumer ``method``; no
            #   consumer-origin ``distinct``.
            # - Publish filters + metadata as ONE immutable expansion snapshot
            #   only when ``should_cache_expansion`` succeeds and the build
            #   completes; the snapshot slot(s) are registered in
            #   ``SetLifecycleAttrs`` so ``registry.clear()`` resets both
            #   together and stale metadata is unobservable by construction.
    ...
    def filter_for_field(
        ...
        # TODO(docs/row-preserving-predicates-part1-plan.md Slice C.2): stamp
        # ONE frozen generation-provenance record on the ACTUAL returned filter
        # instance -- not a bare pre-stamp ``distinct`` boolean on ``default``.
        # ``default`` arrives pre-merged with ``Meta.filter_overrides`` (a True
        # cannot distinguish upstream default from consumer override), and the
        # GlobalID replacement branches below return NEW instances that would
        # never see an attribute left on ``default``. The record distinguishes
        # framework-default generation, package-owned replacement, declared,
        # and ``filter_overrides`` origin; carries whether the framework added
        # ``distinct``; and must be stamped again on every replacement instance
        # this method constructs. ``_expand_related_filter`` deepcopies carry
        # it and APPEND "expanded from <name>" without overwriting origin. The
        # metadata build must never infer consumer-origin distinctness from
    ...
    def filter_queryset(self, queryset: models.QuerySet) -> models.QuerySet:
        ...
        """
        # TODO(docs/row-preserving-predicates-part1-plan.md Slice C.3): replace
        # wholesale upstream delegation for FLAT leaves with one
        # framework-owned applicator that mirrors BaseFilterSet exactly while
        # routing eligible to-many leaves through Slice B.
        #
        # Pseudo:
        # - Initialize ``qs = queryset`` and iterate
        #   ``self.form.cleaned_data.items()`` in insertion order.
        # - Look up the final field name in
        #   ``type(self).__dict__.get(candidate_mapping_name, {})``. Missing or
        #   ineligible metadata calls ``self.filters[name].filter(qs, value)``
        #   unchanged, preserving custom methods/distinct/classes byte-for-byte.
        # - For an eligible leaf, build ``inner_root =
        #   correlated_inner_root(qs)`` and call THAT SAME filter object's
        #   original ``filter(inner_root, value)`` through a named invocation
        #   helper that suppresses the FRAMEWORK-ADDED ``distinct`` flag for
        #   the duration (``try/finally`` on the per-instance deepcopy; the
        #   flag must be restored on success AND on exception, and no
        #   class-level/base filter is ever mutated). ``Query.exists()`` never
        #   clears ``distinct``, so an unsuppressed invocation compiles
        #   ``EXISTS(SELECT DISTINCT 1 ...)`` -- redundant planning inside
        #   every correlated branch. Consumer-origin ``distinct`` filters are
        #   ineligible and never reach the helper. Do not reconstruct lookup
        #   kwargs: filter/exclude selection, range decomposition, GlobalID
        #   decoding, integer coercion, and Django ``split_exclude`` semantics
        #   must remain owned by the original invocation.
        # - If ``inner_result is inner_root``, treat it as upstream's no-op and
        #   attach nothing. This covers inactive form fields, upstream
        #   MultipleChoiceFilter no-ops, and the package's exclude-empty
        #   match-all branch. A restrictive empty producing ``inner_root.none()``
        #   is NOT identity and must become ``Exists(none) == False``.
        # - Otherwise call ``attach_exists(qs, inner_result)`` and apply the
        #   returned POSITIVE branch with ``qs = qs.filter(branch)``. Never move
        #   negation outside the original filter invocation and never combine
        #   two active leaves into one inner body: separate leaves intentionally
        #   preserve cross-row AND semantics.
        # - After every invocation, retain upstream's ``isinstance(qs,
        #   models.QuerySet)`` assertion and exact diagnostic. Once the flat loop
