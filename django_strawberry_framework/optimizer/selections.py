...
# inliner, but keep the default path byte-for-byte unconditional. Design
# contract: spec-035 Decision 6 (the tri-state classifier and its accept set)
# and Decision 7 (narrow, do not multi-plan). Reachability: an abstract
# (interface/union) root field never reaches the walker today -
...
# contract (spec-035 Decision 6 R1). Pseudocode: no classifier means INLINE-all
# for extension cache-key and connection extraction callers; a walker-supplied
