...
- A frozen object - ``MappingProxyType``, frozen dataclass, ``pydantic`` model
  with ``frozen=True``.

Two subsystems hand off through the request context and therefore share these
helpers: the optimizer's plan / elision / strictness stashes
(``optimizer/_context.py``) and the request resource policy
(``resource_policy.py``). Centralizing the dispatch means a future broadening
(a new context shape, a new exception class to swallow on write) lands in one
place.

Defensive-coerce stance (package-wide). Reflective shape reads off Strawberry /
graphql-core / Django descriptors use the pattern ``getattr(obj, name, None) or
{}``. That is *correct* for upstream descriptors whose attribute may legitimately
be absent or ``None``, and is the opposite of the posture taken for
consumer-supplied input (see ``conf.py``'s module docstring). The two should not
...
def stash_on_context(context: Any, key: str, value: Any) -> None:
    ...
    skipped - a read-only context must not abort the resolver chain. The
    two frozen-shape error modes the dict path must absorb are
    ``TypeError`` (``MappingProxyType``, frozen dataclasses, ``pydantic``
    models with ``frozen=True``) and ``AttributeError`` (Django's
    ``QueryDict`` when locked raises
    ...
    A stash that cannot land is not a silent loss of a bound: every
    consumer of a stashed value falls back to a *bounded* default when the
    read misses (``resource_policy.py::policy_from_info`` returns the
    package default policy, not "unbounded"), so a frozen context degrades
    to the package default rather than to no policy at all.
    ...
    try:
        ...
        # caller. Other exception classes (``KeyError`` from a guarded
