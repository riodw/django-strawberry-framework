...
The iterative, cycle-safe pass inside [`DjangoResourcePolicyExtension`](#djangoresourcepolicyextension) that charges a request's ARGUMENT VALUES, which document limits cannot see: a tiny document can carry an enormous variable payload. It charges total input nodes, container width, membership (`in`) items, Relay node-refetch ids, relation ids per mutation field and in aggregate across the request, nested input-object rows, [value nesting depth](#max_value_depth), upload count / per-file bytes / aggregate bytes, and scalar UTF-8 byte size - and it runs entirely on coerced-shape input, so it rejects BEFORE any GlobalID is decoded and before any queryset is built. Every amount it charges is measured on a representation the package owns: a container's width is the number of members the walk queues rather than the number the value reports, text is sized with the built-in encoding, and a buffer through the C buffer protocol - a custom scalar's `parse_value` can return a `list`, `Mapping`, `str` or `bytes` subclass, and every one of those measurements would otherwise be the value's own code answering for how much work it is asking for.
