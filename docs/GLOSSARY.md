...
The write pipeline is **hardened** (2026-07-15): `get_serializer_kwargs` is constructor-only — `data`, `instance`, `partial`, `context["request"]`, and `context["write_alias"]` are framework-owned, and a conflicting reserved return is a [`ConfigurationError`](#configurationerror) (the reserved checks use an omission sentinel plus object identity, never deep equality - the returned `data` must be omitted or the exact clone the hook received, the returned `instance` omitted or the located row itself; explicit `None` and rebuilt-equal copies are rejected); all three hooks (constructor, injection, and save-kwargs) receive a recursive, iteratively-built plain-container clone of the data, so even nested mutation attempts never reach the authoritative data or `validated_data`, and client-controlled JSON nesting depth cannot crash the pipeline. A writable serializer `source` must be unique across the whole write surface (input fields and `Meta.injected_fields`) — two differently-named fields feeding one model attribute are rejected at class creation. Server-supplied fields pair `Meta.injected_fields` with `get_serializer_injected_data(info, *, data, instance=None)`, whose returned keys must exactly match the declaration; only writable fields may be injected, so `read_only` fields and `HiddenField` instances are rejected at class creation, and injected fields are narrowed out of the GraphQL input there. Every relation queryset DRF re-validates against — input **and** injected, at every nesting depth — is composed as author queryset AND target visibility, pinned to the operation's write alias, and locked when `Meta.select_for_update` locks; a cross-alias author queryset fails closed. Querysets owned by DRF's field and serializer validators — including nested serializer validators and uniqueness/date validators — are recursively pinned to that same alias without changing any other validator semantics, and an explicitly cross-routed validator queryset also fails closed. `get_serializer_save_kwargs` collisions are checked against the actual `serializer.validated_data` keys, and every consumer-reachable pipeline phase (permission, decode, validation, write, re-fetch — shared by all three write flavors) runs under a pipeline-wide alias guard that rejects EVERY SQL statement on a non-pinned connection before it executes, with no read/write classification (lexical keyword tests are bypassable via SQL comments, `EXPLAIN ANALYZE`, and write-capable functions), covering the signal-less `QuerySet.update()`/`bulk_create`/raw-SQL paths by construction (the request's auth state is warmed before the guard, since the auth models live on their own router alias); the serializer write phase additionally runs under a thread-scoped write witness recording the backing model's actual writes with a pk snapshot taken at the signal, the `get_serializer_save_kwargs` hook runs inside the same error-mapped closure as `save()`, and the `serializer.save()` result is validated (correct model, identity with `serializer.instance`, persisted — non-null pk and not `_state.adding` — exact write alias, a pk-snapshot-matched witnessed insert on create so an existing row cannot be laundered as “created” even by re-pointing a mutable pk, and a matching same-alias witnessed update of the immutable authorized-pk snapshot on update, so a custom no-op update returning the existing instance is rejected; the pipeline captures that snapshot immediately after the locate, before the permission hook can touch the mutable instance, and a flavor-independent backstop enforces the same snapshot) before the re-fetch.
