...
`TestClient` / `AsyncTestClient` and the typed `Response` - thin wrappers over Django's `django.test.Client` / `django.test.AsyncClient` that post a GraphQL operation with the right content type, decode it, and return a typed `Response(errors, data, extensions, response)` carrying the raw `HttpResponse` beside the decoded triple. Imported from `django_strawberry_framework.testing` (never the package root). Across the client family, endpoint precedence is per-call `query(..., url=...)` > constructor `path=` (the standalone clients) > `GraphQLTestMixin.GRAPHQL_URL` (the unittest family) > `DJANGO_STRAWBERRY_FRAMEWORK["TESTING_ENDPOINT"]` > `"/graphql/"`; the flavor-specific constructor or class-attribute choice resolves before the request, while a per-call override routes one request without mutating stored state. `query()` gains `operation_name=` and returns the typed `Response`; its default `assert_no_errors=True` raises `AssertionError` on a GraphQL-errors response (an explicit raise, so it holds under `python -O`), while the unittest mixin deliberately defaults to `False`. Multipart uploads ride the same call: `query(..., variables={"data": {"image": None}}, files={"data.image": f})` binds each path-keyed file part to its `None` placeholder (nested input objects and list indexes included, validated at the source). `login(user)` brackets a block with `force_login` / `logout` (the async twin wraps both in `sync_to_async`). An endpoint typo is not wrapped as a framework error: Django's HTML 404 reaches `response.json()`, which raises `ValueError` naming the non-JSON `Content-Type`; a response that declares JSON but contains malformed JSON instead surfaces `json.JSONDecodeError`. Subclasses Strawberry's `strawberry.test.BaseGraphQLTestClient` over the package's existing hard `strawberry-graphql` dependency, so the family adds **no** new dependency, guard, or install hint. Companion: [`GraphQLTestCase`](#graphqltestcase).
