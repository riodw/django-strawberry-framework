...
- [`DEFAULT_ERROR_POLICY`](#errorpolicy) — the all-defaults `ErrorPolicy` instance, exported as an inspectable value template; a deployment that configures nothing is masked by a private copy of its values, never by this object.
- [`DEFAULT_RESOURCE_POLICY`](#resourcepolicy) — the all-defaults `ResourcePolicy` instance, exported as an inspectable value template; a deployment that configures nothing is bounded by a private copy of its values, never by this object.
...
- [`DjangoErrorPolicyExtension`](#djangoerrorpolicyextension) — response-side enforcement of the [production error policy](#production-error-policy); built by `DjangoSchema` first in every operation's chain, never an entry in `extensions=`, so it masks after every other teardown.
...
The field reads the target's definition EXACTLY ONCE, at the line that built it, and that object must be the exact definition the type registry holds for the target - the same canonical-source rule [`DjangoListField`](#djangolistfield) and the Relay node fields apply. The generated `<TypeName>Connection` is cached on target identity together with the definition that generated it, so a second field arriving with different metadata for one target is rejected rather than silently served the first field's shape. A synthesized relation connection takes its definition from the registry at finalization, and the optimizer plans that same field from the child definition the relation was resolved through, so the shape, the published arguments, the cursor vocabulary and the planned window all come from one answer.
...
The `SchemaExtension` that ENFORCES the [production error policy](#production-error-policy), root-exported for a consumer assembling a plain `strawberry.Schema`. `DjangoSchema` builds one per operation, FIRST in the chain, from the policy it was constructed with; the extension is not an entry in `extensions=` and no entry a resolver can reach decides whether a response is masked.
...
Non-Relay `list[T]` **root Query field**. The smallest entry point for migrants coming from `graphene-django`'s `DjangoListField` and for use cases that do not need Relay cursor pagination, edges, or page-info. Implemented as a **factory function** (not a class): consumer usage is `all_branches: list[BranchType] = DjangoListField(BranchType)`, and Strawberry's `@strawberry.type` class-body walk picks up the factory's return value the same way it picks up `strawberry.field(...)`. Outer-list nullability is driven by the consumer's class-attribute annotation — `list[T]` renders as `[T!]!` and `list[T] | None` renders as `[T!]`. The default resolver pulls `_default_manager.all()` for the model it captured from the target's definition at construction -- the field reads that definition EXACTLY ONCE, at the line that built it, and the seed, both visibility seals, the published `orderBy` argument and every resolver dispatch all run on that one object, so a stateful target cannot answer a later read with a different model or sidecar; the object it accepts must also BE the exact definition the type registry holds for that target, compared by identity, so a fabricated or copied same-origin lookalike is rejected with [`ConfigurationError`](#configurationerror) at the constructing line rather than surfacing later as an engine type error over an unrelated model -- and applies the type-level [`get_queryset`](#get_queryset-visibility-hook) in both sync and async contexts (the sync path rejects an async `get_queryset` with [`SyncMisuseError`](#syncmisuseerror), mirroring the Relay defaults). A consumer-supplied `resolver=` overrides the default body; when its return value is a Django `Manager` or `QuerySet`, the wrapper coerces the `Manager` to a `QuerySet` and applies `target_type.get_queryset(qs, info)` (graphene-django parity), so a custom resolver still honors the visibility hook. Async consumer resolvers are detected at construction time via the partial-aware `is_async_callable` predicate (checked on the resolver, on its `__call__` so callable-instance resolvers with `async def __call__` are covered, and through a one-hop `functools.partial`) and routed through an `async def` wrapper that awaits the coroutine before applying the isinstance check. Python `list` returns from sync or async resolvers pass through unchanged. Optimizer cooperation rides the existing root-gated [`DjangoOptimizerExtension`](#djangooptimizerextension) hook (`info.path.prev is None`), so root-position `DjangoListField` selections receive `select_related` / `prefetch_related` / `only` planning automatically; nested non-root usage is functional but not root-optimized in `0.0.7`. Standard field-level metadata pass-through (`description`, `deprecation_reason`, `directives`) is forwarded into the inner `strawberry.field(...)` call.

**Argument surface and ordered offset (`0.0.15`, spec-050).** Every `DjangoListField` publishes nullable optional `offset` and `limit` arguments. When the target type declares [`Meta.orderset_class`](#metaorderset_class), a typed `orderBy` argument is conditionally published under the active schema naming converter. The field provides strictly **ordered offset** paging, NOT stable or repeatable pagination: an active order fixes the sort expression, not which of two tied rows falls on either side of a page boundary. Unlike Relay connections, flat lists inject no primary-key tiebreaker and no `DISTINCT`; consumers wanting deterministic pagination across pages with duplicate values must append a unique final term to the ordering themselves. A published `offset` is a runtime precondition rather than a per-field capability claim: `offset > 0` requires a materially active order on the post-visibility queryset (via `orderBy` or a still-effective model `Meta.ordering`), and permanently fails with [`ListArgumentError`](#listargumenterror) (`order_required`) when neither order source is available.

**Async-iterable resolvers and queryset completion.** A consumer `resolver=` may also be an **async generator function**, or a plain sync callable that returns an **async-only iterable**; both arms are supported and both are row-bounded exactly like the sync and coroutine arms. An async-only iterable is meaningful only under asynchronous execution: met from synchronous GraphQL execution it raises [`SyncMisuseError`](#syncmisuseerror) rather than silently yielding nothing, so use `await schema.execute(...)` for such a resolver. In async GraphQL execution, Django `QuerySet` results are completed safely through the [async queryset completion adapter](#async-queryset-completion-adapter).
...
The `SchemaExtension` that SPENDS the [execution resource policy](#execution-resource-policy). `DjangoSchema` builds one per operation, LAST in the chain before the admission guard, so charge counters are never shared across concurrent requests; it is root-exported for a consumer assembling a plain `strawberry.Schema`. It is not an entry in `extensions=`: a consumer entry of either enforcement kind is a configuration declaration folded into the schema's own record, a subclass of either is refused at construction, and a factory resolving to either refuses the operation. There is no configuration under which it is installed and enforces nothing.
...
The one immutable budget a request is allowed to spend, introduced in `0.0.14` (spec-047). `DjangoSchema` resolves it ONCE at construction from `DjangoSchema(resource_policy=...)`, then `DJANGO_STRAWBERRY_FRAMEWORK["RESOURCE_POLICY"]`, then the package defaults, stores a private duplicate whose values are validated again rather than the instance the caller passed, and publishes the resolved [`ResourcePolicy`](#resourcepolicy) on the request context under `dst_resource_policy` - the same seam shape the optimizer's `DST_OPTIMIZER_*` keys use. It owns three families of bound: the DOCUMENT bounds (tokens and structural depth, charged before the parse; expanded selections, aliases and multiplicative collection cost, charged after the parse and before validation), the VALUE bounds the [value-budget walker](#value-budget-walker) charges, and the COLLECTION bounds the fields enforce (`max_page_size` as a ceiling over `relay_max_results`, `max_list_rows` as the returned-row ceiling and accepted-skip ceiling for every raw list). Coordinate ceilings represent accepted boundaries rather than physical database rows scanned.

**Held where a resolver cannot reach it.** The resolved policy is stored in a package-owned registry keyed by schema identity rather than in an attribute of the schema, because `info.schema` is handed to every resolver in every operation and the schema outlives the request - an ordinary attribute would be a process-lived widening primitive, reachable by assignment and through `schema.__dict__` past a property that has no setter. The schema itself holds the record, which is the only home whose lifetime is right - an extension instance reaches its schema through the execution context, and a subclass may configure `extensions=[self.make_extension]`, so a registry owning that configuration strongly would keep every such schema alive for the life of the process - and the registry holds only a weak reference to the record that was accepted, keyed by the schema's IDENTITY rather than by hash and equality, which are methods a schema subclass may define and which would let one schema's record answer for another that merely compares equal to it. A record written behind the attribute holding it is not the accepted one, so what answers is the fail-closed default, and the entries that enforce it are put back rather than taken from a configuration that can no longer be read. A directly supplied exact extension INSTANCE is read once, where it is accepted, and the bound it declared is folded into this record; the entry itself never reaches an operation's chain, so no resolver finds one in `info.schema.extensions` to widen a later request through. The policy such an extension holds is kept on the same private terms regardless, because the standalone use the extension also supports - a plain `strawberry.Schema` - accepts an instance entry as itself, and there what it holds is what the next operation arms. Enforcement does not depend on the schema's extension list at all: the extension that spends the policy is built per operation from this record, and no entry - accepted, replaced, or returned by a factory on the operation itself - can be an enforcement authority. Accepting an entry proves which object the schema was configured with, and a factory is consumer code whose next answer is decided after that acceptance. One operation is bounded by ONE snapshot, taken when the budget is armed, so the pre-parse scan, the document walk and the per-field seams cannot enforce different budgets within one request.
...
**See also:** [`ResourcePolicy`](#resourcepolicy) · [`DjangoListField`](#djangolistfield) · [`DjangoResourcePolicyExtension`](#djangoresourcepolicyextension) · [Value-budget walker](#value-budget-walker) · [Request-body cap](#request-body-cap).
...
**See also:** [`OrderSet`](#orderset) · [`DjangoListField`](#djangolistfield) · [`ListArgumentError`](#listargumenterror) · [`DjangoConnectionField`](#djangoconnectionfield).
...
The package's own masker satisfies the rule from the other side: [`DjangoErrorPolicyExtension`](#djangoerrorpolicyextension) is built by `DjangoSchema` FIRST in every operation's chain rather than as an entry in `extensions=`, so it tears down last and the debug extension — always after it, wherever the consumer puts it — has already read the originals. A consumer who assembles the extension list by hand keeps the same rule: the masker first, the debug class after it.
...
When `total_count` is true, [`DjangoConnectionField`](#djangoconnectionfield) resolves the type through a concrete per-target connection class exposing `totalCount`; when it is false the generated class is the same concrete `<TypeName>Connection` minus those members, never the generic [`DjangoConnection`](#djangoconnection)`[T]` base itself. The option is type-level, not per field, so a node type has one stable connection shape.
...
The ordering subsystem, introduced in `0.0.9` (spec-028) as the order counterpart to [`FilterSet`](#filterset). Declared on a [`DjangoType`](#djangotype)'s `Meta.orderset_class`, an `OrderSet` generates a typed Strawberry GraphQL input type (`orderBy: [ItemOrderInputType!]`) through the [`order_input_type`](#order_input_type) factory, exposes fields as [`Ordering`](#ordering) enums (`ASC` / `DESC`), and applies ordering terms to a Django `QuerySet` via `queryset.order_by(...)`.

To-many paths are row-preserving: ascending terms order each parent by `Min(path)` and descending terms by `Max(path)`, so reverse-FK / M2M joins do not duplicate nodes or inflate `totalCount`. A root `DjangoConnectionField` cursor-slices that grouped queryset and appends its deterministic primary-key tiebreaker. In addition to connections, `OrderSet` is reused by [`DjangoListField`](#djangolistfield) (`0.0.15`, spec-050) when the target type declares `Meta.orderset_class`, applying user ordering before row bounding without injecting a pk tiebreaker or DISTINCT. Nested relation connections carrying `orderBy:` deliberately bypass window/lateral planning and use the per-parent pipeline, so a to-many aggregate is never stacked below the optimizer's row-number window. This contract runs unchanged on SQLite and PostgreSQL.
...
**See also:** [`Meta.orderset_class`](#metaorderset_class) · [`DjangoListField`](#djangolistfield) · [`RelatedOrder`](#relatedorder) · [`Ordering`](#ordering) · [`order_input_type`](#order_input_type) · [`FilterSet`](#filterset).
...
The guarantee that every GraphQL operation reads and writes its OWN extension state. Strawberry resolves a class entry into a fresh instance per operation, but passes an INSTANCE entry through unchanged and calls a factory exactly as written - so the documented singleton-in-a-factory shape returns one object to every operation. The isolation is therefore per-operation STATE rather than a per-operation object: `DjangoSchema` overrides `create_extensions_runner` and binds one state object per resolved framework extension around the operation, result collection, every streaming frame and every resumption of a streamed result (`django_strawberry_framework/extensions/operation_state.py::DjangoExtensionsRunner`). The engine's own `execution_context` assignment is deliberately not an authority: the runner is handed that same context directly and upstream runs no hook in between, so nothing is recorded in front of the runner for a refusing assignment loop to strand or a resolver to forge, and an extension the runner has managed ignores every later assignment of every shape. What a binding holds is a lease on a weak reference to state the runner owns (`django_strawberry_framework/utils/operation_lease.py::OperationLease`), and the scope closes it on the way out: a task that copied the binding - a background job started from a resolver - reads nothing rather than the completed request, at the instant the scope ends rather than whenever the owner is collected. Closure is what invalidates a copied context; resetting a token only restores the enclosing operation in the context that made it, which is why the armed resource budget and the optimizer's execution frame are leases too. Lifetime and binding are separate: an operation arms its budget for as long as it runs, and the resume driving a streamed frame takes that binding back in its own task when the frame is handed over - so between two frames the driving task answers from its enclosing operation, or from nothing, rather than from a request it is only waiting on. Each framework extension reads its engine context, its capture payload and its optimizer stashes from that state, so a nested `info.schema.execute_sync(...)` and two overlapping requests cannot answer for one another - the charge landing on the wrong document, the mask landing on the wrong result, and a parse failure republishing the previous request's debug payload are all one defect. The runner also settles whether its operation is running inside another one, from the live scope bound when it opens its first binding, which is what keeps an inner operation - from a resolver, from a consumer extension's teardown, from result collection - out of the outer request's published optimizer keys while leaving a job that merely inherited an ended request's context top-level. The chain those extensions come from is admitted as one transaction: the accepted entries are resolved once, every resolved member has to be a `SchemaExtension` instance, and no member may claim an enforcement role - the resource-policy and error-policy extensions are built per operation from the schema's own record, so a raising factory, a factory returning something else, and a factory returning either enforcement extension all fail closed on the wire with the stable `SCHEMA_CONFIGURATION_UNAVAILABLE` code instead of reaching the engine. That refusal is published before the parse stage and replaces both the document and the operation selector with the package's own - one anonymous operation of a type the transport allows, parsed once at import - so a malformed document and an `operationName` no document can carry are refused on the same terms as a valid request, on the synchronous, awaited and streamed paths alike, and the chain keeps the package's resource extension, whose pre-parse token and depth scan still bounds what a refused schema is sent. **Accepted entry identity is not enforcement authority.** Authenticating an entry answers which object a schema was configured with, which is the whole question for an ordinary extension and none of it for an enforcement one: the object is consumer code, `info.schema.extensions` hands it to every resolver, and a stateful factory, a closure over a mutable cell, a singleton selector and a subclass overriding the load-bearing hook all keep their identity while changing what they do. Resource and error configuration is therefore canonical schema state, declared through `DjangoSchema(resource_policy=..., error_policy=...)` and settled at construction, while every Strawberry entry spelling stays supported for the extensions that are not authorities. A plain `strawberry.Schema` never calls that runner factory and is not promised what it cannot be given: there the supported entries are the class and a factory that builds a fresh extension per call, which are operation-local because the instance is. The optimizer's singleton-in-a-factory stays supported under `DjangoSchema` and keeps its intentional cross-request [Plan cache](#plan-cache); what is per operation is its state, not its cache.

**See also:** [`DjangoDebugExtension`](#djangodebugextension) · [`DjangoErrorPolicyExtension`](#djangoerrorpolicyextension) · [`DjangoOptimizerExtension`](#djangooptimizerextension) · [`DjangoResourcePolicyExtension`](#djangoresourcepolicyextension) · [Hard dependency](#hard-dependency).
...
The rule that seals a `Prefetch` child queryset onto the same database alias as its parent so one GraphQL resolution never spans two connections. Every `Prefetch` — including the `queryset=None` case — is rebuilt as an EXACT `django.db.models.Prefetch` (only the exact-`str` / `None` path state is copied) so a consumer `Prefetch` subclass cannot survive with a `get_current_querysets` override that substitutes an unsealed child at fetch time; non-`Prefetch` lookups must be exactly `str`. Each inner queryset is recursively [sealed](#sealed-execution-queryset) with the outer effective alias threaded in under `require_shared_alias`: a child explicitly routed off a DIFFERENT alias fails closed, and when the parent is UNROUTED (effective alias `None`) an explicitly routed child also fails closed while an unrouted child inherits the parent alias. The child seal admits a sliced prefetch queryset (`reject_sliced=False` in `_PREFETCH_CHILD_POLICY`) because a consumer-supplied `Prefetch` is the consumer's own Django call and Django's rules govern it: the slice survives with `to_attr` or on a to-one relation and raises Django's own error otherwise. The optimizer's generated child takes the same policy only on the nested-connection path, whose planner classifies a sliced child and degrades; a plain list relation's generated child is sealed under `_LIST_RELATION_CHILD_POLICY`, which differs on `reject_sliced` alone and refuses the slice with the typed `sliced` defect. Implemented by [`django_strawberry_framework/utils/querysets.py::_rebuilt_prefetch_or_defect`][querysets] and [`django_strawberry_framework/utils/querysets.py::_sealed_prefetch_related_lookups`][querysets].
...
The frozen dataclass in `django_strawberry_framework/resource_policy.py` carrying every bound of the [execution resource policy](#execution-resource-policy), root-exported alongside `DEFAULT_RESOURCE_POLICY`. Its `__post_init__` validates every field, so an invalid bound raises [`ConfigurationError`](#configurationerror) at schema construction rather than on the first request that reads it - and `bool` is rejected explicitly, since `isinstance(True, int)` would otherwise turn a typo into a silent bound of `1`. A bound is also bounded ABOVE, at `MAX_RESOURCE_BOUND` (the signed 64-bit maximum): a value past it cannot be bound as a `LIMIT` by a database adapter and cannot be rendered into the rejection it drives, so it is a configuration with no working rejection rather than a looser one.
...
- Also raised by `utils/execution_mode.py::async_execution` when a field is asked which executor is driving it and the answer is neither branch: the synchronous GraphQL executor holds the operation while an event loop is running in the thread, which is what `schema.execute_sync(...)` called from inside an awaited operation produces. The message names both recourses - `await schema.execute(...)`, or the synchronous call in a worker thread - and it is raised before any coroutine or async-only adapter is built, because that value is exactly what the synchronous executor would cancel unawaited.
...
**The kill switch.** Every applier is gated by `APPLY_UPSTREAM_PATCHES`, default on, read once per `apply()` at app load. Two shapes are accepted: a bool, which turns all four off (`DJANGO_STRAWBERRY_FRAMEWORK = {"APPLY_UPSTREAM_PATCHES": False}`), or a mapping keyed by dependency name (`django`, `strawberry`, `cross_web`, `graphql_core`), so `{"django": False}` drops only the test-only teardown patch while the production request hardening stays installed. Names left out of the mapping default to `True`. Any other shape raises `ConfigurationError` at the first gate rather than silently inverting the consumer's intent — an unknown name, a non-bool mapping value, or a `"false"` string that bool coercion would read as *enable* — and the whole mapping is validated on every read regardless of which applier reads it first. `strawberry` and `cross_web` jointly own the malformed-body hardening on Strawberry's own sync view, so disabling one of that pair leaves it half-done. The `graphql_core` gate owns only executor completion, so either workaround can be disabled or retired without changing the other.
...
The iterative, cycle-safe pass inside [`DjangoResourcePolicyExtension`](#djangoresourcepolicyextension) that charges a request's ARGUMENT VALUES, which document limits cannot see: a tiny document can carry an enormous variable payload. It charges total input nodes, container width, membership (`in`) items, Relay node-refetch ids, relation ids per mutation field and in aggregate across the request, nested input-object rows, [value nesting depth](#max_value_depth), upload count / per-file bytes / aggregate bytes, and scalar UTF-8 byte size - and it runs entirely on the RAW argument values the request carried - the decoded variables map and literal arguments - so it rejects BEFORE any GlobalID is decoded, before a custom scalar's `parse_value` converts anything, and before any queryset is built. It runs at the parsing stage, before ANY validation, for exactly that reason: validating a document parses every literal argument and every variable-definition default through the scalar it is typed as, so a walk that ran after validation would already have handed a consumer's parser the argument the request was about to be refused for. That includes validation an installed extension runs itself - Strawberry's `ValidationCache` does - which is why the stage is not a member of the validation chain at all: every extension's parsing hook finishes before any validation hook begins, and a refused operation carries no validation rules for anything to run. Its rejection is published as the operation's pre-execution error rather than raised, which is what every transport renders into an error entry and what makes graphql-core's own validation stand down; because that field is one any validation hook may assign over, the verdict itself is recorded where only the package writes, restated by an entry appended behind every consumer extension, and read again at the hook execution begins from. What it bounds is therefore the shape the client sent; what a resolver builds out of it is bounded at the collection seams instead. Every amount it charges is measured on a representation the package owns: a container's width is the number of members the walk queues rather than the number the value reports, text is sized with the built-in encoding, and a buffer through the C buffer protocol - an in-process caller's `variable_values` can carry a `list`, `Mapping`, `str` or `bytes` subclass, and every one of those measurements would otherwise be the value's own code answering for how much work it is asking for. Measuring stays inside the budget as well: a container that is not an exact built-in is read one member past the width bound and no further, is never asked how many members it has, is counted by the walk's own counter so the whole declared bound domain works, and is refused rather than raised through when it cannot be read at all.
