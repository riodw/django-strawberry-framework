...
**Row-preserving to-many predicates (Part 1).** A framework-generated flat filter leaf whose ORM `field_name` crosses a reverse-FK or many-to-many hop compiles as a correlated `EXISTS` subquery rather than a membership `JOIN` paired with an outer `SELECT DISTINCT`. This is the row-preserving multiset-selection contract: filtering a to-many relation preserves the outer queryset's natural row multiplicity and ordering and no longer imposes an accidental global deduplication that would collapse legitimately-distinct rows or corrupt connection `totalCount`. Routing is fail-closed and per-request over a FINITE, enumerated integrity boundary: only a leaf proven package-generated -- an unbroken generation-capability token minted on an owner that overrode none of the generation seams (`filter_for_field`, `filter_for_lookup`, `FILTER_DEFAULTS`, `__init__`) -- whose live semantic signature still matches its frozen build-time signature (the effective `.filter` call graph -- `.filter`, `get_method`, `get_filter_predicate`, `is_noop` -- plus the non-pk-`to_field` pk-qualification marker, the effective `to_field_name` the model-choice predicate reads off the live built form field at execution -- not the constructor copy -- and the execution knobs) is invoked through the correlated adapter. A consumer-declared filter, a consumer relation override (`Meta.filter_overrides` or a shadowed class-level `FILTER_DEFAULTS`, which is consumer-owned by whole-entry object identity -- so an `extra`-only override that keeps the standard filter class but supplies its own `extra` provider is honored too -- keeps its own wire shape and is never converted to a package GlobalID primitive), or a filter mutated through any covered seam runs the original outer invocation unchanged (a missed optimization, never a changed result set). Negation remains an outer `pk__in` complement and generated candidates never carry `exclude=True`.
