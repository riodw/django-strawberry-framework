...
The guarantee that every GraphQL operation reads and writes its OWN extension state. Strawberry resolves a class entry into a fresh instance per operation, but passes an INSTANCE entry through unchanged and calls a factory exactly as written - so the documented singleton-in-a-factory shape returns one object to every operation - and it assigns `execution_context` on every resolved member before building the runner, without restoring it afterwards. The isolation is therefore per-operation STATE rather than a per-operation object: `DjangoSchema` overrides `create_extensions_runner`, claims the engine's assignment as a handoff while the runner is built, and binds one state object per resolved framework extension around the operation, result collection and every streaming frame (`django_strawberry_framework/extensions/operation_state.py::DjangoExtensionsRunner`). Each framework extension reads its engine context, its capture payload and its optimizer stashes from that state, so a nested `info.schema.execute_sync(...)` and two overlapping requests cannot answer for one another - the charge landing on the wrong document, the mask landing on the wrong result, and a parse failure republishing the previous request's debug payload are all one defect. A plain `strawberry.Schema` never calls that factory and is not promised what it cannot be given: there the supported entries are the class and a factory that builds a fresh extension per call, which are operation-local because the instance is. The optimizer's singleton-in-a-factory stays supported under `DjangoSchema` and keeps its intentional cross-request [Plan cache](#plan-cache); what is per operation is its state, not its cache.
