...
- [`spec-optimizer_beyond.md`](spec-optimizer_beyond.md) — eight optimizer improvements beyond what strawberry-graphql-django ships: AST-cached plans (B1, shipped), forward-FK-id elision (B2, shipped), N+1 detection in dev mode (B3, shipped), `Meta.optimizer_hints` (B4, shipped), plan introspection via context (B5, shipped), schema-build-time optimization audit (B6, shipped), precomputed optimizer field metadata (B7, shipped), and queryset optimization diffing (B8, shipped).
