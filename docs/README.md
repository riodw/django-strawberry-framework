...
- serializer-based mutations (new in `0.0.13`) — the DRF write flavor on the same `class Meta` surface: `SerializerMutation` declared through `Meta.serializer_class` (a `rest_framework.serializers.ModelSerializer`) + an explicit `Meta.operation` ∈ `{"create", "update"}`, subclassing `DjangoMutation` via the `_resolve_model` seam so the `DjangoModelPermission` write-auth default, the visibility-scoped `update` locate, and the optimizer post-write re-fetch all carry over. The input shape is derived from the serializer's declared fields (`serializer_converter.py`, reusing the read-side converters), and `serializer.errors` maps onto the same frozen `FieldError` envelope (`validate_<field>` keyed to its field; object-level `validate()` and `UniqueTogetherValidator` under `"__all__"`). The `djangorestframework` dependency is soft: the package imports without DRF, and `SerializerMutation` is a lazy root export resolved through the package `__getattr__` (not in `__all__`). See [`GLOSSARY.md#serializermutation`][glossary-serializermutation].
- auth mutations (new in `0.0.13`) — the opt-in session-auth surface over `django.contrib.auth`: four field factories at the `django_strawberry_framework.auth` submodule path (imported explicitly, never injected into a schema and never a package-root export) — `login_mutation()` (flat `username:` / `password:` arguments; a failed authentication is ONE non-field `FieldError`, `"Incorrect username/password"`, never a credential-enumerating split), `logout_mutation()` (the pinned model-less `{ ok, errors }` payload; idempotent), `register_mutation()` (a narrow `DjangoMutation` create over `get_user_model()` whose input is structurally limited to `(USERNAME_FIELD, *REQUIRED_FIELDS, password)` — privilege columns unreachable — with `validate_password(password, user)` failures keyed to `password` and the password stored only through `set_password`), and `current_user()` (the nullable session actor, `null` for anonymous — never an error). The user surface types as the consumer's own primary `DjangoType` over `get_user_model()` (validated loudly at finalize), and every factory accepts `permission_classes=` through the standard `check_permission` machinery with the explicit allow-any default (the documented auth inversion of the write family's deny-by-default). Session transport requires Django's `SessionMiddleware` + `AuthenticationMiddleware`; Channels/websocket auth waits for the `0.0.14` router card. See [`GLOSSARY.md#auth-mutations`][glossary-auth-mutations].
