...
- `get_queryset` visibility hook (cooperates with the optimizer via `Prefetch` downgrade). As of the visibility-boundary hardening on `main`, every framework-owned invocation of the hook runs through one shared hardened boundary (`utils/querysets.py`): a `Manager` return is coerced through `.all()`; `None`, lists, generators, and other non-queryset returns fail closed, as does a queryset off the registered type's concrete table (proxy siblings compose, MTI children do not); under a pinned resolution (an active write pipeline, or a source explicitly routed with `.using(...)`) an unrouted hook return is repinned onto the pinned alias while an explicitly divergent one fails closed; and an evaluated queryset supplied to or returned by the hook is refreshed to a lazy clone so cached rows never cross the visibility boundary. An async `get_queryset` resolving to a nested awaitable (and an async consumer resolver resolving to a residual awaitable) also fails closed rather than skipping visibility.
