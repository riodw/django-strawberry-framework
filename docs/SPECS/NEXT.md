...
`TODAY.md`'s "What products is still waiting for" section is the canonical pre-state for what's NOT shipped. The spec's `Current state` and `Out of scope` sections will reuse this framing.
...
- The archive set is every `docs/spec-*.md` and its matching `*-terms.csv` / `*-rationale.md` currently at the top level of `docs/`.
...
- Do run the reference-definition rewrite, all moved-spec `SpecDoc.path` repoints, and the `KANBAN.md` / `KANBAN.html` regeneration.
...
- **Cross-references**: source files are cited as repository-relative paths using symbol-qualified `path::QualifiedName` form (e.g. `types/base.py::DjangoType.resolve_fields`), with `#"unique substring"` pinpoints for in-body lines and `path #"unique substring"` for module-level lines (per [`AGENTS.md`][agents] #"Source refs in docs and code comments use symbol paths never line numbers"); KANBAN cards by their full ID; prior specs by markdown link; upstream packages by absolute local path from `docs/TREE.md`.
...
The "active" spec is the one you just authored in Step 6. Every other `spec-*.md` is a prior in-flight spec from an earlier card cycle that should now live under `docs/SPECS/` alongside the older legacy specs.

**A spec archives to `docs/SPECS/`; its companions archive one level further, to `docs/SPECS/appx/`.** While a spec is live its `*-terms.csv` (glossary terms) and `*-rationale.md` (the deliberative layer, when the build cycle produced one) sit beside it at `docs/`; on archive they separate, so `docs/SPECS/` holds specs and nothing else. Both companions are optional — a legacy spec may have no rationale — and each is archived only if it exists. The split has two link consequences, both handled in action 3 below: a moved spec's own reference to its companion gains an `appx/` prefix, and a moved rationale lands **two** directories deeper than it started, not one.
...
> 2. **FROM the moved spec → everywhere else** (links INSIDE the moved spec body pointing at repo files). Every relative path inside the spec body just shifted one directory deeper. In current specs those paths are usually bottom link definitions, not inline links — e.g. `[kanban]: ../KANBAN.md` from `docs/spec-XXX-…` is correct, but from `docs/SPECS/spec-XXX-…` it must become `[kanban]: ../../KANBAN.md`; `[rf-inputs]: ../django_strawberry_framework/...` must become `[rf-inputs]: ../../django_strawberry_framework/...`; `[glossary-foo]: GLOSSARY.md#foo` must become `[glossary-foo]: ../GLOSSARY.md#foo`. The file's contents did not change but the file's location did, and every relative target inside it is now wrong by one `../` level unless rewritten — by **two** levels for a companion `*-rationale.md`, which archives into `docs/SPECS/appx/` (see action 3). **This is the failure mode that gets missed** because the visible diff is just a rename — the broken links live inside the moved file's unchanged body.
...
2. **Move each candidate** into `docs/SPECS/` and its companions into `docs/SPECS/appx/`, preserving filenames:

   ```
   git mv docs/spec-<old_NNN>-<old_topic>-<old_version>.md docs/SPECS/
   git mv docs/spec-<old_NNN>-<old_topic>-<old_version>-terms.csv docs/SPECS/appx/
   git mv docs/spec-<old_NNN>-<old_topic>-<old_version>-rationale.md docs/SPECS/appx/
   ```

   Use `git mv` rather than `mv` so the rename is tracked. If the file is not yet tracked by git, fall back to `mv`. Skip either companion line when that file does not exist; `docs/SPECS/appx/` already exists, so no `mkdir` is owed.
3. **Rewrite cross-references INSIDE each moved spec** (the "FROM the moved spec → everywhere else" direction — the failure mode most likely to be missed). When a spec at `docs/spec-<old_NNN>-…` becomes `docs/SPECS/spec-<old_NNN>-…`, every relative link target inside its body must be re-relativized one directory deeper. Current specs use reference-style links, so enumerate reference definitions first, then legacy inline links:
   ...
   - **The moved spec's own companions** (`*-terms.csv`, `*-rationale.md`): they are no longer siblings — they went one level further, into `appx/`. A def like `[terms]: spec-<old_NNN>-…-terms.csv` becomes `[terms]: appx/spec-<old_NNN>-…-terms.csv`, and every `[rationale-dN]: spec-<old_NNN>-…-rationale.md#…` becomes `appx/spec-<old_NNN>-…-rationale.md#…` with its anchor untouched. Any prose that names a companion's repo-relative path gains the `appx/` segment too.

   Then do the same for **the moved rationale file itself**, which is the one file that shifts **two** levels (`docs/` → `docs/SPECS/appx/`): every relative target inside it gains two `../`, not one, except targets naming a file that moved with it. Its links back to its own spec (`spec-<old_NNN>-…md#decision-N`) become `../spec-<old_NNN>-…md#decision-N`, and `[next]: SPECS/NEXT.md` becomes `[next]: ../NEXT.md`. Companion `*-terms.csv` files hold no links and need no content edit — verify rather than assume, since a `notes` column could name a path.
...
The flow is complete when Step 8 finishes: only the active spec and its companions live at `docs/spec-*`, every prior spec is at `docs/SPECS/spec-*` with its companions at `docs/SPECS/appx/`, every cross-reference resolves (including in `KANBAN.md` / `KANBAN.html`), exactly one non-Done card has `status.key == "wip"`, and the checker exits 0 against the active spec.
...
- Do **not** modify any file other than the new spec file, its companion `*-terms.csv`, the spec and companion files being archived under Step 8, the cross-reference updates Step 8 prescribes in `docs/`, `README.md`, `GOAL.md`, `TODAY.md`, and `AGENTS.md`, and — for kanban — the regenerated `KANBAN.md` / `KANBAN.html` plus `examples/fakeshop/db.sqlite3` carrying the `apps.kanban` rows they are rendered from.
- Do **not** commit.
...
- The artifacts this flow produces: the new spec file, its companion `*-terms.csv`, the moves of every prior `spec-*.md` from `docs/` to `docs/SPECS/` (with its companions to `docs/SPECS/appx/`), path-only cross-reference updates in every doc that pointed at a moved spec, and — for the active WIP card's spec reference, any moved-spec links, and queue normalization in generated kanban — `examples/fakeshop/db.sqlite3` carrying the `apps.kanban` rows plus the regenerated `KANBAN.md` / `KANBAN.html` they export to.
