...
The load-bearing half of this cycle. Every item in the spec's `## Slice checklist`, `## Goals`, `## User-facing API`, `## Test plan`, and `## Definition of done` was walked, and **no item was found that was never shipped.** The full disposition table lives in this cycle's R1 artifact, `docs/builder/bld-017-r1-rationale_and_spec_reconciliation.md`. Two claims needed more than a grep, and both were re-derived rather than accepted:
