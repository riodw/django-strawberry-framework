...
Together these four commits produce the three-way byte-identity contract the spec now states: `stdout` output, the `--path` file's bytes, and `print_schema(schema)`'s return value are identical. `examples/fakeshop/tests/test_export_schema.py::test_export_schema_stdout_matches_print_schema` and `::test_export_schema_path_file_matches_print_schema` assert the three against the shipped `config.schema`, and `::test_export_schema_file_write_disables_newline_translation` pins the `newline=""` kwarg at the `write_text` call.
...
| `--path` whitespace-only (widened from empty-string) | `7f04c5b2` | `examples/fakeshop/tests/test_export_schema.py::test_export_schema_raises_command_error_when_path_flag_is_whitespace_only` |
...
| selector with an empty module path | `61f6726c` | `examples/fakeshop/tests/test_export_schema.py::test_export_schema_raises_command_error_for_malformed_selector` (cases `""` and `":schema"`) |
