...
| `246d1dad` | spec-050 review fixes. (i) **The request context stopped being the authority for the budget.** `begin_resource_budget` / `end_resource_budget` arm a module-private `ContextVar`; `policy_from_info` reads it and ignores `DST_RESOURCE_POLICY` entirely while it is armed, and `check_deadline` takes the tighter of the armed deadline and `DST_RESOURCE_DEADLINE` (`_effective_deadline`), so the mirror can narrow and can no longer widen or clear. At the release a resolver could widen `max_list_rows`, the `offset` ceiling and `max_page_size`, or buy unlimited wall clock, by writing its own `info.context`. (ii) A numeric deadline that is **not finite** is refused rather than read as a distant future. (iii) **The bound domain became the EXACT built-in type** for every integer bound and for the deadline: a numeric SUBCLASS that compares normally was accepted and STORED at the release, and its `__format__` / `__ceil__` / reflected `__radd__` then reached the rejection message, `math.ceil`, and the `time.monotonic() + seconds` derivation — the last of which could return `nan` and silently disarm an accepted policy's deadline. | correction | [D1][spec-047-d1], [D2][spec-047-d2], [D9][spec-047-d9], Edge cases |
| `fb6ccdb6` | spec-050 review fixes. **The exact-built-in domain moved from the policy's stored bounds to every number that crosses into the budget machinery**, stated once as `resource_policy.py::_is_builtin_number`. (i) A consumer-written `DST_RESOURCE_DEADLINE` is admitted only as an exact built-in: a numeric SUBCLASS answers both the comparison the narrowing rule is decided by and the one deciding whether time is up, so it could present as a narrowing and then read as time remaining, running an operation past an armed deadline that had already passed. (ii) An uploaded file's reported `size` and a `first` / `last` page bound supplied through a variable are held to the same domain; a subclass reaching either poisoned the running aggregate through its reflected `__radd__` / `__rmul__`. (iii) `_ValueBudget._reject` refuses any charged amount that is not a built-in integer, so an amount read off a request value - a buffer's `nbytes` - cannot answer the comparison that would reject it. | correction | [D1][spec-047-d1], [D2][spec-047-d2], [D4][spec-047-d4] |
| *(uncommitted at the time of writing)* | spec-050 review fixes. **The object a bound is read from stopped being an object consumer code can name.** (i) `schema.resource_policy` and `schema.error_policy` answer with a copy; the operation's budget is a private snapshot taken at the arm point; the `dst_resource_policy` mirror is a third object; and `policy_from_info` returns a copy rather than the snapshot or the `DEFAULT_RESOURCE_POLICY` singleton. Before this, `policy_from_info` handed every resolver the armed object itself, one `__dict__` write widened every bound for the rest of the process's life rather than the request's, and the same write through `info.schema.error_policy` turned masking off and put raw exception text on the wire. (ii) A policy SUBCLASS is read out once at schema construction and replaced by an exact instance (`utils/policies.py::canonical_policy`): `__post_init__` speaks for one past read, not for a read a seam performs later. (iii) `DjangoSchema.get_extensions` deduplicates the resource-policy extension as it already did the error-policy one, so a consumer factory no longer resolves beside an automatic entry that arms last and answers every resolve-time bound with the package defaults. | correction | [D1][spec-047-d1], [D2][spec-047-d2], [D11][spec-047-d11], Edge cases |
| *(uncommitted at the time of writing)* | spec-050 review fixes. **`max_document_tokens` and `max_depth` are stated as request-level bounds**, charged over the whole document text including operations the request did not name, because they bound the parse and the parse reads the whole document whatever `operationName` says. The named-operation rule covers every bound charged after the parse. No behavior changed; the Edge case claimed the named-operation rule for all five document bounds while the shipped scan had never applied it to two of them. | correction | [D3][spec-047-d3], Edge cases |
...
- **[Decision 2][spec-047-d2] — "a value no comparison can order" as the shape a written
  deadline is refused for.** Being unorderable was never the escape; answering the comparison
  was. A numeric subclass orders perfectly well, in whichever direction it chooses per call,
  which is how one value reads as a narrowing where the narrowing rule asks and as time
  remaining where the clock does. Nothing may describe the mirror's refusal as a fallback for
  values that cannot be compared.
- **[Decision 1][spec-047-d1] — "frozen" as the reason a resolver cannot widen its own
  budget.** A frozen dataclass rejects `setattr` and admits `policy.__dict__[bound] = wider`,
  so freezing is an accident guard and was never the authority boundary. What holds the bound
  is that no consumer-visible name reaches the object a bound is read from. Nothing may cite
...
- **[Decision 1][spec-047-d1] — the exact-built-in domain as a rule about STORED bounds.**
  ...
  amount is compared against its limit, accumulated into a running total, and formatted into a
  rejection; a subclass reaching one answers all three, and a reflected `__radd__` or
  `__rmul__` turns the total into `nan`, which no bound is ever exceeded by again.
