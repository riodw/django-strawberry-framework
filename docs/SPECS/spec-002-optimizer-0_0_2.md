...
## O4 extraction
The detailed O4 implementation record lives in `docs/SPECS/spec-003-optimizer_nested_prefetch_chains-0_0_2.md`. This parent spec only records the shipped behavior at a high level.
...
## Visibility status
O1 through O6 have shipped. The optimizer is public via `DjangoOptimizerExtension`.
...
The visibility-leak / `Prefetch` downgrade discussion that motivated bundling the optimizer with `spec-001-django_types-0_0_1.md` originally: issue #572 and PR #583 on `strawberry-graphql/strawberry-django`.
