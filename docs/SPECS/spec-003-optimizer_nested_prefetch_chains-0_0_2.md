...
- Apply the FK-id elision check at this level; B2 short-circuits before recursion. The FK-column append above must stay **ahead** of this short-circuit: an elided branch returns without planning a join, and the resolver that serves it reads the source row's FK column, so appending the column after the short-circuit would leave it unprojected and silently reintroduce the N+1 the elision exists to remove. The order is pinned by assertion, not by statement position alone: the same document run at two parent cardinalities issues an equal, absolute query count, and only that count distinguishes an elided read from a per-row lazy load — the wire result is identical either way.
