...
`spec-001-django_types-0_0_1.md` defines the pipeline (Meta validation, scalar synthesis, relation conversion, choice enums, `get_queryset` hook). This spec defines the boundary of that pipeline — what is rejected, what is reserved for the future, what consumers can and cannot rely on.

`spec-002-optimizer-0_0_2.md` defines the N+1 optimizer subsystem. The `has_custom_get_queryset` sentinel landed alongside the override-detection contract in 0.0.3 and is consumed by O6's `Prefetch` downgrade rule; this spec covers the type-system half of the contract.
