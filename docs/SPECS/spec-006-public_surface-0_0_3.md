...
- `deferred` — Reserved for a future spec; no work in flight; no Meta key accepted today (or accepted-and-rejected per `spec-005-django_type_contract-0_0_3.md` "Accepted vs deferred Meta keys"). Not importable.
...
- The optimizer-visibility decision specifically is amended into `spec-002-optimizer-0_0_2.md` "Visibility status" so the optimizer spec carries the local context for its own re-export trajectory; this spec carries the rule that produced the decision.
