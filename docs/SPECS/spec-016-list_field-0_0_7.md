...
- **Catch-all `**kwargs` in the resolver signature** (rev1 / rev2 / rev3 pseudocode used `**kwargs` defensively). Rejected by rev4 H1 — Strawberry treats every parameter as a GraphQL argument by default; an annotated `**kwargs: Any` is interpreted as a `kwargs` argument and later fails schema construction with `Unexpected type 'typing.Any'`; an unannotated `**kwargs` fails earlier with `MissingArgumentsAnnotationsError`. GraphQL arguments for filter/order/search land in their own Layer-3 specs (`WIP-ALPHA-021-0.0.8` / `WIP-ALPHA-022-0.0.8` / `TODO-BETA-039-0.1.2`) with named, typed kwargs; this card does not anticipate them.
