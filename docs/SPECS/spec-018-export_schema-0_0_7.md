...
- [ ] Slice 3: Promotion + docs
  ...
  - [ ] No edits to [`README.md`][readme], [`GOAL.md`][goal], or [`TODAY.md`][today]. Justification: the command is `manage.py` plumbing, not a consumer-name surface change; the fakeshop schema is unchanged by this card. Same posture as [`docs/SPECS/spec-017-apps-0_0_7.md`][spec-017] Slice 3 for the AppConfig.
  - [ ] Version bump (deferred to **the last `0.0.7` card to ship**, NOT this card; per [Decision 9](#decision-9--joint-0_0_7-cut)): see [`docs/SPECS/spec-016-list_field-0_0_7.md`][spec-016] Decision 10. This card does NOT bump `pyproject.toml`, `django_strawberry_framework/__init__.py`'s `__version__`, or `tests/base/test_init.py`'s version assertion.
  ...
  - [ ] Final gates (same posture as [`docs/SPECS/spec-017-apps-0_0_7.md`][spec-017] rev6 L2):
...
- A second [`Django AppConfig`][glossary-django-appconfig] hook for the command. Django's `manage.py` discovers commands by walking `management/commands/` directories in installed apps — no AppConfig method is involved. See the [`docs/SPECS/spec-017-apps-0_0_7.md`][spec-017] [Decision 4][spec-017-decision-4--no-readyhook-in-0-0-7] `ready()`-body deferral, which is preserved here.
...
- Symmetric with [`docs/SPECS/spec-017-apps-0_0_7.md`][spec-017] [Decision 3][spec-017-decision-3--no-public-export] for [`Django AppConfig`][glossary-django-appconfig], and with strawberry-django which does not re-export its `Command` either.
...
**No auto-call to [`finalize_django_types()`][glossary-finalize-django-types].** The consumer's `config/schema.py` (or equivalent) calls [`finalize_django_types()`][glossary-finalize-django-types] before constructing the schema; resolving the dotted path triggers the consumer's module-level imports, which run the finalize call as a side effect. Adding a `finalize_django_types()` call in `handle()` would either be silently redundant (the consumer's module already ran it) or — if the consumer's schema module deferred finalization to a function — would call it too early (before the consumer's imports are complete). The same anti-pattern is pinned in [`docs/SPECS/spec-017-apps-0_0_7.md`][spec-017] [Decision 4][spec-017-decision-4--no-readyhook-in-0-0-7] for [`Django AppConfig`][glossary-django-appconfig]`.ready()`; the same logic applies here.
...
- [`docs/README.md`][readme]
  - **Surgically remove the entire `- schema export management command` bullet** in the [`docs/README.md`][readme] #"**Coming in `0.1.0`**" section. DONE-017-0.0.7 removed only `, Django `AppConfig`` from that line; this card removes the remainder.
...
<!-- docs/SPECS/ -->
[spec-016]: spec-016-list_field-0_0_7.md
[spec-016-decision-10--joint-007-cut]: spec-016-list_field-0_0_7.md#decision-10--joint-007-cut
[spec-017]: spec-017-apps-0_0_7.md
[spec-017-borrowing-posture]: spec-017-apps-0_0_7.md#borrowing-posture
[spec-017-decision-3--no-public-export]: spec-017-apps-0_0_7.md#decision-3--no-public-export
[spec-017-decision-4--no-readyhook-in-0-0-7]: spec-017-apps-0_0_7.md#decision-4--no-readyhook-in-0_0_7
[spec-017-decision-5--no-default-auto-field-and-no-models]: spec-017-apps-0_0_7.md#decision-5--no-default_auto_field-and-no-models
