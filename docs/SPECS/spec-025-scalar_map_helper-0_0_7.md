...
2. Establish the registration helper any future package-defined scalar (`Upload`, `DONE-037-0.0.11`) reuses without API change.
...
- The spec filename uses `0_0_7` per the card tag (`spec-025-scalar_map_helper-0_0_7.md`) because the card is `DONE-025-0.0.7`. If a future maintainer re-tags the card to `WIP-ALPHA-020-0.0.8` (because it ended up in the 0.0.8 cut), the spec file moves with the rename — see [Risks](#risks-and-open-questions) entry 1.
...
- [`KANBAN.md`][kanban] (Slice 5)
  - Move `DONE-025-0.0.7` to the Done column with the next available `DONE-NNN-0.0.7` id (the column-move pass renumbers as usual; the next available id is determined at merge time, not pinned in this spec). Past-tense Done body:
  ...
  - Update the `### In progress` summary paragraph (anchored at [`KANBAN.md #"### In progress"`][kanban]) to remove `DONE-025-0.0.7` from the remaining-cards list once this card moves to Done.
...
- Promoting [`Upload`][glossary-upload-scalar] (planned for `0.0.11`) early. `Upload` is `DONE-037-0.0.11`; the helper this card ships is the registration point `Upload` reuses without modification.
...
- AppConfig and Trac #37064 hardening: [`docs/SPECS/spec-021-apps-0_0_7.md`][spec-017] and `DONE-024-0.0.7` — independent shipped siblings.
