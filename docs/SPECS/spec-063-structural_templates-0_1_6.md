...
Planned for `0.1.6` (card `TODO-BETA-063-0.1.6`, created 2026-08-08 seated
immediately ahead of the optimizer-explain card `TODO-BETA-064-0.1.6`; the
former cards 063–069 shifted up by one and the post-`0.1.5` patch versions
shifted up by one). **The explain card shares this patch version, lands
last, and owns the `0.1.6` version cut, so this spec defers every
release-state artifact to it**
([Decision 1](#decision-1--seat-063-before-explain-joint-cut-at-016)).

The second of two graph foundation cards, sibling to the graph substrate
([`spec-053`][spec-053]). Where card `053` ships the shared planning
...
Status: **PLANNED — no slice built yet; card created (`TODO-BETA-063-0.1.6`).**
...
This card consumes — and is gated on — card `053` landing first: `EdgeScope`
...
Card `053`'s spec explicitly "prepares the structural/bound split the
sibling card ships" ([`spec-053`][spec-053] Decision 8 posture); this card
is that sibling.
...
[`spec-063-structural_templates-0_1_6-terms.csv`][templates-terms]
...
`GraphPathPlan`, the operation memo) is defined by [`spec-053`][spec-053];
...
- `optimizer/nested_planner.py::_divergent_key_windows` classifies `filter:`
  ...
  selected). [`spec-053`][spec-053] deliberately pinned this arm as
...
- `optimizer/nested_fetch.py::unwindowable_child_queryset_reason` rejects a
  ...
  emits one row per qualifying sibling and still windows. [`spec-053`][spec-053]
...
- **No explain rendering.** The operation plan map is the *data*; rendering
  it is the explain card (`TODO-BETA-064-0.1.6`), which is
...
### Decision 1 — Seat `063` before explain; joint cut at `0.1.6`

The hard constraints are a floor and a ceiling: after card `053` (this card
...
map). Within that window the card seats at `063`, immediately ahead of
explain — the tightest satisfying seat, matching the audit's proposed order,
and the smallest renumber (the former cards `063`–`069` shifted to
`064`–`070`).
...
(`TODO-BETA-064-0.1.6`). Explain renders what this card publishes — natural
...
[`spec-053`][spec-053]'s posture verbatim: **no request value — user,
...
- exact owning GraphQL type identity (object identity, not name —
  [`spec-053`][spec-053]'s exact-owner rule);
...
(`EdgeScope`, from card `053`), and materializes the concrete `Prefetch`.
...
filtered-connection arm [`spec-053`][spec-053] pinned as characterized-only.

### Decision 8 — Row-identity proof enforcement; never an automatic `DISTINCT`

`RowIdentityProof` (vocabulary from card `053`, baseline from its Slice 2)
...
Meta keys and sidecar `Set` classes as card `053` pins it; this card adds no
...
`django_strawberry_framework/graph/` (card `053`) and neither is imported by
...
This card owns the four reproductions [`spec-053`][spec-053]'s R-index
...
| R3 (filtered arm) | Filtered nested connection batching | The per-parent fallback counts pinned as *characterized* by card `053` become asserted equalities: query count identical for 1 and 100 parents with a `filter:` argument |
...
- The explain card's amendment (consume the plan map; retire
  ...
  those cards at this card's creation, mirroring card `053`'s
...
- **Fingerprint normalization completeness.** A slot the fingerprint misses
  ...
  equivalence corpus plus an adversarial arm in the `068` suite guard it;
...
- **`FieldDependencyPlan` vocabulary breadth.** The template's dependency
  graph consumes whatever vocabulary has shipped by then (`053` ships the
  narrow form; `054` expands it). Binding treats unknown dependency kinds
  as bind-time walks, so the template layer does not gate on `054`'s
  expansion.
...
- **Explain rendering over the plan map** — the explain card
  (`TODO-BETA-064-0.1.6`) owns the surface and the legacy-key
  retirement.
- **Adversarial attack suite** — the adversarial card (`TODO-BETA-068-0.1.8`),
...
- **Optional PostgreSQL repeatable-read snapshot policy** — non-gating,
  unscheduled (card `053`'s R11 disposition).
...
[templates-terms]: spec-063-structural_templates-0_1_6-terms.csv
