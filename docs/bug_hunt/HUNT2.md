# Plan: rebuild the bug-hunt agentflow (HUNT.md v2)

Status: proposal, second draft, for maintainer decision. No repo mechanism changes yet. The first
draft was revised after an adversarial review; every finding of that review is folded in below and
the places it changed are marked.

The one-line version: **specify contracts and a pilot first, build orchestration second.** The
first deliverable is a three-scenario pilot specification plus a runner that provably rejects bad
evidence. The full workflow rewrite, generators, dependency groups, and any second profile are
built only from what the pilot earns.
...
two numbers describe different runs of the same release. That collision is finding 15's subject.
...
- Debris: the 0.0.15 header still reads `Status: in-progress`; `docs/shadow/current/` does not
  ...
  while the method lives only in `HUNT.md`; `AGENTS.md` spells the glob `bug_hunt.*.md` while the
...
- **The uniform sweep is the wrong unit.** High confidence for transaction, authorization,
  ...
  code invisible (finding 7).
- **The historical record is not a controlled comparison of designs.** Any claim that a proposed
  mechanism would have found more is a hypothesis for the pilot, not a conclusion.

## 2. Design principles (each from a review finding)

1. **An error-contract table replaces the blanket containment rule.** `HUNT.md`'s "every
...
2. **A property is validated before it can define a fix.** The first draft proposed `pascal_case`
   idempotence; `utils/strings.py::pascal_case` is documented for snake_case input and
   `pascal_case("PaymentMethod")` is `"Paymentmethod"` by design. A hunter obeying the draft would
...
3. **Per-example state isolation is designed before Hypothesis touches a write path.** pytest-django
   ...
   state, not a seed.
4. **A scratch directory is not a sandbox.** Nothing in `HUNT.md` or the first draft stops a probe
...
13. **Permanent regression dependencies live where CI installs them.** The first draft put
    Hypothesis in an optional group and also promised permanent property tests; ordinary CI installs
    `dev`, so those tests would fail collection or be skipped silently. Rule: either a dependency a
...
15. **Run identity is separate from release identity.** Two runs of one release collide under both
    `bug_hunt-<release>.md` and the first draft's `hnt-<release>-*` names; the 0.0.14 record already
    ...
    input.
16. **Leads keep provenance and never become oracles.** `HUNT.md`'s ban on historical seeding
    discards useful failure patterns; the first draft overcorrected by merging maintainer questions,
    `pbugs.md`, and root `vulns.md` into one required `dicta.md` and deleting the sources, which
    turns old hypotheses and stale `pending` labels into expected results. Rule: maintainer
    questions stay in `dicta.md`, separate from generated or history-derived leads. Each lead
    carries provenance, contract, last-tested tree state, and disposition, and is revalidated on
    current source. An independent discovery pass runs before prior diagnoses are exposed. Every
    numbered item in `pbugs.md` and `vulns.md` is reconciled with an explicit counting method before
    either file retires; `vulns.md` lives at the repository root.
17. **Pilot before infrastructure.** The first draft committed to seven dependencies, generators,
    artifacts, custom rules, and a second flow before measuring benefit. Review mode is read-only;
...
- Matched inputs and budgets: the current `HUNT.md` method and the proposed method each run the
  three scenarios.
...
- `HUNT.md` v2 text, the scenario-record template, the generator and its tests, `START.md` and
  `AGENTS.md` references, cleanup and link rules, all in one change; active records migrated
  explicitly, historical runs never overwritten.
