...
Example: spec `docs/spec-017-deferred_scalars-0_0_6.md` pairs with build plan `docs/builder/build-013-deferred_scalars-0_0_6.md`. Earlier specs predating this pattern may live without the NNN/version segments; new specs and their build plans use the pattern.
...
The spec's nested sub-bullets for this slice from `## Slice checklist`, copied verbatim as `- [ ]` boxes (preserve exact text, nested sub-bullets, inline citations). **Worker 2 ticks each `- [x]` as it implements that sub-check during the build pass (and on re-passes)** — the box flips to `- [x]` in the same build report that lands the contract, so progress is visible incrementally instead of only at the end. Worker 2 ticks ONLY a box whose contract actually landed in its diff; a box whose contract is deferred or not yet built stays `- [ ]` (Worker 2 notes the deferral in its build report). **Worker 1 audits these boxes at final verification** (it is no longer the original ticker): confirm each `- [x]` truly landed in the diff (un-tick it and set `revision-needed` if a box was ticked without matching implementation), tick any box whose contract landed that Worker 2 left un-ticked, and for any remaining `- [ ]` record a one-line deferral reason under `### Spec changes made (Worker 1 only)` or set the slice `revision-needed`. Silently un-ticked-and-undeferred boxes block `final-accepted`. Worker 3 walks the list during review; a sub-check that appears silently un-addressed in the diff is a Medium finding, AND a box ticked `- [x]` with no matching implementation in the diff is likewise a finding (Worker 2 over-ticked).
...
6. **Sweep the whole tree for staged anchors naming this build's spec OR card:** `grep -rEn 'TODO\(spec-<NNN>|TODO-(ALPHA|BETA|STABLE)-<NNN>' .` (also catch the card-id form some slices use to stage work, e.g. `TODO-ALPHA-037-0.0.11`). Every pre-placed `TODO(spec-<NNN> Slice <M>)` anchor — and every `TODO-<MILESTONE>-<NNN>` card-id anchor left in shipped source/tests/comments — must be discharged by the build's end: the work it names has landed **and** the anchor was removed in the slice that shipped that work (replace with non-TODO provenance such as `spec-<NNN>` / `DONE-<NNN>` where historical context is useful, otherwise delete it). `AGENTS.md`'s "shipped behavior folds into `docs/TREE.md` … and the staged anchor is removed in the same change that ships the slice" is standing authority even when the file carrying the anchor was omitted from the spec's `## Slice checklist` or `## Doc updates` lists — a slice checklist that forgets a staged-anchor file does not waive the obligation. (Exclude `KANBAN.md` / `KANBAN.html` / `BACKLOG.md`, where `TODO-<MILESTONE>-<NNN>` legitimately names not-yet-shipped board cards.) Record any still-present anchor as a finding and route it to the owning slice (re-loop) before the build closes.
...
7. The maintainer commits the post-closeout updates **separately** from the pre-closeout build commit. The pre-closeout commit (per the `## Slice handoff (no maintainer pause between slices)` handoff) already carried the source/test changes, the completed build plan, and every `bld-*.md` artifact — that commit is what Worker 0 just scanned in step 1 above. The post-closeout commit contains only the retrospective workflow-doc updates (any approved edits to `docs/builder/BUILD.md` / `docs/builder/worker-*.md` from step 5), plus any explicit new closeout artifact if one was created. Splitting the two commits keeps the build-cycle commit range stable (Worker 0 scanned a fixed set of commits, not a moving target) and keeps the retrospective change attributable to its own commit for `git log` archaeology.
