...
**The three round artifacts named throughout this file were deleted at cycle close** (`bld-review-1-rationale_and_spec_reconciliation.md`, `bld-review-2-db_backed_doc_reconciliation.md`, `bld-integration.md`), so every citation of them below is a provenance record, not a live path. Each was `final-accepted` before deletion, and their content is preserved where it matters: the deferred-work catalog re-derives every population inline rather than referring out. They remain readable at commit `36cd1925`.
