...
Spec reference: `docs/SPECS/spec-017-apps-0_0_7.md` (Slice 1 sub-bullets at lines 55-63; Decisions 1 / 2 / 3 / 4 / 5 / 8)
...
- None; tests land in Slice 2 per spec (`docs/SPECS/spec-017-apps-0_0_7.md:64-66` Slice 2 checklist; Worker 1 plan's `### Test additions / updates` section pins "0 (tests land in Slice 2)").
...
- **Class docstring**: used the spec's suggested wording verbatim (`"""Register django-strawberry-framework with Django's app loader."""`) per `docs/SPECS/spec-017-apps-0_0_7.md:61`.
...
- **Attribute order**: `name` first, `verbose_name` second per the spec's listing at `docs/SPECS/spec-017-apps-0_0_7.md:57-59` and the upstream `strawberry_django/apps.py` shape.
...
- Class docstring uses the spec's suggested wording verbatim (`docs/SPECS/spec-017-apps-0_0_7.md:61`); no rephrasing, no `# noqa: D101` suppression — the root-cause docstring per rev3 H1 / AGENTS.md line 4.
- Import shape `from django.apps import AppConfig` matches the upstream `strawberry_django/apps.py` exactly per the Borrowing posture; class name `DjangoStrawberryFrameworkConfig` matches the spec's pinning; attribute order (`name` then `verbose_name`) matches the spec's listing at `docs/SPECS/spec-017-apps-0_0_7.md:57-59`.
