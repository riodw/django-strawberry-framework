...
3. **Edit `docs/README.md` — add a one-line forward-pointer at the end of the `### Sharded mode (multi-DB)` section** per spec line 566. Current file ends the section at line 216 with `In sharded mode \`default\` → \`db_shard_a.sqlite3\` and \`shard_b\` → \`db_shard_b.sqlite3\`. The single-DB \`db.sqlite3\` is invisible while the env var is set.` followed by a blank line at line 217 and then `## Using the package in your own project` at line 218.
