...
Spec reference: `docs/SPECS/spec-017-apps-0_0_7.md` (Slice 3 sub-bullets at lines 67-79; Doc updates at lines 397-427; Decisions 6 / 7)
...
- **Duplication risk avoided.** Four specific risks the naive implementation could introduce, and how the plan blocks them:
  ...
  4. **Adding `tests/test_apps.py` after `test_list_field.py` instead of before it in `docs/TREE.md`'s current test-tree section.** `test_apps.py` sorts BEFORE `test_list_field.py` alphabetically (`a` < `l`); rev2 L1 explicitly corrected the rev1 wording that misplaced it "between `test_list_field.py` and `test_registry.py`." See [Doc updates](../SPECS/spec-017-apps-0_0_7.md#doc-updates) at spec line 412.
...
4. **`KANBAN.md` — three edits in one file.**
   ...
   - **Edit (b) — rewrite the body in past tense per the existing Done-column convention.** Current `In progress` body (`KANBAN.md:78-88`) is:
     ...
     ```
     The spec's suggested past-tense Done-column body at `docs/SPECS/spec-017-apps-0_0_7.md:415` is:
     ```text
...
Per the spec's [Doc updates](../SPECS/spec-017-apps-0_0_7.md#doc-updates) `CHANGELOG.md` bullet at `docs/SPECS/spec-017-apps-0_0_7.md:419`, the exact bullet text Worker 2 appends under the existing `[0.0.7]` `### Added` subsection (character-for-character; do NOT paraphrase):
...
- **CHANGELOG verbatim `diff` is the other load-bearing review surface.** Worker 3's `### CHANGELOG sanity` review section will `diff` the appended CHANGELOG bullet against `docs/SPECS/spec-017-apps-0_0_7.md:419`. Both strings should match character-for-character; no paraphrase was applied.
...
- `git diff -- django_strawberry_framework/__init__.py` is empty. `__all__` is unchanged per [Decision 3](../SPECS/spec-017-apps-0_0_7.md#decision-3--no-public-export) / DoD item 2 / DoD item 12.
...
- **No version bump in this card.** Verified: `pyproject.toml:4` is `version = "0.0.6"`; `django_strawberry_framework/__init__.py:26` is `__version__ = "0.0.6"`. Per [Decision 6](../SPECS/spec-017-apps-0_0_7.md#decision-6--joint-0_0_7-cut) / DoD item 11.
...
- The verbatim CHANGELOG bullet matches the spec's wording at `docs/SPECS/spec-017-apps-0_0_7.md:419` character-for-character, with the only delta being the bullet's leading `- ` list marker (mechanical, expected). Worker 2 quoted it cleanly without paraphrase.
...
- **Spec slice checklist:** every `- [ ]` in the Plan's `### Spec slice checklist (verbatim)` is now `- [x]`. Walked each box against the Slice 3 diff (`git diff -- docs/GLOSSARY.md docs/README.md docs/TREE.md KANBAN.md CHANGELOG.md`):
  ...
  - CHANGELOG append — one bullet added under the existing `[0.0.7]` `### Added` heading at `CHANGELOG.md:24`, character-for-character matching the spec's wording at `docs/SPECS/spec-017-apps-0_0_7.md:419`. Exactly one `## [0.0.7]` heading in the file (no parallel second heading per Decision 6).
