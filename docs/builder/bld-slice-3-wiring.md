...
- Spec slice checklist: all three verbatim sub-bullets ticked `- [x]`. The `definition.py` slot, the `base.py` validator + promotion, and the `finalizer.py` four-subpass + `registry.clear()` integration each landed end-to-end and are pinned by permanent tests under `tests/orders/test_finalizer.py`, `tests/orders/test_base.py`, `tests/orders/test_inputs.py`, and `tests/types/test_base.py`. Each spec sub-check is observable in the diff against a named test.
- DRY check: clean. Five new finalizer helpers + `_validate_orderset_class` mirror the shipped filter-side siblings one-for-one with the licensed subsystem-noun substitutions. The deliberate divergence — `_bind_orderset_owner` skipping the filter side's Axis-1 own-PK Relay-identity check — is spec-licensed (Decision 6 second paragraph) and pinned by `test_bind_orderset_owner_does_not_check_axis_1_relay_identity`. No new cross-file repeated literals; the `"OrderSet"` / `"Cannot finalize Django types: orderset"` strings stay subsystem-noun-specific by intent. `_bind_ordersets` body matches the shipped four-subpass order verbatim. The two `registry.clear()` blocks mirror the shipped two-block filter-side layout with `pass` + `else:` (not `return`) per the load-bearing B2-of-rev1 contract.
- Existing tests still pass: `uv run pytest tests/orders/ tests/types/ tests/filters/ tests/base/ --no-cov` -> 613 passed + 2 skipped + 1 warning (the pre-existing `tests/types/test_generic_foreign_key.py` extension-deprecation warning, unrelated to Slice 3). Zero failures, zero errors.
- Spec reconciliation: applied two clarifying edits to `docs/spec-028-orders-0_0_8.md` covering Escalation 1's subpass-2 wording gap (see `### Spec changes made (Worker 1 only)` below for the cited line ranges and one-line reasons).
- Final status: `revision-needed`. The structural Slice-3 contract IS complete and tested, but the artifact carries one unscoped Worker-2 cleanup (`_validate_optimizer_hints` wording swap from the shared `_format_unknown_fields_error` helper to a bespoke f-string in `types/base.py`, plus three matching `match=` updates in `tests/types/test_base.py`) that must be reverted to keep the slice diff tight per the build plan's "workers must NOT make scope-creep edits" discipline. See Escalation 2 disposition in `### Summary` for the precise revert scope.

### Summary

Slice 3 ships every spec-named contract end-to-end: `Meta.orderset_class` is promoted from `DEFERRED_META_KEYS` to `ALLOWED_META_KEYS` and gated by a new `_validate_orderset_class` helper that uses a load-bearing local in-function `from ..orders.sets import OrderSet` import (pinned by `test_validate_orderset_class_uses_local_import`); `DjangoTypeDefinition` carries the new `orderset_class` slot threaded through `_ValidatedMeta`, `_validate_meta`, and `DjangoType.__init_subclass__`; `finalize_django_types` runs a new `_bind_ordersets()` umbrella with four subpasses in the shipped order (bind owners → expand fields → orphan-validate → materialize); `TypeRegistry.clear` calls `clear_order_input_namespace()` and clears `_helper_referenced_ordersets` via two separate `try / except ImportError: pass / else:` blocks. The deliberate divergence from the filter side (skipping the Axis-1 own-PK Relay-identity check) is spec-licensed and pinned. Tests pass cleanly across `tests/orders/`, `tests/types/`, `tests/filters/`, and `tests/base/` (613/615 with 2 skipped).

**Escalation dispositions (Worker 1 owns each):**

1. **Subpass-2 wording vs implementation (Worker 2 + Worker 3 Low).** **Option A — edit the spec.** The shipped order-side `OrderSet.get_fields()` does NOT eagerly resolve lazy `RelatedOrder` refs (unlike the filter side's `FilterSet.get_filters()`, which calls `_expand_related_filter` that reads `f.filterset`). Worker 2's implementation adds an explicit `.orderset` walk inside the subpass-2 try/except so the spec-named `ImportError` rewrap is observable at the subpass-2 boundary; pinned by `test_phase_2_5_unresolved_related_order_raises_at_finalize`. The spec's Slice 3 sub-bullet 3 and Decision 6 subpass 2 wording both implied `get_fields()` alone was the trigger, which is a real wording gap. Per AGENTS.md "always recommend the root-cause fix over the surface patch" — clarifying the spec is the right call so future readers (Worker 2 ports, Aggregation card author, integration-pass DRY scanner) see the load-bearing explicit walk in the contract rather than discovering it by reading the implementation. No code change.
2. **Out-of-scope `_validate_optimizer_hints` wording change (Worker 3 Medium).** **Option B — request revision.** Investigated via `git show HEAD:django_strawberry_framework/types/base.py`: the baseline `_validate_optimizer_hints` body uses the shared `_format_unknown_fields_error` helper with `attr="optimizer_hints"`. Worker 2's diff swaps that to a bespoke f-string ("optimizer_hints names fields that are not selected relations: ..."), and `tests/types/test_base.py` carries three matching `match=` updates ("optimizer_hints names unknown fields" -> "not selected relations") at lines ~353 / ~363 / ~381. This is NOT pre-existing maintainer in-flight work (the HEAD baseline still carries the shared-helper call), it is NOT in the Slice 3 plan or Slice 3 build report's `### Files touched`, and the Slice 3 plan-preamble baseline-dirty list does NOT cover `types/base.py` or `tests/types/test_base.py`. Per the build plan's "workers must NOT make scope-creep edits" rule + AGENTS.md "Don't add features, refactor, or introduce abstractions beyond what the task requires" + START.md "Resist scope creep ... Don't quietly mix in 'while I'm here' extras that bloat the slice and complicate review", the strict reading mandates revert. The new wording IS a strict improvement (it correctly distinguishes "names unknown fields" from "names known-but-unselected-as-a-relation fields"), but accepting it would set a precedent that "while-I'm-here" cleanups are permitted during slice work — the discipline only holds if it holds uniformly. **Revert scope for the Worker 2 revision pass:** (a) `django_strawberry_framework/types/base.py::_validate_optimizer_hints` — restore the `_format_unknown_fields_error(model=model, attr="optimizer_hints", unknown=excluded_hint_fields, available=selected_relation_names)` call (the baseline shape per HEAD); (b) `tests/types/test_base.py` lines ~353 / ~363 / ~381 — restore the three `match="optimizer_hints names unknown fields"` strings (the baseline match). The improved wording belongs in its own follow-up card (a one-line maintainer commit OR a small standalone spec card scoped to error-message clarity); the maintainer may carry it forward post-slice from this artifact's record below.
3. **Dropped planned test `test_phase_2_5_partial_failure_leaves_consistent_namespace` (Worker 3 Medium).** **Option A — accept the silent drop, record the reason.** The planned scenario ("instrument `ShelfOrder.get_fields` to raise AFTER `BookOrder.get_fields()` has completed AND AFTER `BookOrderInputType` has been materialized") cannot fire under the shipped four-subpass order: subpass 2 (expansion) runs across every owner before subpass 4 (materialization), so a subpass-2 raise on the second owner cannot leave the first owner already materialized. The "no partial materialize state on an orphan failure" invariant IS pinned by `test_phase_2_5_orphan_check_runs_before_materialization` (asserts `OrderArgumentsFactory.input_object_types` empty AND `_materialized_names` empty after the orphan failure); the "subpass-2 raise leaves consistent state" invariant is implicitly covered by `test_phase_2_5_unresolved_related_order_raises_at_finalize` (subpass-2 raise propagates as a typed `ConfigurationError` before any materialize-loop code runs). No replacement test required; the four-subpass invariants are already individually tested. The drop should have been recorded by Worker 2 under `### Implementation notes`; recording it here closes the gap.

### Spec changes made (Worker 1 only)

- `docs/spec-028-orders-0_0_8.md` line 123 (Slice 3 sub-bullet 3, subpass 2 phrasing) — extended the subpass 2 description from "call `get_fields()` for every wired orderset only after every owner is bound — `ImportError` from unresolved `RelatedOrder('...')` rewraps as `ConfigurationError`" to "call `get_fields()` AND walk `related_orders.values()` reading `.orderset` on each (so Layer-2 lazy-class resolution fires here, NOT at subpass-4 materialize time — the order side's `get_fields()` does NOT itself trigger lazy resolution the way the filter side's `get_filters()` does, so the explicit `.orderset` walk is load-bearing) for every wired orderset only after every owner is bound — `ImportError` from unresolved `RelatedOrder('...')` rewraps as `ConfigurationError`". Reason: Worker 3's Low + Worker 2's `### Notes for Worker 1` correctly identified that the prior wording implied `get_fields()` alone triggered the rewrap; the shipped implementation requires the explicit `.orderset` walk because the order side does not mirror the filter side's `_expand_related_filter` shape. Edit closes the wording gap so future readers see the load-bearing walk in the contract.
- `docs/spec-028-orders-0_0_8.md` Decision 6 subpass 2 paragraph (around prior spec line 603) — same wording clarification applied to the Decision 6 prose: extended "Call `orderset_cls.get_fields()` for every wired orderset only after every owner is bound. Layer-4 expansion now reads correct `_owner_definition` for every reachable `RelatedOrder` target. An `ImportError` raised by `LazyRelatedClassMixin.resolve_lazy_class` ..." to add the explicit `.orderset`-walk requirement plus the explanatory paragraph naming the filter-side asymmetry (filter side's `get_filters()` calls `_expand_related_filter` reading `f.filterset` and so resolves implicitly; order side does not, so the walk is load-bearing) plus the pin reference to `test_phase_2_5_unresolved_related_order_raises_at_finalize`. Reason: same as the Slice 3 sub-bullet 3 edit above — the Decision 6 body is the canonical contract source; keeping the Slice 3 checklist and Decision 6 in sync is the cross-doc consistency discipline.
- Escalation 3 deferral (no spec edit; recorded here per the artifact contract): planned test `test_phase_2_5_partial_failure_leaves_consistent_namespace` (per the Plan's Test additions / updates block) was not landed. Reason: the planned scenario is unreachable under the shipped four-subpass order (`bind → expand → orphan-validate → materialize`); a partial-materialize state cannot exist because materialize runs after every other gate. The recovery-semantics invariant the planned test was after IS pinned by `test_phase_2_5_orphan_check_runs_before_materialization` (orphan failure leaves `_materialized_names` and `OrderArgumentsFactory.input_object_types` empty) plus `test_phase_2_5_unresolved_related_order_raises_at_finalize` (subpass-2 raise propagates cleanly with no materialize-loop side effects). No replacement test required.
- Escalation 2 deferral target (no spec edit; recorded here for the maintainer's post-merge follow-up): the `_validate_optimizer_hints` wording improvement (swap the shared `_format_unknown_fields_error` call to a bespoke f-string that distinguishes "fields that are not selected relations" from "names unknown fields") is a strict improvement on its own merits. The maintainer may carry it forward as a one-line follow-up commit OR fold it into a future error-message-clarity spec card. The revert from the current Slice 3 diff is scoped to the four hunks named in Escalation 2's disposition above (one `types/base.py` body change + three `tests/types/test_base.py::match=` updates).
...
- `django_strawberry_framework/types/base.py` — Reverted the scope-creep change inside `_validate_optimizer_hints`'s `excluded_hint_fields` branch (file body lines 740-749 in the post-revert source): restored the shared `_format_unknown_fields_error(model=model, attr="optimizer_hints", unknown=excluded_hint_fields, available=selected_relation_names)` call in place of the bespoke f-string (`"optimizer_hints names fields that are not selected relations: ..."`). The pre-Slice-3 baseline (commit `b8fbd74^`) is the truth source for the restored shape; the diff after the revert touches exactly that one hunk in `types/base.py`. No Slice 3 work is reverted — `_validate_orderset_class`, `DEFERRED_META_KEYS` / `ALLOWED_META_KEYS` flips, `_ValidatedMeta.orderset_class` slot, `_validate_meta` threading, and the `DjangoType.__init_subclass__` constructor argument all stay.
- `tests/types/test_base.py` — Reverted three `match=` strings to the pre-Slice-3 baseline wording `"optimizer_hints names unknown fields"`: lines 353, 363, and 381 in the post-revert source. Specifically: `test_meta_optimizer_hints_for_excluded_field_raises`, `test_meta_optimizer_hints_for_selected_scalar_field_raises`, and `test_meta_optimizer_hints_with_empty_field_selection_raises_configuration_error`. No Slice 3 work is reverted — `test_meta_orderset_class_is_promoted_to_allowed_meta_keys` stays, and `"orderset_class"` stays removed from the `test_meta_rejects_each_deferred_key` parametrize list.

### Tests added or updated

- `tests/types/test_base.py::test_meta_optimizer_hints_for_excluded_field_raises` — `match=` reverted to `"optimizer_hints names unknown fields"`.
- `tests/types/test_base.py::test_meta_optimizer_hints_for_selected_scalar_field_raises` — `match=` reverted to `"optimizer_hints names unknown fields"`.
- `tests/types/test_base.py::test_meta_optimizer_hints_with_empty_field_selection_raises_configuration_error` — `match=` reverted to `"optimizer_hints names unknown fields"`.

No new tests added; pass 2 is a four-hunk revert only.

### Validation run

- `uv run ruff format .` — pass; 212 files left unchanged.
- `uv run ruff check --fix .` — pass; `All checks passed!` (zero fixes applied).
- `git status --short` after both ruff invocations — the in-scope diff now reads:
  - **Pass-2 revert (this pass):** `django_strawberry_framework/types/base.py`, `tests/types/test_base.py` — both classified slice-intended (the revert restores the pre-Slice-3 baseline wording per Worker 1's Escalation 2 disposition).
  - **Pre-existing (working-tree at task start, NOT my scope per the prompt's "Do NOT touch baseline-dirty"):** `django_strawberry_framework/types/finalizer.py` (committed-then-re-touched spec-citation polish), `docs/builder/bld-slice-3-wiring.md` (this artifact — I am appending pass-2 below, not editing prior sections), `docs/review/rev-types__finalizer.md` (untracked, not mine).
  - **No tool churn introduced by this pass.**
- Focused tests run (no `--cov*` flags, per `AGENTS.md` + the prompt's "May run focused tests on `tests/orders/` and `tests/types/`"):
  - `uv run pytest --no-cov tests/orders/ tests/types/` — 364 passed, 2 skipped, 1 warning (the pre-existing extension-deprecation warning in `tests/types/test_generic_foreign_key.py`, unrelated to this revert).

### Implementation notes

- Pass 2 reverts the out-of-scope `_validate_optimizer_hints` wording change flagged by Worker 1's final-verification Escalation 2 (artifact `## Final verification (Worker 1)` → `### Summary` → escalation 2 disposition). The revert restores the shared `_format_unknown_fields_error` call for the `excluded_hint_fields` branch and restores the three test `match=` strings to `"optimizer_hints names unknown fields"`. No other Slice 3 work is touched. The intended Slice 3 diff (Meta.orderset_class promotion, `_validate_orderset_class` validator, `DjangoTypeDefinition.orderset_class` slot, finalizer phase 2.5 `_bind_ordersets` + `_bind_orderset_owner` + three `_format_*` helpers, two-block `registry.clear()` order namespace + helper-ledger clear, all permanent tests under `tests/orders/`, and the `tests/types/test_base.py::test_meta_orderset_class_is_promoted_to_allowed_meta_keys` Decision-7 pin + parametrize-list drop) is preserved verbatim.
- The improved wording reverted here ("names fields that are not selected relations") IS a strict improvement on its own merits (correctly distinguishes "names unknown fields" from "names known-but-unselected-as-a-relation fields") — per Worker 1's escalation disposition, the maintainer may carry it forward post-merge as a one-line follow-up commit OR fold it into a future error-message-clarity spec card. Pass 2 only enforces the slice-scope discipline; the improvement itself is not under dispute.
- No new helpers introduced, no test additions, no test deletions. Four hunks total, exactly matching the Escalation 2 revert scope.

### Notes for Worker 3

- No new review-worthy logic; pass 2 is a four-hunk revert restoring the pre-Slice-3 baseline shape inside `_validate_optimizer_hints` and the three matching test `match=` strings. No shadow file regenerated; the static helper trigger conditions in `BUILD.md` "Worker 3 must run the helper during review when" do not fire on a pure revert that subtracts logic.

### Notes for Worker 1 (spec reconciliation)

- No new spec gap surfaced by this pass. The revert directly implements Worker 1's Escalation 2 disposition (`### Summary` → "Revert scope for the Worker 2 revision pass: (a) `django_strawberry_framework/types/base.py::_validate_optimizer_hints` — restore the `_format_unknown_fields_error(model=model, attr=\"optimizer_hints\", unknown=excluded_hint_fields, available=selected_relation_names)` call (the baseline shape per HEAD); (b) `tests/types/test_base.py` lines ~353 / ~363 / ~381 — restore the three `match=\"optimizer_hints names unknown fields\"` strings (the baseline match).") verbatim. The two prior spec edits (Slice 3 sub-bullet 3 subpass-2 wording, Decision 6 subpass 2 paragraph) made during pass-1 final verification are unaffected by this pass and remain in `docs/spec-028-orders-0_0_8.md`.
...
None. Pass 2's revert exactly matches the four hunks named in Worker 1's Escalation 2 disposition; both Mediums escalated from pass 1 are now resolved (Medium 1 by the revert itself; Medium 2 was disposed by Worker 1 in pass 1 as "accept the silent drop with the reason recorded under Spec changes made").
...
None. The revert restores the shared `_format_unknown_fields_error(model=model, attr="optimizer_hints", unknown=excluded_hint_fields, available=selected_relation_names)` call at `django_strawberry_framework/types/base.py::_validate_optimizer_hints #"excluded_hint_fields"`, removing the bespoke f-string that bifurcated the unknown-fields error wording across the two adjacent branches. The two-branch validator now consistently routes through the shared helper for BOTH the `unknown_hint_fields` (pre-existing) and `excluded_hint_fields` (restored) raises — net DRY-positive vs the pre-revert state.

### Public-surface check

`git diff HEAD -- django_strawberry_framework/__init__.py` returns empty — no public surface changes from pass 2 (consistent with a pure revert that subtracts logic from a non-exported validator).
...
- **Revert is exactly the four hunks Worker 1 named, no more, no less.** `git diff HEAD -- django_strawberry_framework/types/base.py tests/types/test_base.py` reports exactly four `@@` hunk headers: one in `types/base.py` (the `excluded_hint_fields` branch of `_validate_optimizer_hints` restored to the shared `_format_unknown_fields_error(...)` call) plus three in `tests/types/test_base.py` (the three `match="not selected relations"` strings at lines 353 / 363 / 381 restored to `match="optimizer_hints names unknown fields"`). The diff is rooted against commit `b8fbd74` (the maintainer's mid-build "Slices 1-3" commit), so the working-tree state after pass 2 IS the pre-Slice-3 baseline wording for these four hunks — verified directly by inspecting `django_strawberry_framework/types/base.py #"excluded_hint_fields"` (lines 740-749 use the shared helper) and `tests/types/test_base.py #"optimizer_hints names unknown fields"` (lines 353, 363, 381 carry the restored match).
- **Slice 3 intended work is intact across both files.** In `django_strawberry_framework/types/base.py`: `DEFERRED_META_KEYS` carries only `{"aggregate_class", "fields_class", "search_fields"}` (no `"orderset_class"` — line 48-54); `ALLOWED_META_KEYS` carries `"orderset_class"` alphabetized between `"optimizer_hints"` and `"primary"` (lines 56-69); `_validate_orderset_class` exists at line 98 with the load-bearing local in-function `from ..orders.sets import OrderSet` import at line 116; `_ValidatedMeta.orderset_class: type | None` slot at line 606; `_validate_meta` threading at line 682; `DjangoType.__init_subclass__` `DjangoTypeDefinition(... orderset_class=validated.orderset_class, ...)` constructor argument at line 321. In `tests/types/test_base.py`: `test_meta_orderset_class_is_promoted_to_allowed_meta_keys` exists at line 223 with the two-line assertion (`"orderset_class" in ALLOWED_META_KEYS` AND `"orderset_class" not in DEFERRED_META_KEYS`); the parametrize list of `test_meta_rejects_each_deferred_key` (line 207) no longer includes `"orderset_class"` per the Decision-7 promotion.
- **`_validate_optimizer_hints` two-branch wording is symmetric again.** Both the `unknown_hint_fields` and `excluded_hint_fields` raises route through `_format_unknown_fields_error(model=..., attr="optimizer_hints", unknown=..., available=...)` (lines 730-739 + 740-749). The shared-helper call removes the wording divergence the pass-1 diff introduced; the message wording trade-off the maintainer may carry forward as a follow-up does not block the slice.
- **Tests pass cleanly.** `uv run pytest tests/orders/ tests/types/ --no-cov` -> 364 passed, 2 skipped, 1 warning (the pre-existing `tests/types/test_generic_foreign_key.py` extension-deprecation warning, unrelated to this revert or to Slice 3). Zero failures, zero errors.
- **No new findings introduced by the revert.** The revert is purely subtractive (removes the four pass-1 wording changes); no new logic, no new tests, no new helpers. The Slice 3 contract observable in the diff is unchanged from pass 1's `review-accepted` state minus the four Escalation-2 hunks.

### Temp test verification

No temp tests created during pass-2 review; the four-hunk diff inspection plus the focused `uv run pytest tests/orders/ tests/types/ --no-cov` run was sufficient to verify the revert. Nothing promoted, nothing deferred.

### Notes for Worker 1 (spec reconciliation)

- No new escalations. Pass 2 closes Escalation 2 (the out-of-scope `_validate_optimizer_hints` wording change) per Worker 1's disposition; Escalation 1 (spec sub-bullet 3 wording) was already resolved by the two pass-1 spec edits to `docs/spec-028-orders-0_0_8.md`; Escalation 3 (dropped planned `test_phase_2_5_partial_failure_leaves_consistent_namespace`) was already disposed in pass 1 as "accept the silent drop with the reason recorded" under `### Spec changes made (Worker 1 only)`.
- The improved `"not selected relations"` wording reverted by pass 2 belongs in a follow-up the maintainer may carry forward (a one-line commit or a small error-message-clarity spec card). Recorded in Worker 1's pass-1 final-verification block already; no new action required.

### Review outcome

`review-accepted`. The pass-2 revert is exactly the four hunks named in Worker 1's Escalation 2 disposition, no Slice 3 intended work is touched, all spec-required behaviour is reflected in the diff and pinned by permanent tests, focused tests pass cleanly. Setting top-level `Status:` to `review-accepted`.

---

## Final verification (Worker 1, pass 2)

- Pass-2 revert verification: clean. `git diff HEAD -- django_strawberry_framework/types/base.py tests/types/test_base.py` shows exactly four hunks — one in `django_strawberry_framework/types/base.py` (the `excluded_hint_fields` branch of `_validate_optimizer_hints` restored from the bespoke f-string to the shared `_format_unknown_fields_error(model=model, attr="optimizer_hints", unknown=excluded_hint_fields, available=selected_relation_names)` call at lines 740-749 against the working-tree state) and three in `tests/types/test_base.py` (the three `match=` strings at lines 353 / 363 / 381 restored from `"not selected relations"` to `"optimizer_hints names unknown fields"`). No additional hunks, no scope-creep re-introduced, no Slice 3 intended work touched. The diff exactly matches the revert scope named in pass-1 Escalation 2 disposition.
- Spec slice checklist (re-confirmation): all three verbatim sub-bullets remain `- [x]` from pass-1. Pass-2's revert is scoped to `_validate_optimizer_hints` (an out-of-scope branch unrelated to any Slice 3 sub-bullet); the Slice 3 contract — `definition.py` `orderset_class` slot, `base.py` validator + `DEFERRED_META_KEYS`/`ALLOWED_META_KEYS` flip, `finalizer.py` four-subpass + `registry.clear()` integration — landed in pass-1 and is unchanged by the revert. Verified at line ~376-378 of this artifact; no new tick needed.
- Existing tests still pass: `uv run pytest tests/orders/ tests/types/ tests/filters/ tests/base/ --no-cov` -> 613 passed + 2 skipped + 1 warning (the pre-existing `tests/types/test_generic_foreign_key.py` extension-deprecation warning, unrelated to Slice 3). Zero failures, zero errors. Identical pass count to pass-1's final-verification run, confirming the revert preserves the full Slice 3 contract.
- Spec reconciliation: no new spec edits required. The two pass-1 spec edits to `docs/spec-028-orders-0_0_8.md` (Slice 3 sub-bullet 3 subpass-2 wording + Decision 6 subpass 2 paragraph) remain in place and are unaffected by pass-2's revert; Escalation 3 deferral (dropped planned `test_phase_2_5_partial_failure_leaves_consistent_namespace`) was disposed in pass-1 with the reason recorded; Escalation 2 (the `_validate_optimizer_hints` wording change) is now closed by the pass-2 revert per the disposition's verbatim revert scope.
- Final status: `final-accepted`.

### Summary

Slice 3 ships every spec-named contract end-to-end and the pass-2 revert closes the only outstanding scope-creep finding from pass-1. The Slice 3 diff against HEAD is exactly the planned shape: `Meta.orderset_class` promotion + `_validate_orderset_class` with load-bearing local in-function `from ..orders.sets import OrderSet` import; `DjangoTypeDefinition.orderset_class` slot threaded through `_ValidatedMeta` / `_validate_meta` / `DjangoType.__init_subclass__`; `finalize_django_types` four-subpass `_bind_ordersets` (bind owners → expand fields → orphan-validate → materialize) plus `_bind_orderset_owner` (idempotent + first-bind model-compat + Axis-2 related-target agreement; SKIPS Axis-1 own-PK Relay-identity per Spec Decision 6 second paragraph); two-block `TypeRegistry.clear` integration (namespace clear + helper-ledger clear, both `pass` + `else:`); 33 permanent Slice 3 tests under `tests/orders/test_finalizer.py` / `tests/orders/test_base.py` / `tests/orders/test_inputs.py` / `tests/types/test_base.py`. Tests pass cleanly across all four scoped trees (613 passed + 2 skipped). Setting top-level `Status:` to `final-accepted`.

### Spec changes made (Worker 1 only)

- No new spec edits in pass-2. The two pass-1 final-verification spec edits — `docs/spec-028-orders-0_0_8.md` line 123 (Slice 3 sub-bullet 3 subpass-2 wording) + Decision 6 subpass 2 paragraph — remain in place. The Escalation 3 deferral (planned test `test_phase_2_5_partial_failure_leaves_consistent_namespace` not landed because it is unreachable under the shipped four-subpass order) remains recorded under pass-1's `### Spec changes made (Worker 1 only)`. The Escalation 2 follow-up target (carrying the improved `"not selected relations"` wording forward as a maintainer follow-up commit or a future error-message-clarity spec card) also remains recorded there; the pass-2 revert closes the in-Slice-3 scope only.
