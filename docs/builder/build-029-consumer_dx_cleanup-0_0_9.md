...
- **RESOLVED 2026-06-05 (post-feedback follow-up).** Per the maintainer review (`docs/feedback.md`) and the "fix everything" directive, the kanban fixtures were reordered to seed the `done`-card glossary link **before** the status flip (and one signal test's chained protection assertions isolated in `transaction.atomic()`); the `apps/kanban/signals.py` invariant was kept untouched. Closing the suite also unmasked + closed a 4-line coverage gap in `inspect_django_type.py`. The full gate is now green — `1391 passed, 3 skipped, 0 failed`, 100% coverage. See `docs/builder/bld-final.md` → "Post-feedback follow-up".
