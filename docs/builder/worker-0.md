...
**Scope a version-cut / release slice's dispatch completely.** A slice that bumps the package version or changes the public surface owns more than the version strings: it also owns updating the **public-API export pin** (the `tests/base/test_init.py` `__all__` surface assertion) to match any symbols earlier slices added to `__init__.py.__all__`. That pin test lives in `tests/base/`, which a feature slice's focused test scope typically does not run — so a public-surface change can sit red across several slices until the version-cut slice (or the final gate) runs `tests/base/`. When dispatching such a slice, name the export-pin update explicitly in the scope so it is not missed; it is the natural home for the "export pin updated accordingly" definition-of-done item.
...
**Workers 1–3 do not read `worker-0.md`.** This procedure lives here, but the card-wrap / DB-backed glossary work is executed by the slice cycle (Worker 1 plans it, Worker 2 runs the ORM edits + regenerate). So when a slice's contract includes the card move or a DB-backed `KANBAN.md` / `docs/GLOSSARY.md` update, Worker 0 must **embed the relevant steps of this procedure (and the DONE-card invariants below) into that slice's planning and build dispatch prompts** — the workers cannot follow a procedure they are not allowed to read. Pre-verify the live DB references named below and pass the verified findings (current card status, whether the `SpecDoc` already exists, which glossary anchors already exist) into the dispatch so the workers do not re-derive them.
...
2. **Create the `SpecDoc`:** `SpecDoc.objects.create(card=card, name="spec-<NNN>-<topic>-<ver>", url="https://github.com/riodw/django-strawberry-framework/blob/main/docs/spec-<NNN>-<topic>-<ver>.md")` (name is unique; the `url` must contain the repo `docs/…` path the build/`import_spec_terms` parse). If a `SpecDoc` row already exists for the card, **update** its `url`/`name` instead of `.create()` (`name` is unique, so a create collides).
...
6. **Fix card-body content the spec wrap names** (e.g. stale `docs/spec-0NN-…` filename refs, `## [0.0.X]` → `[Unreleased]`) by editing `CardItem.text`, and mark every **shipped** `definition_of_done` `CardItem.is_complete = True` (done-card convention — see existing DONE cards). When the spec **defers** DoD items, leave those `is_complete = False` with a follow-up-card marker — the spec's deferral overrides the "mark every DoD" convention. Keep this to what the spec authorizes; leave unrelated card-body prose alone.
