...
2. **Create the `SpecDoc`:** `SpecDoc.objects.create(card=card, name="spec-<NNN>-<topic>-<ver>", path="docs/spec-<NNN>-<topic>-<ver>.md")` — **`path` is the writable column and holds the repo-relative path**; `SpecDoc.url` is a read-only `@property` deriving `f"{SPEC_URL_PREFIX}/{self.path}"`, so passing or assigning `url=` raises. Both KANBAN exporters and `import_spec_terms` read `path` directly. If a `SpecDoc` row already exists for the card, **update** its `path`/`name`; `name` is unique, so `.create()` collides.
