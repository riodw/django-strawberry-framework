...
The standard the answers are held to is `docs/dry/DRY.md` "Design principles": one authoritative definition per rule, a closed declarative contract for a family, one owned translation, a stated lifetime for every stored derivation, and an owner chosen by responsibility. Read that section before planning; do not restate it here.
