...
Relevant anchors:
`docs/spec-021-filters-0_0_8.md #"branch_public(is_private=False"` and
`examples/fakeshop/apps/library/models.py::Branch`.

### M2. The `related_target_for` reverse-relation test names the wrong relation

The test plan says `related_target_for("loan_set")` should resolve from `Loan.book`, but
`examples/fakeshop/apps/library/models.py::Loan` declares
`book = ForeignKey(Book, related_name="loans", ...)`. The reverse relation exposed by that model is
`loans`, not `loan_set`.

Fix the spec to test `related_target_for("loans")`. If default reverse-name behavior also needs
coverage, add a separate fixture/model without `related_name`.

Relevant anchors:
`docs/spec-021-filters-0_0_8.md #"related_target_for(\"loan_set\")"` and
`examples/fakeshop/apps/library/models.py::Loan`.

### M3. `DjangoListField` is described as a 0.0.8 consumer path, but it cannot currently expose filter arguments

The risk section says 0.0.8 consumers using `DjangoListField` or plain `@strawberry.field` resolvers
can reach for `filter_input_type(...)`.

Current `django_strawberry_framework/list_field.py::DjangoListField` wraps resolvers with a
two-argument `(root, info)` callable and does not preserve or expose arbitrary resolver arguments.
That means `filter_input_type(...)` is usable through explicit `@strawberry.field` resolvers, but
not through `DjangoListField` without an argument-injection design.

Fix the spec wording to limit 0.0.8 consumer usage to plain resolvers, or add a concrete
`DjangoListField` filter-argument design. If list-field filtering is not intended until
`DjangoConnectionField`, make that boundary explicit.

Relevant anchors:
`docs/spec-021-filters-0_0_8.md #"0.0.8 consumers using DjangoListField"` and
`django_strawberry_framework/list_field.py::DjangoListField`.

### M4. The registry-clear snippet does not match the current registry implementation
...
def clear(self):
    self._reset_state()
    from django_strawberry_framework.filters.inputs import clear_filter_input_namespace

    clear_filter_input_namespace()
```

Current `django_strawberry_framework/registry.py::TypeRegistry.clear` clears its dictionaries inline;
there is no `_reset_state()` helper. The snippet also omits clearing `_helper_referenced_filtersets`,
although Slice 3's Definition of Done correctly requires both caches to clear.

Fix the snippet to patch into the existing `clear()` body after the registry dictionaries are
cleared, and show both `clear_filter_input_namespace()` and `_helper_referenced_filtersets.clear()`
with local imports.

Relevant anchors:
`docs/spec-021-filters-0_0_8.md #"self._reset_state()"` and
`docs/spec-021-filters-0_0_8.md #"registry.clear() invokes clear_filter_input_namespace() AND clears _helper_referenced_filtersets"`.

### M5. Flat related field paths need an explicit source-path mapping

The spec pins `Meta.fields = {"galaxy__name": ["exact"]}` as a flat GraphQL field named
`galaxyName`, and later says the factory and normalizer need consistent lookup-name mapping.
However, it does not clearly specify where the generated input field stores the source Django path
needed to normalize `galaxyName` back to `galaxy__name`.

Fix the spec to require an internal field spec that carries at least the Python attribute name,
GraphQL name, and Django source path. The runtime normalizer should use that mapping rather than
reconstructing the source path from the public GraphQL/Python field name.

Relevant anchors:
`docs/spec-021-filters-0_0_8.md #"renders as a flat field named galaxyName"` and
`docs/spec-021-filters-0_0_8.md #"Both the factory (_build_input_fields) and the runtime input-data normalizer"`.

## Low priority

### L1. The current-state section is stale after the TODO-skeleton pass

The spec still says `django_strawberry_framework/filters/` does not exist and that there is no
production code on disk yet. That was accurate before the TODO anchors were added, but the package
now has non-public filter skeleton modules with TODO-only implementation anchors.

Fix the current-state/status language to distinguish "TODO skeleton present, no public filter
behavior shipped" from "not implemented at all".

Relevant anchors:
`docs/spec-021-filters-0_0_8.md #"django_strawberry_framework/filters/: does not exist"` and
`docs/spec-021-filters-0_0_8.md #"no production code on disk yet"`.
