...
docstring is the manual. "Shared" = the shared tree; `$WS` = a workspace copy ("Workspace").
...
| [count_queries.py][count-queries] | Performance roles | `$WS` | queries per cardinality + verdict ("Performance") |
| [bench_plan_cache.py][bench-plan-cache] | Worker-0, Performance roles | `$WS` | plan cache warm vs cleared ("Bench baseline") |
| [bench_optimizer_walk.py][bench-optimizer-walk] | Worker-0, Performance roles | `$WS` | the walk alone ("Bench baseline") |
| [bench_nested_fetch.py][bench-nested-fetch] | Worker-0 on Postgres; others `--sqlite-smoke` | `$WS` | nested-connection strategies ("Database cells") |
| [importtime_report.py][importtime-report] | Worker-0, Performance roles | `$WS` | per-module import-time minimum ("Performance") |
| [prove_failability.py][prove-failability] | Worker-2, Performance + Mechanics verifiers | `$WS` only, `--workspace "$WS"` | a gate can fail ("Workspace") |
...
written to scratch, three fresh workspace copies, three fresh Worker-1s dispatched on the same
axes, each from its axis record and its handoffs, Performance and Mechanics at once, Comments
after those two report.
...
plan item, tick, remove item scratch and `$WS` by explicit path, advance (`autonomous`) or report
and wait (`pause-after-each-item`). Worker-0 never regrades a verifier.
