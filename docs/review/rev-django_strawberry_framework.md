...
- **Shared utility candidates already evaluated.** Three per-file and folder artifacts proposed extractions and each carry-forward has a documented trigger condition rather than a fire-and-defer hand-wave: (a) `_walk_selection_tree(on_node=...)` visitor to collapse `optimizer/extension.py:91-127` (`_walk_directives`) and `extension.py:198-226` (`_walk_reachable_fragment_definitions`) — deferred per `rev-optimizer.md` Low 1 with trigger "when a third walker lands"; (b) `RelationPlanCtx` dataclass to collapse the 9-/10-arg planner call sites at `optimizer/walker.py:248-273,427-467` — deferred per `rev-optimizer.md` Low 2 with trigger "when a planner gains an 11th argument OR a third file outside `walker.py` needs one"; (c) `_resolve_origin_for_type_name(strawberry_schema, type_name)` to share between `extension.py:336-363` and `extension.py:389-420` — deferred per `rev-optimizer__extension.md` DRY analysis with trigger "if a third call site lands". The four-extraction bundle from `rev-types.md` M2 (`_collect_consumer_authored_fields`, `_assert_no_relay_id_collision`, `_apply_id_filter_and_materialize_{single,many}`, `_model_for(type_cls)`) is the only carry-forward that touches the `__init_subclass__` → `field_map` → resolver pipeline and is the right shape to bundle when the next `types/base.py` cycle or Relay-resolver behavior change happens. No new project-scope extraction is justified today; the carry-forwards are correctly held.
