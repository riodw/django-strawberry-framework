...
- Folder-scope deferred candidates (rationale in Low §1 and Low §2 below): (1) `_walk_selection_tree(on_node=...)` visitor to collapse `extension.py:_walk_directives` (`extension.py:91-127`) and `extension.py:_walk_reachable_fragment_definitions` (`extension.py:198-226`) — trigger: a third walker lands anywhere in `optimizer/`. (2) `RelationPlanCtx` dataclass to collapse `walker.py`'s four 9-/10-arg planner call sites — trigger: any planner gains an 11th argument OR a third file outside `walker.py` needs the same call shape. (3) `_resolve_origin_for_type_name(strawberry_schema, type_name)` shared between `extension.py:336-363` and `extension.py:389-420` — trigger: a third call site lands.
