...
- **No cross-file literal duplication.** Per the seven shadow overviews under `docs/review/shadow/django_strawberry_framework__optimizer*.overview.md`, every repeated string literal is intra-file and already documented (per-file artifacts pinned each). Cross-file dedupe is clean — no folder-level DRY consolidation is needed.
