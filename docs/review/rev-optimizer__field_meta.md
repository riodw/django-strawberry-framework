...
- **New helpers a fix might justify.** The hard-coded `{"many", "reverse_many_to_one"}` membership set at `optimizer/field_meta.py:116` should call the existing `is_many_side_relation_kind` helper at `utils/relations.py:68-70` (which already wraps the canonical `MANY_SIDE_RELATION_KINDS` frozenset at `utils/relations.py:14-19`). No new helper is needed — the existing helper is the right call site. Outside that, `from_django_field` is the right home for the "Django field descriptor → FieldMeta" responsibility; consolidation does not require a new abstraction.
