...
- **New helpers a fix might justify.** A `_walk_relation_target(sel, related_model, plan, prefix, info, runtime_paths)` helper that wraps the two parallel `_walk_selections(sel.selections, django_field.related_model, …)` call sites at `walker.py:302-309` (inside `_plan_select_relation`, prefix=`f"{full_path}__"`, parent plan) and `walker.py:369-376` (inside `_build_prefetch_child_queryset`, prefix=`""`, child plan) would re-center the recursive carry-forward signature in one place — both sites take the same six arguments minus the `prefix` / `plan` axis. Not justified today; the parameter axes that differ (prefix shape, plan identity) are exactly what would have to be re-threaded through the new helper, so the helper would not collapse meaningful surface. Re-evaluate if a third nested-walk call site lands.
- **Duplication risk in the current file.** `_apply_hint`'s `force_select` branch (`walker.py:427-453`) and `force_prefetch` branch (`walker.py:455-467`) re-pass an identical 9-arg sequence (`sel, django_field, …, plan, prefix, full_path, info, runtime_paths, resolver_identities`) to `_plan_prefetch_relation` and `_plan_select_relation`, mirroring the same 9-arg sequence already passed at the default dispatch (`walker.py:248-273`). The `parent_type=type_cls` argument in the `_plan_select_relation` branches (`walker.py:265,445`) adds the 10th slot. The repetition is shaped by the long positional argument lists on the two relation-planner functions rather than by a true behavioural duplicate, so collapsing it would require converting those planners to take a `RelationPlanCtx` dataclass — a larger refactor than the duplication justifies today, but the surface should be re-examined at the folder pass if any planner gains an 11th argument. `_target_has_custom_get_queryset(target_type)` is called twice per prefetch dispatch (`walker.py:247` inside `plan_relation`, then again at `walker.py:325` inside `_plan_prefetch_relation`); the precomputed-result pattern at `walker.py:339,365,382` shows the explicit-flag-threading idiom but is not extended back across the public `plan_relation` boundary. Low cost; flagged for the next pass that touches `plan_relation`'s signature.
