...
- **New helpers a fix might justify.** None at the file level — `_parse_bigint` and `_serialize_bigint` are already the right single-responsibility shape (one input gate, one output gate). The cross-module `warnings.catch_warnings()` block at `scalars.py:91-102` is the only deprecation-suppression site in the package; a shared `_with_strawberry_compat(...)` helper is not justified until a second scalar (e.g., the `Upload` TODO-ALPHA-027 scalar named in the module docstring) lands in this same module — at that point both definitions belong inside one shared suppression context rather than two parallel `with warnings.catch_warnings():` blocks. Carry forward to the next scalar-add.
