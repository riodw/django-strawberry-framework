...
- **No cross-file literal duplication.** Per the eight shadow overviews under `docs/review/shadow/django_strawberry_framework__types*.overview.md`, every repeated string literal is intra-file and already documented in its per-file artifact (`base.py`: `optimizer_hints` / `description` / `interfaces`; `relay.py`: `__func__`). Cross-file dedupe is clean — no folder-level DRY consolidation is needed.
