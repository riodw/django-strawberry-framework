...
- Folder-scope extraction bundle (rationale and trigger conditions detailed in Medium §M2 below): (1) `_collect_consumer_authored_fields(cls, fields) -> tuple[frozenset[str], dict[str, frozenset[str]]]` and `_assert_no_relay_id_collision(cls, relay_shaped)` from `rev-types__base.md` M3; (2) `_apply_id_filter_and_materialize_single` / `_apply_id_filter_and_materialize_many` from `rev-types__relay.md` M1; (3) `_model_for(type_cls) -> type[models.Model]` from `rev-types__relay.md` L5 (collapses `cls.__django_strawberry_definition__.model` reads at `relay.py:78,134,297`). Evaluated together because they touch the `__init_subclass__` → `field_map` → resolver pipeline; bundle when the next `types/base.py` cycle or Relay-resolver behavior change happens.
