...
- Defer until a third postgres-soft-import lands (no current spec home): generalize the `_resolve_array_field` / `_resolve_hstore_field` pair through a single `_resolve_postgres_field("ArrayField") | None` helper. Two call sites with explicit names today give test-side `monkeypatch.setattr(converters, "_ARRAY_FIELD_CLS", ...)` a stable name to bind, so the explicit pair is correct at N=2.
