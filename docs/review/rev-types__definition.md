...
- None — `types/definition.py` is a 46-line pure-dataclass record with a single construction site at `types/base.py:222-240`; the deferred-surface drift between dataclass slots and the construction site is tracked as the Medium finding (M1 above), and the frozen-vs-mutable question is forwarded to the project-pass.
...
```django_strawberry_framework/types/definition.py:34:41
    # TODO(deferred specs; see docs/GLOSSARY.md): tighten ``Any | None`` to the
...
- The TODO anchor at definition.py:34-36 is correctly formatted per AGENTS.md: it names the active design references (docs/GLOSSARY.md), enumerates the specs whose ship would tighten the types, and explicitly says "update or remove this anchor in the same change that lands each slice."
