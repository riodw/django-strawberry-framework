...
- Thread `definition.field_map[snake_case(pending.field_name)]` through to `resolved_relation_annotation` at `types/finalizer.py:130-133` — `resolved_relation_annotation` accepts a `field_meta` parameter but the finalizer passes `None`, so `FieldMeta.from_django_field(field)` re-runs per pending record despite the pre-computed `FieldMeta` already living on `DjangoTypeDefinition.field_map`. Saves a redundant FieldMeta build per pending relation.
