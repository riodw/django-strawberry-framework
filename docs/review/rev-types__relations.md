...
- **New helpers a fix might justify.** None justified inside this 48-line module. The dataclass is intentionally minimal — every field is read from at least one consumer site, and the `_PendingRelationAnnotationMeta` indirection is a single-purpose metaclass whose only job is to shape the schema-construction `TypeError` repr. The only adjacent helper candidate is at the producer site: `_build_annotations` (`types/base.py:810-820`) inlines all seven keyword args verbatim; a `PendingRelation.from_field(cls, source_model, field, field_meta)` classmethod could absorb the construction site and shrink the producer to one call. Single call site today, so defer; revisit only if a second producer lands (e.g. the deferred lazy/forward-reference path mentioned in `types/finalizer.py:179-184`).
