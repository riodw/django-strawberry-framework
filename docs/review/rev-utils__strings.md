...
- **Duplication risk in the current file.** None. Static helper's repeated-literal report at `docs/review/shadow/django_strawberry_framework__utils__strings.overview.md:39-41` returned "None." Both functions are ~5 lines and share no copy-pasted branch logic; they are inverse-direction siblings only at the *concept* level (snake↔pascal), not at the implementation level.
...
- Zero imports, zero side effects, zero ORM markers (the helper's `Django / ORM markers` table at `docs/review/shadow/django_strawberry_framework__utils__strings.overview.md:22-24` only matches the literal word "only" inside the docstring at `:26`, a false positive). Pure-function module is safe to import at any phase.
