...
Dispatch mode: serial — one item at a time, Worker 1 then Worker 2, no parallel items. Prior sessions
completed the 22 top-level `django_strawberry_framework/*.py` items and the full `auth/` package; this
continuation covers the full `django_strawberry_framework/extensions/` package (three files plus its folder
integration pass). Worker 0 reports after that package boundary. Blockers escalate to the maintainer immediately.
Cycle baseline: `HEAD` = `947f7494`. The tracked tree was already dirty at plan creation (see below), so per-item
baselines are captured at dispatch via `git stash create` and workers scope their diff to the target's own paths.

Concurrent work present at plan creation, out of scope, preserved untouched: modified `KANBAN.md`,
`KANBAN.html`, `docs/SPECS/spec-006-public_surface-0_0_3.md`,
`docs/SPECS/spec-007-onboarding_docs_spec_consolidation-0_0_4.md`, `examples/fakeshop/db.sqlite3`;
untracked `docs/SPECS/appx/spec-006-*-rationale.md`, `docs/SPECS/appx/spec-007-*-rationale.md`,
`docs/builder/bld-006-r1-rationale_move.md`, `docs/builder/bld-007-r1-rationale_move.md`,
`docs/builder/build-006-public_surface-0_0_3.md`,
`docs/builder/build-007-onboarding_docs_spec_consolidation-0_0_4.md`; leftover generated
`docs/shadow/django_strawberry_framework____init__.*` and `docs/shadow/django_strawberry_framework__conf.*`.

Prior-cycle artifacts: the five `rev-*.md` files completed under `review-0_0_10.md`/`review-0_0_11.md`/
`review-0_0_13.md` (`rev-_cross_web_patches.md`, `rev-_django_patches.md`, `rev-_strawberry_patches.md`,
`rev-apps.md`, `rev-conf.md`) are deleted in the working tree by concurrent maintainer work. This cycle does not
restore them and does not adopt them: 0.0.14 is a new release plan, every item below starts unchecked, and a
worker assigned one of those targets writes a fresh artifact at that path.

New since the `0.0.13` plan (created at `b27b3c5a`, before the 0.0.14 transport, policy, and hardening cards):
top-level `_boundary_ordering.py`, `_request_body.py`, `consumers.py`, `error_policy.py`, `resource_policy.py`,
`schema.py`, `views.py`; `auth/sessions.py`; `extensions/error_policy.py`, `extensions/resource_policy.py`;
`middleware/request_body.py`; `optimizer/nested_planner.py`, `optimizer/predicates.py`,
`optimizer/single_parent_fetch.py`; `rest_framework/hook_context.py`; `utils/context.py`, `utils/sessions.py`,
...
- `django_strawberry_framework/`
  ...
  - `django_strawberry_framework/middleware/`
    ...
    - [ ] `django_strawberry_framework/middleware/request_body.py` -> `docs/review/rev-middleware__request_body.md`
  ...
  - `django_strawberry_framework/rest_framework/`
    - [ ] `django_strawberry_framework/rest_framework/hook_context.py` -> `docs/review/rev-rest_framework__hook_context.md`
  ...
  - `django_strawberry_framework/utils/`
    ...
    - [ ] `django_strawberry_framework/utils/context.py` -> `docs/review/rev-utils__context.md`
    ...
    - [ ] `django_strawberry_framework/utils/sessions.py` -> `docs/review/rev-utils__sessions.md`
    ...
    - [ ] `django_strawberry_framework/utils/write_transaction.py` -> `docs/review/rev-utils__write_transaction.md`
