...
Release version: `0.0.7` (matches `pyproject.toml` and `django_strawberry_framework/__init__.py`)
...
- Every `__init__.py` is out of scope as a standalone artifact. Subpackage `__init__.py` files are reviewed inside their folder pass; the top-level `django_strawberry_framework/__init__.py` is reviewed inside the project pass.
- Non-`.py` files (e.g. `py.typed`) are out of scope entirely; their behavior is governed by packaging configuration.

## New since 0.0.6

The following files / subpackages are new in this release and have no prior `rev-*.md`:

- `django_strawberry_framework/_django_patches.py` — package-level Trac #37064 hardening (AppConfig.ready() patch).
- `django_strawberry_framework/apps.py` — `DjangoStrawberryFrameworkConfig`.
- `django_strawberry_framework/list_field.py` — `DjangoListField` factory.
- `django_strawberry_framework/management/` (+ `management/commands/export_schema.py`) — `manage.py export_schema` command.
- `django_strawberry_framework/test/` (+ `test/_wrap.py`) — `safe_wrap_connection_method` consumer helper.

Worker 1 should not assume reviewer familiarity with these files; treat each as a first-pass review and confirm the shadow overview was generated before reading.
...
- `django_strawberry_framework/`
  ...
  - `django_strawberry_framework/test/`
    - [x] `django_strawberry_framework/test/_wrap.py` -> `docs/review/rev-test___wrap.md`
    - [x] folder pass: `django_strawberry_framework/test/` -> `docs/review/rev-test.md`
