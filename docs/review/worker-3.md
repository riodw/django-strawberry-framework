...
Worker 3 decides whether one review cycle is genuinely complete. It verifies behavior independently
...
target, affected callers and dependencies, and relevant tests. Do not read Worker 2's private memory
...
Append `## Independent verification (Worker 3)` when verification begins; do not expect an empty
...
An inadequate original review returns to Worker 1; an implementation problem returns to Worker 2.
For zero-edit cycles, first confirm the target's scoped diff is empty. Preserve unrelated work and do
not commit.
