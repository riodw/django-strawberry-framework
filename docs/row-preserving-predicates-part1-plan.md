...
[spec 054][spec-054]** (its "Slice 0"): the shipped-defect fixes and
search-independent machinery land ahead of card `TODO-BETA-054-0.1.2`, and
...
undocumented between Part 1 landing and card 054 wrapping. Version and
...
`EXISTS` distributes over OR for same-value disjunctions (card 054's future
...
2. **`optimizer/predicates.py`** — correlation, alias allocation, and
   ...
   nothing about django-filter or Strawberry. Card 054 later builds its
...
- `classify_path(model, field_path)` classifies a **model-field path only**
  (the shape `FilterSet.filter_for_field` and card 054 declarations
...
- `attach_exists(queryset, inner_queryset)` — wraps the caller-built inner
  ...
  Slice C; card 054's positive OR never needs it either).
...
consumed by this plan and later by card 054, built on existing library
...
(b) **card 054 integration test** — the live `/graphql/` `search:`
...
Owned by card 054's completion bookkeeping (see Identity above), but the
...
- `docs/GLOSSARY.md` / `KANBAN.md` updates flow through card 054's DB
...
- Everything card-054-shaped: no `search:` argument, no `Meta.search_fields`
  handling, no pipeline step, no same-value OR grouping, no
  `Meta.search_strategy`, no benchmark harness. Release-state artifacts
  beyond Slice D's source-of-truth updates stay with card 054.
...
- **Low-selectivity regime.** Many independent `EXISTS` branches over a
  ...
  No escape hatch here (card 054 holds `Meta.search_strategy` in reserve);
