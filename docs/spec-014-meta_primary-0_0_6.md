...
Status: draft (revision 5, post-feedback-4 review).
...
- [ ] Slice 4: Consumer-site updates (relation conversion + optimizer)
  ...
  - [ ] **Audit ALL `_resolve_field_map(model)` call sites (M2 fix).** Current `optimizer/walker.py` has **two** callers: `_walk_selections:125` (the obvious root path from `plan_optimizations`) and `_selected_scalar_names:474` (a sibling helper that also resolves the field map). Worker 1's planning pass MUST inspect both call sites and determine which are reachable from the root planning path (and therefore need `source_type` threading from `plan_optimizations`) versus which are pure nested-recursion helpers (and therefore correctly leave `source_type=None`). Default expectation: any caller whose model argument originates from the resolver's return-type-to-model conversion in `optimizer/extension.py` is a root caller and needs the keyword. The planning artifact records the per-call-site decision before Worker 2 implements.
...
`optimizer/walker.py` currently has two `_resolve_field_map(model)` call sites — `_walk_selections:125` and `_selected_scalar_names:474`. Worker 1 audits both during planning and pins which sites are root-path callers needing `source_type` threading (M2 fix; see Slice 4).
...
`origin` is `None` for nested-prefetch planning (where the model lookup uses the primary anyway), and is the resolver's actual Strawberry return type for root-path planning. Worker 1 confirms the exact tuple position during planning; the contract is "two resolvers for the same model with different Strawberry return types must not share a cached plan."
