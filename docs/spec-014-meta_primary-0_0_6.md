...
Status: draft (revision 4, post-feedback-3 review).
...
- [ ] Slice 4: Consumer-site updates (relation conversion + optimizer)
  ...
  - [ ] **`types/finalizer.py:68`** pending-relation resolution. Still calls `registry.get(pending.related_model)`. Per Slice 3, the ambiguity audit runs first; by the time this line executes, `get()` returns the primary (or the single registered type) or `None` for "no type registered at all" (the unchanged "unresolved target" case). No code change required; behavior follows from Slice 1's `get()` and Slice 3's audit.
...
Consumers calling `registry.get(model)` today (in `types/base.py:620`, `types/converters.py:343`, `types/finalizer.py:68`, `optimizer/walker.py:80`, `optimizer/walker.py:191`, `optimizer/extension.py:621`) all assume the lookup is deterministic. With one type per model, it is.
