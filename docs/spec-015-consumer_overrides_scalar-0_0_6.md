...
- [ ] Slice 1: Track annotation-only scalar overrides on `DjangoTypeDefinition`
  ...
  - [ ] **Relay `id` collision guard (H1 fix).** After the `consumer_annotated_scalar_fields` / `consumer_assigned_scalar_fields` collections are built but before `_build_annotations` is invoked, detect: (a) `interfaces` (from `_validate_meta`) includes `relay.Node` — checked the same way `_build_annotations` does it at `types/base.py:609`, via `any(issubclass(i, relay.Node) for i in interfaces) or issubclass(cls, relay.Node)`; AND (b) the model's pk field name appears in `consumer_annotated_scalar_fields | consumer_assigned_scalar_fields`. When both conditions hold, raise `ConfigurationError` with message: `"<ClassName>: cannot override Meta.model._meta.pk.name on a relay.Node-shaped type without using strawberry.relay.NodeID[...]. The Relay interface supplies id: GlobalID! — declare the id field via relay.NodeID[<pk_type>] if you need a different id shape, or remove relay.Node from Meta.interfaces."`. Fires at type-creation time so the consumer sees a `DjangoType`-shaped error instead of the downstream Strawberry `ValueError` from `Node.id`-vs-`Int!` mismatch. (See [Decision 7](#decision-7--relay-id-override-collision-h1-fix).)
  - [ ] Tests in `tests/types/test_definition_order.py` (the existing override-contract host, where the three relation-override tests at `:179`, `:206`, `:235` live, plus the `:278` `test_assigned_scalar_field_override_keeps_consumer_resolver` test). The annotation-only scalar contract is the natural fourth sibling; placement matches the existing relation/scalar/annotation/assigned 2×2 matrix:
    ...
    - [ ] **Converter-bypass regressions (H2 fix; three new tests).** The new short-circuit skips `convert_scalar(...)` for the overridden field, which means every converter-side validation and side effect is bypassed for that field. The bypass is the intended consumer-authoritative contract (see [Decision 7a](#decision-7a--converter-validation-bypass-h2-fix)), but the spec needs explicit tests that pin it so future readers understand the surface and so converter changes do not silently re-introduce validation against an overridden field:
      - [ ] `test_annotation_override_of_unsupported_scalar_field_type_is_allowed`: declare a `DjangoType` selecting a Django field whose type is NOT in `SCALAR_MAP` and has no subclass MRO match (e.g., a synthetic `models.Field`-subclass fixture, or a real field type the converter rejects). Without the override, `convert_scalar` would raise `ConfigurationError`. With a consumer `myfield: bytes` (or similar) annotation, assert no error is raised and `definition.consumer_annotated_scalar_fields` contains the field name. The consumer's override is the recourse for unsupported scalars; `Meta.exclude` is still the recourse for "drop the field entirely".
      ...
      - [ ] `test_annotation_override_of_arrayfield_with_nested_array_is_allowed`: declare a `DjangoType` selecting an `ArrayField(base_field=ArrayField(...))` (nested array — converter currently rejects). With a consumer `tags: list[list[str]]` annotation, assert no error is raised and the type is finalizable. Worker 1 verifies that the existing `ArrayField` rejection test in `tests/types/test_converters.py` still passes (i.e., un-overridden nested arrays still raise).
  - [ ] **Relay collision test (H1 fix).** New test in `tests/types/test_definition_order.py` alongside the four-corner cluster:
...
- [ ] Slice 5: Docs, KANBAN, CHANGELOG, archive (separate commit; may follow Slice 4 by any interval).
  ...
  - [ ] `KANBAN.md` — move `WIP-ALPHA-015-0.0.6` → `DONE-015-0.0.6`. **Drop in the verbatim body below:**
    ...
    - **`relay.Node` pk collision rejected at type-creation time.** A consumer
      ...
      `strawberry.Schema(...)` construction, which obscures the source.
  ...
  - [ ] `CHANGELOG.md` — `[Unreleased]` entries (**permission granted by this spec**, overriding [`AGENTS.md`](../AGENTS.md)'s default prohibition):
    ...
    - `Added`: `ConfigurationError` raised at `DjangoType.__init_subclass__` time when a consumer overrides the pk of a `Meta.interfaces = (relay.Node,)`-shaped type (annotation or assigned). Points at `strawberry.relay.NodeID[<pk_type>]` as the supported escape hatch. Replaces the downstream Strawberry-side `ValueError` ("Interface field Node.id expects type ID! but ...") that surfaced only at `strawberry.Schema(...)` construction.
...
**Contract.** This card adds a package-owned `ConfigurationError` raised at `DjangoType.__init_subclass__` time when the consumer's annotation/assignment shadows the pk of a Relay-Node-shaped type:
...
suppress_pk_annotation = (
    any(issubclass(i, relay.Node) for i in interfaces)
    or issubclass(cls, relay.Node)
)
if suppress_pk_annotation:
    pk_name = meta.model._meta.pk.name
    if pk_name in consumer_annotated_scalar_fields or pk_name in consumer_assigned_scalar_fields:
        raise ConfigurationError(
            f"{cls.__name__}: cannot override Meta.model._meta.pk.name on a "
            "relay.Node-shaped type without using strawberry.relay.NodeID[...]. "
            "The Relay interface supplies id: GlobalID! — declare the id field "
            "via relay.NodeID[<pk_type>] if you need a different id shape, "
...
**Why `relay.NodeID[...]` is the supported escape hatch.** Strawberry's relay module documents `relay.NodeID[<pk_type>]` as the way to declare a custom id shape on a Relay-Node-shaped type. The consumer escape hatch already exists upstream; this card pins it as the recommended path in the error message.
...
6. **Choice-enum registration.** Successful `convert_choices_to_enum` calls register the generated enum into `registry._enums[(model, field_name)]` so two `DjangoType`s reading the same choice column share one cached enum (the existing `Choice enum generation` contract from `0.0.1`).
...
- **`relay.Node` `id` collision.** Pinned as a behavior contract, not a flagged edge case — see [Decision 7](#decision-7--relay-id-override-collision-h1-fix). A consumer who writes `id: int` (or assigns `id = strawberry.field(...)`) on a Relay-Node-shaped type raises `ConfigurationError` at `__init_subclass__` time with a message pointing at `relay.NodeID[...]` as the supported escape hatch. The Slice 1 `test_consumer_id_annotation_on_relay_node_type_raises` test pins the early-raise contract.
...
The Slice 1 test cluster has eight tests total:
...
**One Relay-collision test** (H1 fix) — pins [Decision 7](#decision-7--relay-id-override-collision-h1-fix):

- `test_consumer_id_annotation_on_relay_node_type_raises` — `ConfigurationError` at class-creation time with message pointing at `relay.NodeID[...]`.
...
- [ ] All eight Slice 1 tests in `tests/types/test_definition_order.py` (four core overrides + three converter-bypass + one Relay collision) pass.
