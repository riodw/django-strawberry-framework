...
Status: draft (revision 4, post-rev3 review).
...
- [ ] Slice 1: Track annotation-only scalar overrides on `DjangoTypeDefinition`
  ...
  - [ ] **Relay collision tests (H1 fix; five new tests, rev4 expanded — added string-annotation accept and assigned-id reject).** New tests in `tests/types/test_definition_order.py` alongside the four-corner cluster:
...
def _id_annotation_is_relay_node_id(cls: type) -> bool:
    ...
    reference at type-creation time), return True. Accept conservatively
    rather than raise spuriously — if the consumer's intent was a NodeID
    annotation but the forward reference does not resolve yet, the
    Strawberry-side check at schema construction will still catch any
    real mismatch.
    """
    try:
...
**Fail-soft on unresolved forward references.** `typing.get_type_hints` raises `NameError` (or `AttributeError`, for nested attribute references) when a name in the annotation cannot be resolved in `cls`'s module globals at the time `__init_subclass__` runs. That can legitimately happen — a consumer may declare `id: "MyForwardRef"` where `MyForwardRef` is imported later. The detection helper accepts conservatively in that case (returns `True` — "is a valid NodeID annotation") rather than raising spuriously. Strawberry's own schema-construction pass will still catch a real interface mismatch downstream.
...
Total expected delta: ~160 lines added across the package and tests; one delete in `tests/types/test_base.py`; one substantial KANBAN body insert in `KANBAN.md`. No new source files. No new test files (the M2-placed `ArrayField` bypass test extends an existing test module if Worker 1 picks placement b).
...
- **`relay.Node` `id` collision.** Pinned as a behavior contract, not a flagged edge case — see [Decision 7](#decision-7--relay-id-override-collision-h1-fix-rev3-narrowed). A consumer who writes `id: <non-NodeID-type>` annotation OR assigns any `id = <StrawberryField>` on a Relay-Node-shaped type raises `ConfigurationError` at `__init_subclass__` time. The annotation-side error points at `relay.NodeID[...]` as the supported escape hatch; the assigned-side error points at both `relay.NodeID[...]` and `@classmethod resolve_id`. The guard is narrow: `id: relay.NodeID[int]` passes in direct and stringified forms (rev4 H1: detection uses `typing.get_type_hints(cls, include_extras=True)`), and non-`id` consumer scalar overrides on Relay-Node-shaped types pass (no `Node.id` collision). The five Slice 1 tests pin the reject + accept paths: `test_consumer_id_annotation_on_relay_node_type_raises`, `test_consumer_id_assigned_strawberry_field_on_relay_node_type_raises`, `test_consumer_id_relay_nodeid_annotation_on_relay_node_type_is_accepted`, `test_consumer_id_string_relay_nodeid_annotation_on_relay_node_type_is_accepted`, and `test_consumer_non_id_scalar_override_on_relay_node_type_is_accepted`.
...
The Slice 1 test cluster has 12 tests total (rev4 expanded from 10):
...
**Five Relay-collision tests** (H1 fix, rev4 expanded — string-annotation accept + assigned-id reject added) — pin [Decision 7](#decision-7--relay-id-override-collision-h1-fix-rev3-narrowed):
...
- `test_consumer_id_string_relay_nodeid_annotation_on_relay_node_type_is_accepted` — `id: "relay.NodeID[int]"` stringified form passes the guard (rev4 H1 — pins the `typing.get_type_hints(cls, include_extras=True)` evaluation for PEP 563 / stringified annotations).
...
- [ ] All 12 Slice 1 tests pass (four core overrides + three converter-bypass in `tests/types/test_definition_order.py`, plus five Relay-collision tests — two reject + three accept; the `ArrayField` bypass test may live in `tests/types/test_converters.py` per M2 placement choice).
