...
Status: draft (revision 3, post-rev2 review).
...
- [`ConfigurationError`](FEATURES.md#configurationerror) — raised at type-creation time for unsupported shadow shapes; this card adds one new error site (the Relay collision guard in Slice 1 per [Decision 7](#decision-7--relay-id-override-collision-h1-fix-rev3-narrowed)).
...
- [ ] Slice 1: Track annotation-only scalar overrides on `DjangoTypeDefinition`
  ...
  - [ ] **Relay `id` collision guard (H1 fix, rev3 narrowed).** After the `consumer_annotated_scalar_fields` / `consumer_assigned_scalar_fields` collections are built but before `_build_annotations` is invoked, detect: (a) `interfaces` (from `_validate_meta`) includes `relay.Node` — checked the same way `_build_annotations` does it at `types/base.py:609`, via `any(issubclass(i, relay.Node) for i in interfaces) or issubclass(cls, relay.Node)`; AND (b) the consumer authored an entry for the GraphQL field name `"id"` — either an annotation (`cls.__annotations__.get("id") is not None`) or an assignment (`isinstance(cls.__dict__.get("id"), StrawberryField)`); AND (c) the `id` annotation is NOT a `relay.NodeID[...]`-marked annotation. The detection mechanism for (c) is Worker 1's choice during planning; two viable options: (i) `typing.get_args` inspection of `cls.__annotations__["id"]` looking for the `Annotated[T, NodeIDPrivate]` marker shape that Strawberry's `relay.NodeID` uses internally, or (ii) reuse strawberry's own probe — once Relay defaults are injected (Phase 2.5), `cls.resolve_id_attr()` returns the consumer's `relay.NodeID`-marked attribute or raises `NodeIDAnnotationError`; the H1 guard fires only when `NodeIDAnnotationError` is raised. Option (i) is `__init_subclass__`-time-cheap; option (ii) is finalize-time and runs alongside `_check_composite_pk_for_relay_node` (`types/relay.py:134-148`) which uses the same probe. **Important: the predicate is keyed off the GraphQL field name `"id"`, not the model's pk name.** A model with `code = models.CharField(primary_key=True)` and a consumer `code: str` override does NOT trigger the guard — the GraphQL fields are `id: ID!` (from Relay) and `code: String!` (from the consumer), no collision. When all three conditions hold, raise `ConfigurationError` with message: `"<ClassName>: cannot override the id field on a relay.Node-shaped type without using strawberry.relay.NodeID[...]. The Relay interface supplies id: GlobalID! — declare the id field via relay.NodeID[<pk_type>] if you need a different id shape, or remove relay.Node from Meta.interfaces."`. (See [Decision 7](#decision-7--relay-id-override-collision-h1-fix).)
  - [ ] Tests in `tests/types/test_definition_order.py` (the existing override-contract host, where the three relation-override tests at `:179`, `:206`, `:235` live, plus the `:278` `test_assigned_scalar_field_override_keeps_consumer_resolver` test). The annotation-only scalar contract is the natural fourth sibling; placement matches the existing relation/scalar/annotation/assigned 2×2 matrix:
    ...
    - [ ] **Converter-bypass regressions (H2 fix; three new tests).** The new short-circuit skips `convert_scalar(...)` for the overridden field, which means every converter-side validation and side effect is bypassed for that field. The bypass is the intended consumer-authoritative contract (see [Decision 7a](#decision-7a--converter-validation-bypass-h2-fix)), but the spec needs explicit tests that pin it so future readers understand the surface and so converter changes do not silently re-introduce validation against an overridden field:
      ...
      - [ ] `test_annotation_override_of_arrayfield_with_nested_array_is_allowed` (M2 fix — placement decision): real `django.contrib.postgres.fields.ArrayField` testing requires the `_ARRAY_FIELD_CLS` monkeypatch + `_FakeArrayField` fixture pattern that lives in `tests/types/test_converters.py:449-1100` (every existing `ArrayField` test uses it; the production code at `types/converters.py:91` soft-imports the real class and CI environment-dependence is the failure mode without the monkeypatch). Worker 1 picks during planning between (a) place this single test in `tests/types/test_converters.py` beside the existing `_FakeArrayField` tests so the fixture lookup stays local (smaller-touch; recommended), or (b) place in `tests/types/test_definition_order.py` and either re-import `_FakeArrayField` or duplicate the fixture. Test body: `monkeypatch.setattr(converters, "_ARRAY_FIELD_CLS", _FakeArrayField)`; build a `_FakeArrayField(_FakeArrayField(models.IntegerField()))` instance; declare a `DjangoType` selecting that field with a consumer `tags: list[list[int]]` annotation; assert no error is raised at class-creation time and `finalize_django_types()` succeeds. Verify that the existing `tests/types/test_converters.py:1021` nested-array rejection test still passes (un-overridden nested arrays still raise).
  - [ ] **Relay collision tests (H1 fix; three new tests, rev3 expanded).** New tests in `tests/types/test_definition_order.py` alongside the four-corner cluster:
...
- [ ] Slice 5: Docs, KANBAN, CHANGELOG, archive (separate commit; may follow Slice 4 by any interval).
  ...
  - [ ] `KANBAN.md` — move `WIP-ALPHA-015-0.0.6` → `DONE-015-0.0.6`. **Drop in the verbatim body below:**
    ...
    - **`relay.Node` `id` collision rejected at type-creation time.** A consumer
      who writes `id: int` (or assigns `id = strawberry.field(...)`) on a
      `DjangoType` with `Meta.interfaces = (relay.Node,)` raises
      `ConfigurationError` from `__init_subclass__` with a message pointing
      at `relay.NodeID[<pk_type>]` as the supported escape hatch. Without the
      ...
      `relay.NodeID[...]`-marked annotation. `id: relay.NodeID[int]` (the
      documented escape hatch) and non-`id` consumer scalar overrides
      (e.g., `description: int`, or `code: str` on a model with `code` as
      pk) pass through unchanged.
  ...
  - [ ] `CHANGELOG.md` — `[Unreleased]` entries (**permission granted by this spec**, overriding [`AGENTS.md`](../AGENTS.md)'s default prohibition):
    ...
    - `Added`: `ConfigurationError` raised at `DjangoType.__init_subclass__` time when a consumer authors an `id` entry (annotation or assigned `strawberry.field(...)`) on a `Meta.interfaces = (relay.Node,)`-shaped type that is not a `relay.NodeID[...]`-marked annotation. Points at `strawberry.relay.NodeID[<pk_type>]` as the supported escape hatch. Replaces the downstream Strawberry-side `ValueError` ("Interface field Node.id expects type ID! but ...") that surfaced only at `strawberry.Schema(...)` construction. Narrow guard: `id: relay.NodeID[int]` is accepted; non-`id` consumer scalar overrides on Relay-Node-shaped types (including custom-named primary keys like `code: str` on `models.CharField(primary_key=True)`) are accepted.
...
### Decision 7 — Relay `id` override collision (H1 fix, rev3 narrowed)
...
**Contract.** This card adds a package-owned `ConfigurationError` raised at `DjangoType.__init_subclass__` time when **and only when** the consumer authored an `"id"` entry on a Relay-Node-shaped type, AND the entry is not a `relay.NodeID[...]`-marked annotation:
...
if relay_shaped:
    id_annotation = cls.__annotations__.get("id")
    id_assignment = cls.__dict__.get("id")
    has_id_annotation = id_annotation is not None
    has_id_assignment = isinstance(id_assignment, StrawberryField)
    if has_id_annotation or has_id_assignment:
        # Pass through valid relay.NodeID[...]-marked annotations (the
        # documented escape hatch). Detection mechanism: Worker 1's choice —
        # `typing.get_args` inspection for the NodeIDPrivate marker, or a
        # probe-call of cls.resolve_id_attr() after Phase 2.5 inject. The
        # spec contract is "valid relay.NodeID[...] annotations pass".
        if has_id_annotation and _is_relay_node_id_annotation(id_annotation):
            pass  # Accept.
        else:
...
**`relay.NodeID` detection — Worker 1 picks the mechanism.** Two viable approaches:

- **`typing.get_args` inspection.** `relay.NodeID[int]` resolves to `Annotated[int, NodeIDPrivate]` (or equivalent) under the hood. `typing.get_args(annotation)` returns `(int, NodeIDPrivate)`; checking the second slot for a `NodeIDPrivate`-shaped marker is a typing-stdlib-only detection that runs at `__init_subclass__` time.
- **Strawberry probe.** After Phase 2.5 of `finalize_django_types()` (which injects Relay defaults including `resolve_id_attr`), `cls.resolve_id_attr()` either returns the `relay.NodeID`-marked attribute's name or raises `NodeIDAnnotationError`. The existing composite-pk gate at `types/relay.py:134-148` uses this pattern; the H1 guard could move to Phase 2.5 alongside it and probe the same way. Cost: the guard fires at finalize time instead of class-creation time — slightly worse UX surface but reuses Strawberry's own detection logic.

The spec is neutral between the two; Worker 1 chooses during planning. Option 1 (class-creation-time detection via `typing.get_args`) is the recommended default — keeps the guard near its data and matches the early-raise spirit of this card.
...
|---|---|---|---|---|
| 1 | `types/base.py`, `types/definition.py`, `tests/types/test_definition_order.py` (and optionally `tests/types/test_converters.py` for the M2-placed `ArrayField` bypass test) | +75/-1 | 9 new tests: 4 core overrides + 3 converter-bypass + 3 Relay collision (1 reject + 2 accept) | Headline change. Includes the H1 Relay guard implementation and detection helper. |
...
Total expected delta: ~140 lines added across the package and tests; one delete in `tests/types/test_base.py`; one substantial KANBAN body insert in `KANBAN.md`. No new source files. No new test files (the M2-placed `ArrayField` bypass test extends an existing test module if Worker 1 picks placement b).
...
- **`relay.Node` `id` collision.** Pinned as a behavior contract, not a flagged edge case — see [Decision 7](#decision-7--relay-id-override-collision-h1-fix-rev3-narrowed). A consumer who writes `id: int` (or assigns `id = strawberry.field(...)`) on a Relay-Node-shaped type raises `ConfigurationError` at `__init_subclass__` time with a message pointing at `relay.NodeID[...]` as the supported escape hatch. The guard is narrow: `id: relay.NodeID[int]` passes (escape-hatch acceptance), and non-`id` consumer scalar overrides on Relay-Node-shaped types pass (no `Node.id` collision). The three Slice 1 tests (`test_consumer_id_annotation_on_relay_node_type_raises`, `test_consumer_id_relay_nodeid_annotation_on_relay_node_type_is_accepted`, `test_consumer_non_id_scalar_override_on_relay_node_type_is_accepted`) pin the reject + two accept paths.
...
The Slice 1 test cluster has nine tests total (rev3 expanded from eight):
...
**Three Relay-collision tests** (H1 fix, rev3 expanded) — pin [Decision 7](#decision-7--relay-id-override-collision-h1-fix-rev3-narrowed):

- `test_consumer_id_annotation_on_relay_node_type_raises` — `ConfigurationError` at class-creation time with message pointing at `relay.NodeID[...]` (reject path).
- `test_consumer_id_relay_nodeid_annotation_on_relay_node_type_is_accepted` — `id: relay.NodeID[int]` passes the guard (escape-hatch accept path).
...
- [ ] All nine Slice 1 tests pass (four core overrides + three converter-bypass in `tests/types/test_definition_order.py`, plus three Relay-collision tests — one reject + two accept; the `ArrayField` bypass test may live in `tests/types/test_converters.py` per M2 placement choice).
...
- [ ] `CHANGELOG.md` `[Unreleased]` carries the four entries from Slice 5 (`Added` annotation-only, `Added` introspection field, `Changed` converter-bypass, `Added` Relay collision guard).
