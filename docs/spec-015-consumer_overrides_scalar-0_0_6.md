...
Status: draft (revision 6, post-rev5 review).
...
- [ ] Slice 1: Track annotation-only scalar overrides on `DjangoTypeDefinition`
  ...
  - [ ] **Relay `id` collision guard (H1 fix, rev4 pinned class-creation-time + assigned-id rejection; rev6 sibling-field workaround in error message + rev6 H1 fail-soft fix).** After the `consumer_annotated_scalar_fields` / `consumer_assigned_scalar_fields` collections are built but before `_build_annotations` is invoked, detect: (a) `interfaces` (from `_validate_meta`) includes `relay.Node` — checked the same way `_build_annotations` does it at `types/base.py:609` (pre-Slice-1), via `any(issubclass(i, relay.Node) for i in interfaces) or issubclass(cls, relay.Node)`; AND (b) the consumer authored an entry for the GraphQL field name `"id"` — either an annotation (`"id" in cls.__annotations__` — rev4 M2: key-presence rather than value-truthiness so unusual annotations like `id: None`, `id: Literal[None]`, or string forms that evaluate to false-y types are also detected; rev6 M3 corrected the rev4 M2 rationale, which described the value as "literally None" — Python actually evaluates `None` to `<class 'NoneType'>`) or an assignment (`isinstance(cls.__dict__.get("id"), StrawberryField)`). Two reject paths:
    - **Assigned `id = <StrawberryField>`**: always rejected on a Relay-Node-shaped type. The error message names three supported alternatives (rev6 M1): `@classmethod resolve_id` for a custom id resolver, `id: relay.NodeID[<pk_type>]` for a custom id annotation, and a **sibling field** like `display_id: ID = strawberry.field(description="…")` for the field-level GraphQL metadata use case (rev4 M1 banned `id = strawberry.field(description="…")` and rev6 M1 acknowledged this removed the only field-metadata path for the Relay-supplied `id`; the sibling-field workaround keeps the Relay-supplied `id` undecorated and attaches the metadata to a co-resident field). Rev4 M1: this is a small intentional behavior change — previously consumers could write `@strawberry.field def id(self) -> relay.GlobalID: ...` and Strawberry would accept it. This card bans that pattern uniformly.
    - **Annotation `id: <type>` where `<type>` is not `relay.NodeID[...]`**: rejected. Detection of "is `relay.NodeID[...]`" uses `typing.get_type_hints(cls, include_extras=True)` (NOT raw `typing.get_args(cls.__annotations__["id"])`) so stringified annotations and PEP 563 (`from __future__ import annotations`) forms are evaluated against `cls`'s module globals. Fail-soft on `NameError`/`AttributeError`: the rev6 H1 fix scopes the fail-soft into two sub-cases — (i) the `id` annotation itself is an unresolved string, accept only when the string syntactically contains `"NodeID["`; (ii) some OTHER annotation on the class failed to resolve while `id` is directly resolved (rev6 H1 fix: fall back to `_has_node_id_marker(raw)` rather than rejecting on the not-a-string branch — the rev5 logic falsely rejected valid escape hatches in this case). The error message points at `relay.NodeID[<pk_type>]` as the supported escape hatch. (See [Decision 7](#decision-7--relay-id-override-collision) for the detection-helper pseudocode and the dropped finalize-time alternative.) **Important: the predicate is keyed off the GraphQL field name `"id"`, not the model's pk name.** A model with `code = models.CharField(primary_key=True)` and a consumer `code: str` override does NOT trigger the guard — the GraphQL fields are `id: ID!` (from Relay) and `code: String!` (from the consumer), no collision.
  ...
  - [ ] **Relay collision tests (H1 fix; eight new tests, rev6 expanded — added H1 sibling-annotation accept and L1 inheritance non-trigger).** New tests in `tests/types/test_definition_order.py` alongside the four-corner cluster:
    ...
    - [ ] `test_consumer_id_assigned_strawberry_field_on_relay_node_type_raises` (rev4 M1 assigned-id rejection; rev6 M1 sibling-field workaround in error message): declare a `DjangoType` with `Meta.interfaces = (relay.Node,)` and an assigned `id = strawberry.field(resolver=...)` (or `@strawberry.field def id(self) -> relay.GlobalID: ...` decorator-style). Assert `ConfigurationError` raised at class-creation time with message containing **all three** of `"resolve_id"`, `"relay.NodeID"`, and `"display_id"` (or the broader substring `"sibling field"`) — the three supported alternatives per rev6 M1. Pins the intentional ban on assigned `id` overrides on Relay-Node-shaped types and the rev6 M1 sibling-field workaround in the error message for the metadata-only assigned-`id` use case.
    ...
    - [ ] `test_consumer_id_string_relay_nodeid_annotation_on_relay_node_type_is_accepted` (rev4 H1 string-annotation acceptance; rev6 L3 — explicit string annotation is the default): declare a `DjangoType` with `Meta.interfaces = (relay.Node,)` and an explicit stringified `id: "relay.NodeID[int]"` consumer annotation. **Default: use the explicit string-annotation form**; the alternative (a test module with `from __future__ import annotations` so every annotation is implicitly stringified) is no longer recommended because it would require restructuring the test module for one test. Assert no `ConfigurationError` at class-creation time; assert `finalize_django_types()` succeeds; assert `strawberry.Schema(...)` builds. Pins that the `typing.get_type_hints(cls, include_extras=True)` detection helper resolves stringified annotations correctly.
    ...
    - [ ] `test_inherited_id_annotation_on_relay_node_subclass_is_not_caught_by_guard` (rev6 L1 — inheritance corner case acknowledgment): declare a base `DjangoType` subclass `BaseWithId` with `id: int` annotation but no `Meta` (so `__init_subclass__` short-circuits the collection pipeline for the base). Then declare a child `ChildRelayType(BaseWithId)` with `class Meta: model = M; interfaces = (relay.Node,)`. Assert: (a) no `ConfigurationError` at class-creation time — the rev6 L1 guard predicate `"id" in cls.__annotations__` is False for the child (inherited annotations don't land in the subclass's own `__annotations__` dict); AND (b) building a `strawberry.Schema(query=Query, types=[ChildRelayType])` raises Strawberry's `ValueError` with a message naming `Node.id`. Pins the documented inheritance behavior — the H1 guard does NOT walk the MRO, so inherited `id` annotations slip past at class-creation time; Strawberry's downstream `ValueError` at schema construction is the (acknowledged) failure mode. Without this test, future changes to the guard could accidentally start walking the MRO and the spec wouldn't catch the regression. (The behavior matches the existing relation-annotation inheritance behavior — see Edge cases.)
...
- [ ] Slice 5: Docs, KANBAN, CHANGELOG, archive (separate commit; may follow Slice 4 by any interval).
  ...
  - [ ] `KANBAN.md` — move `WIP-ALPHA-015-0.0.6` → `DONE-015-0.0.6`. **Drop in the verbatim body below:**
    ...
    - **`relay.Node` `id` collision rejected at type-creation time.** A consumer
      ...
      **sibling-field workaround** (e.g., `display_id: ID =
      strawberry.field(description="…")` for the field-level GraphQL
      metadata use case — the rev4 M1 ban on `id = <StrawberryField>` on
      Relay-Node-shaped types eliminated the only path for attaching
      `description`/`deprecation_reason`/`directives` to the Relay-supplied
      `id` field, and the rev6 M1 sibling-field workaround documents the
      supported alternative). Without the guard the consumer would have
      seen a Strawberry-side `ValueError` only at `strawberry.Schema(...)`
      construction, which obscured the source. The guard is narrow: it
      fires only when the consumer authored an `id` entry on a Relay-
      Node-shaped type AND the annotation is not a `relay.NodeID[...]`-
      marked annotation. Detection uses `typing.get_type_hints(cls,
      include_extras=True)` so direct, PEP 563 / `from __future__ import
      annotations`, and explicit-string forms are all resolved against
      the consumer's module globals; the fail-soft branch covers two
      sub-cases — id-itself-failed-to-resolve (accept only if the raw
      string contains `"NodeID["`) and id-resolved-but-sibling-failed
      (rev6 H1: fall back to `_has_node_id_marker(raw)` on the already-
      resolved object so directly-resolved `id: relay.NodeID[int]`
      alongside a forward-referenced relation annotation is accepted).
      `id: relay.NodeID[int]` and `id: "relay.NodeID[int]"` (the
      documented escape hatch in direct and stringified forms) are
      accepted; non-`id` consumer scalar overrides (e.g., `description:
      int`, or `code: str` on a model with `code` as pk) pass through
      unchanged; inherited `id` annotations on a subclass slip past the
      guard at class-creation time (Strawberry's downstream `ValueError`
      is the acknowledged failure mode — matches the existing relation-
    ...
    - Resolver / metadata overrides for scalars stay on the assigned
      ...
      `0.0.6` (the rev4 M1 / rev6 M1 assigned-`id` ban applies uniformly);
      the documented workaround is a sibling field carrying the metadata.
...
if relay_shaped:
    ...
    if has_id_annotation or has_id_assignment:
        ...
        # this card's contract treats `id` as Relay-owned. Rev6 M1: the
        # error message names a sibling-field workaround for consumers
        # who only wanted to attach GraphQL field-level metadata
        # (description / deprecation_reason / directives) to the
        # Relay-supplied id — the supported alternative is a separate
        # field like `display_id: ID` carrying the metadata, with the
        # Relay-supplied `id` left undecorated.
        if has_id_assignment:
            raise ConfigurationError(
                ...
                "or declare a sibling field (e.g., `display_id: ID = "
                "strawberry.field(description=...)`) if you only need "
                "GraphQL field-level metadata; "
...
def _id_annotation_is_relay_node_id(cls: type) -> bool:
    ...
    1. **``id`` annotation itself failed to resolve.** ``cls.__annotations__["id"]``
       ...
       syntactically contains ``"NodeID["`` — covers the qualified
       (``"relay.NodeID[int]"``, ``"strawberry.relay.NodeID[int]"``) and
       unqualified (``"NodeID[int]"``) forms of a deferred NodeID
       reference. Other unresolved strings (typos like ``id:
       "MissingType"``, non-NodeID forward references) return False so
       the H1 guard fires at class-creation time with the supported-
       escape-hatch message — exactly the failure surface this guard
       exists to replace.
    ...
    try:
        ...
        # Fail-soft (rev5 M1 + rev6 H1): handle both "id failed to
        # resolve" (raw is a string) and "another annotation failed
        # but id is already resolved" (raw is an Annotated object).
        raw = cls.__annotations__.get("id")
        if isinstance(raw, str):
            # Sub-case 1: id is the unresolved string. Accept only
            # NodeID-shaped strings. Typos and non-NodeID forward
            # references fall through to the guard.
...
**Narrow fail-soft on unresolved forward references (rev5 M1 + rev6 H1).** `typing.get_type_hints(cls, include_extras=True)` evaluates **every** annotation on `cls` (and walks the MRO). A single unresolved string annotation anywhere on the class trips `NameError` (or `AttributeError`, for nested attribute references) — even when the `id` annotation itself resolves cleanly. The detection helper's fail-soft branch therefore covers two distinct sub-cases:

1. **The `id` annotation itself failed to resolve.** `cls.__annotations__["id"]` is the raw string the consumer wrote. Accept **only** when that string syntactically contains `"NodeID["` — covering the qualified (`"relay.NodeID[int]"`, `"strawberry.relay.NodeID[int]"`) and unqualified (`"NodeID[int]"`) forms of an unresolved-at-class-creation NodeID reference. Other unresolved strings — typos like `id: "MissingType"`, non-NodeID forward references like `id: "MyEnumType"` — are rejected by the H1 guard with the standard "use `relay.NodeID[...]`" error message. The Strawberry-side schema-construction failure mode that the guard exists to replace is preserved for typo / non-NodeID cases; the fail-soft window is open only for syntactically-NodeID-shaped unresolved strings, which is the genuinely-ambiguous case (a consumer who wrote a NodeID-shaped string but imports the symbol in a later module).
...
|---|---|---|---|---|
| 1 | `types/base.py`, `types/definition.py`, `tests/types/test_definition_order.py`, `tests/types/test_converters.py` (the default placement for the nested-`ArrayField` bypass test per rev6 L3) | +185/-1 | 16 new tests: 4 core overrides + 4 converter-bypass (added cross-type cache, rev6 L2) + 8 Relay collision (3 reject + 5 accept; rev6 added H1 sibling-annotation accept and L1 inheritance non-trigger) | Headline change. Includes the H1 Relay guard implementation and the `_id_annotation_is_relay_node_id` detection helper (with the rev6 H1 fail-soft fix for the resolved-id + unresolved-sibling sub-case). |
...
- **`relay.Node` `id` collision.** Pinned as a behavior contract, not a flagged edge case — see [Decision 7](#decision-7--relay-id-override-collision). A consumer who writes `id: <non-NodeID-type>` annotation, an unresolved-non-NodeID stringified annotation (e.g., `id: "MissingType"`), or assigns any `id = <StrawberryField>` on a Relay-Node-shaped type raises `ConfigurationError` at `__init_subclass__` time. The annotation-side errors point at `relay.NodeID[...]` as the supported escape hatch; the assigned-side error points at three alternatives — `relay.NodeID[<pk_type>]`, `@classmethod resolve_id`, and the rev6 M1 sibling-field workaround (`display_id: ID = strawberry.field(description="…")` for the field-level metadata use case). The guard is narrow: `id: relay.NodeID[int]` passes in direct, stringified, and mixed (directly-resolved `id` alongside other unresolved annotations) forms (rev4 H1 + rev5 M1 + rev6 H1: detection uses `typing.get_type_hints(cls, include_extras=True)` with two fail-soft sub-cases — id-itself-failed-to-resolve scoped by the `"NodeID["` substring check, and id-resolved-but-sibling-failed scoped by direct `_has_node_id_marker(raw)` inspection of the resolved object); non-`id` consumer scalar overrides on Relay-Node-shaped types pass (no `Node.id` collision); inherited `id` annotations on a subclass slip past the guard at class-creation time and surface as Strawberry's downstream `ValueError` (rev6 L1: the guard does not walk the MRO, matching the existing relation-annotation inheritance behavior; the failure mode is acknowledged, not fixed). The eight Slice 1 Relay-collision tests pin the reject + accept + inheritance-non-trigger paths: `test_consumer_id_annotation_on_relay_node_type_raises`, `test_consumer_id_assigned_strawberry_field_on_relay_node_type_raises`, `test_consumer_id_unresolved_non_nodeid_string_on_relay_node_type_raises`, `test_consumer_id_relay_nodeid_annotation_on_relay_node_type_is_accepted`, `test_consumer_id_string_relay_nodeid_annotation_on_relay_node_type_is_accepted`, `test_consumer_id_resolved_relay_nodeid_with_unresolved_sibling_annotation_is_accepted`, `test_consumer_non_id_scalar_override_on_relay_node_type_is_accepted`, and `test_inherited_id_annotation_on_relay_node_subclass_is_not_caught_by_guard`.
...
The Slice 1 test cluster has 16 tests total (rev6 expanded from rev5's 13):
...
**Eight Relay-collision tests** (H1 fix; rev6 expanded — added H1 sibling-annotation accept and L1 inheritance non-trigger) — pin [Decision 7](#decision-7--relay-id-override-collision):
...
- `test_consumer_id_assigned_strawberry_field_on_relay_node_type_raises` — `ConfigurationError` at class-creation time with message pointing at `resolve_id`, `relay.NodeID[...]`, and the rev6 M1 sibling-field workaround `display_id` (assigned reject path; rev4 M1 — small intentional behavior change; rev6 M1 — sibling-field workaround in the error message).
...
- `test_consumer_id_string_relay_nodeid_annotation_on_relay_node_type_is_accepted` — `id: "relay.NodeID[int]"` stringified form passes the guard (rev4 H1 — pins the `typing.get_type_hints(cls, include_extras=True)` evaluation for PEP 563 / stringified annotations; rev6 L3 — explicit string-annotation form is the default test recipe).
...
- `test_inherited_id_annotation_on_relay_node_subclass_is_not_caught_by_guard` (rev6 L1) — inherited `id` annotation on a Relay-Node-shaped subclass does NOT trigger the guard at class-creation time; Strawberry's downstream `ValueError` at `strawberry.Schema(...)` is the (acknowledged) failure mode (pins the documented inheritance behavior; the guard does not walk the MRO).
...
Slices 3 / 4 / 5 are documentation-only and have no test deltas. Coverage stays at 100% because the new definition field is exercised by Slice 1 tests, the new collection branch in `__init_subclass__` is exercised by every override test in `tests/types/test_definition_order.py`, the new Relay-collision guard is exercised by all eight H1 tests (three reject + five accept including the rev6 H1 sibling-annotation accept and the rev6 L1 inheritance non-trigger), the `_id_annotation_is_relay_node_id` detection helper's resolved-success path is hit by `test_consumer_id_relay_nodeid_annotation_on_relay_node_type_is_accepted`, the resolved-reject path is hit by `test_consumer_id_annotation_on_relay_node_type_raises`, the fail-soft-sub-case-1-accept path is hit by `test_consumer_id_string_relay_nodeid_annotation_on_relay_node_type_is_accepted` (when the test module deliberately leaves `relay` unimported at module scope, the stringified annotation goes through the fail-soft branch and matches the `"NodeID["` substring check), the fail-soft-sub-case-1-reject path is hit by `test_consumer_id_unresolved_non_nodeid_string_on_relay_node_type_raises`, the fail-soft-sub-case-2-accept path (rev6 H1) is hit by `test_consumer_id_resolved_relay_nodeid_with_unresolved_sibling_annotation_is_accepted`, and the converter-bypass paths are exercised by the four H2 / rev6 L2 tests (unsupported / grouped-choices / nested-array / cross-type-cache).
...
- [ ] `docs/FEATURES.md`'s `Scalar field override semantics` body names the rev6 M1 metadata-route limitation: field-level GraphQL metadata on the Relay-supplied `id` is not configurable in `0.0.6`; the documented workaround is a sibling field (`display_id: ID = strawberry.field(description="…")`) carrying the metadata with the Relay-supplied `id` left undecorated (Slice 5).
