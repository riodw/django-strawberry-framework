...
- [ ] Slice 1: Module + `AppConfig` subclass
  ...
  - [ ] Implement `DjangoStrawberryFrameworkConfig(AppConfig)` with exactly four pieces of state (rev3 H1 — bumped from three to four; the class docstring is required by ruff's `D101` rule, which is enabled via `pyproject.toml`'s `[tool.ruff.lint] select = [..., "D", ...]` and NOT excluded by the ignore list; a public `AppConfig` subclass without a class docstring fails the `uv run ruff check --fix .` gate this slice itself runs):
    ...
    - module docstring (one line) naming the module's purpose ("Django AppConfig — registers the package with Django's app loader so consumers can list it in `INSTALLED_APPS` and Django's check / signal hooks resolve against it.").
    - class docstring (one line) naming the class's purpose, e.g. `"""Register django-strawberry-framework with Django's app loader."""` directly under the `class DjangoStrawberryFrameworkConfig(AppConfig):` line. Required by `D101`. Do NOT suppress with `# noqa: D101` — this is a normal public class and the docstring IS the root-cause fix; the noqa would be a workaround the codebase's "highest standard always" posture ([`AGENTS.md`](../AGENTS.md) line 4) rejects.
...
1. Ship `django_strawberry_framework/apps.py` containing `DjangoStrawberryFrameworkConfig(AppConfig)` with `name = "django_strawberry_framework"` and `verbose_name = "Django Strawberry Framework"`. Two behavioral attributes plus a one-line module docstring plus a one-line class docstring (required by ruff's `D101` per rev3 H1); nothing else.
...
- **AppConfig subclass with two attributes.** Same shape adopted here: `name` (the package directory) and `verbose_name` (a human-readable label). Justification: strawberry-django's shape is the minimal Django-correct surface for an installable package; the upstream has shipped this for years without needing more. Borrowing the shape verbatim avoids inventing complexity that the ecosystem has not asked for. **One forced divergence**: this repo's pydocstyle gate (`pyproject.toml [tool.ruff.lint] select = [..., "D", ...]`, `D101` not ignored) requires a class docstring; the upstream `strawberry_django/apps.py` has none. We add a one-line class docstring; see [Decision 2](#decision-2--name--label--verbose_name-pinning) (rev3 H1).
...
The class declares exactly **two behavioral attributes** plus the class docstring required by `D101` (rev3 H1) plus its inherited base behavior:
...
- class docstring `"""Register django-strawberry-framework with Django's app loader."""` (or equivalent one-liner naming the class's purpose) — required by ruff's `D101` rule (`pyproject.toml [tool.ruff.lint] select = [..., "D", ...]`; `D101` is not in the `ignore` list). Diverges from strawberry-django's `apps.py` (which has no class docstring) because this repo's pydocstyle gate is stricter than the upstream's; the divergence is forced by the gate, not chosen for stylistic reasons.
...
1. `django_strawberry_framework/apps.py` exists and defines `DjangoStrawberryFrameworkConfig(AppConfig)` per [Decision 1](#decision-1--module-location--public-export) and [Decision 2](#decision-2--name--label--verbose_name-pinning) — `name = "django_strawberry_framework"`, `verbose_name = "Django Strawberry Framework"`, a one-line **class docstring** (required by ruff's `D101` per rev3 H1), no `label` override, no `default_auto_field`, no `ready()` body, no `default = True` marker.
...
6. The class does not implement `ready()`, does not declare `label`, does not declare `default_auto_field`, and does not declare `default = True` (per [Decision 2](#decision-2--name--label--verbose_name-pinning), [Decision 4](#decision-4--no-readyhook-in-0_0_7), [Decision 5](#decision-5--no-default_auto_field-and-no-models), [Decision 8](#decision-8--no-default--true-marker) respectively). All four absences are pinned by the consolidated `test_djangostrawberryframeworkconfig_defines_no_extra_appconfig_attributes` in `tests/test_apps.py` (rev2 H2 — rev1 only tested the `ready` absence; the other three were review-only until rev2 folded them into the same test).
...
10. `KANBAN.md` moves `WIP-ALPHA-017-0.0.7` to Done with the next `DONE-NNN-0.0.7` id and a past-tense body summarizing the shipped scope.
...
13. `uv run ruff format .` passes; `uv run ruff check --fix .` passes; `uv run pytest` passes with 100% package coverage.
