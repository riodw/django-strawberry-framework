...
- [ ] Slice 1: Foundation — module layout + `Filter` primitives + `FilterSet` metaclass
  - [ ] Create [`django_strawberry_framework/filters/`][filters] subpackage with `__init__.py`, `base.py`, `sets.py`, `factories.py`, `inputs.py` per [Decision 2](#decision-2--subpackage-layout-and-public-export-surface). Module-level docstrings on each file pin the responsibility (`base.py` = `Filter` / `RelatedFilter` / primitives; `sets.py` = `FilterSet` (subclasses `django_filters.BaseFilterSet`) + metaclass; `factories.py` = `FilterArgumentsFactory` + `get_filterset_class` + `_dynamic_filterset_cache`; `inputs.py` = filter input classes materialized as module globals + input-data adapters + lifecycle ledger).
...
- [ ] Slice 2: Factories — `FilterArgumentsFactory` BFS + dynamic-filterset cache
  ...
  - [ ] `factories.py` ships `FILTER_DEFAULTS` (the FK/PK filter mapping; the Strawberry-adapted analogue of `graphene_django/filter/filterset.py::GrapheneFilterSetMixin.FILTER_DEFAULTS`). Conditional on the target `DjangoType`'s Relay shape per [Decision 4](#decision-4--upstream-primitives-parity-floor): Relay-Node targets → `GlobalIDFilter` (required for Relay-aware filtering against `relay.Node`-shaped types shipped in `DONE-011-0.0.5`); non-Relay targets → the scalar PK filter derived from the target PK column conversion.
...
- `filterset.py::AdvancedFilterSet` → `django_strawberry_framework.filters.sets::FilterSet`, subclassing `django_filters.BaseFilterSet` exactly as the cookbook does. `FilterSet.get_filters` runs Layer-4 cycle-safe expansion with the same `__dict__["_expanded_filters"]` cache and `__dict__["_is_expanding_filters"]` recursion guard.
...
When [`DjangoConnectionField`][glossary-djangoconnectionfield] ships in `0.0.9`, it accepts `filterset_class=` and owns the argument injection internally; the manual `filter_input_type(...)` annotation is the explicit shape `0.0.8` consumers reach for at the resolver level and the same shape the connection field will reuse one card later.
...
- `FilterArgumentsFactory._ensure_built` produces both halves: it materializes each input class as a module global (via the helper [`materialize_input_class(module, name, cls)`][filters] in `inputs.py`) AND it emits the `Annotated[...]` shape in field annotations so cycle-safe references between filtersets keep working.
...
- Five of six layers are library-agnostic Python (on top of the shared `django_filters.BaseFilterSet` per [Decision 5](#decision-5--django-filter-as-the-foundation)) — the port is mechanical, not creative.
...
The `FILTER_DEFAULTS` map ships at [`django_strawberry_framework.filters.factories::FILTER_DEFAULTS`][filters] as the Strawberry-adapted variant of `graphene_django/filter/filterset.py::GrapheneFilterSetMixin.FILTER_DEFAULTS`. The mapping is **conditional on the target `DjangoType`'s Relay shape** — corrected per H4 in [`docs/feedback.md`][feedback]:
...
The lookup runs at finalize time inside `FilterArgumentsFactory._ensure_built`: for each relation field, the factory consults `registry.get(target_model)` (the model-to-`DjangoType` registry from [`Meta.primary`][glossary-metaprimary]) to find the primary target `DjangoType`, then checks `implements_relay_node(target_type)` (the same predicate used by [`finalize_django_types`][finalizer]'s phase 2.5 Relay-Node injection) to pick the `GlobalIDFilter` branch vs the scalar PK branch.

Rev1's blanket "FK/PK → `GlobalIDFilter`" mapping (mirroring `graphene-django`'s unconditional shape) would have broken non-Relay filters: a consumer filtering `Book.shelf` by ID against a non-Relay `ShelfType` would have to pass a GlobalID-encoded string even though the exposed GraphQL type uses an integer ID — a wire-shape mismatch the consumer would discover at the first query. `graphene-django`'s unconditional mapping is consistent because graphene-django always Relay-shapes; the package's default is non-Relay (`Meta.interfaces = (relay.Node,)` is opt-in), so the conditional shape matches the actual GraphQL surface the consumer sees.
...
The package's `FilterSet` **subclasses `django_filters.BaseFilterSet`** and `django-filter` remains a **hard dependency** at [`pyproject.toml #"django-filter>=25.2"`][pyproject]. The cookbook's `AdvancedFilterSet` is built on `django_filters.BaseFilterSet` and reuses its load-bearing machinery (`declared_filters`, `base_filters`, `.filters`, `.form`, `.qs`, `get_filters()`, `Filter.method`, form cleaning, lookup generation); porting the cookbook verbatim without that inheritance would mean re-implementing all of those layers, which is a large net-new surface for zero gain when the dependency is already locked in [`pyproject.toml`][pyproject].
...
2. [`GOAL.md`][goal] (line 450) explicitly promises "the existing `django_filters.FilterSet` plugs into `Meta.filterset_class` directly" — a direct migration path that requires `FilterSet` to be a `BaseFilterSet`.
3. The cookbook's `django_graphene_filters/filterset.py::AdvancedFilterSet` IS a `django_filters.BaseFilterSet` subclass at load-bearing layers; the spec's "five of six layers port verbatim" claim only holds when those layers ride on `BaseFilterSet`.
...
- The cost of keeping it is one line of `class FilterSet(django_filters.BaseFilterSet, metaclass=FilterSetMetaclass):` plus an `import django_filters` at the top of `sets.py`.
...
- **Accept either a package-`FilterSet` or a `django_filters.FilterSet` at `Meta.filterset_class`.** Rejected: two consumer-facing surfaces for the same wiring create ambiguity about which `check_*_permission` shape applies and which lazy-resolution mechanism runs. The single package `FilterSet` (which IS a `django_filters.BaseFilterSet` via inheritance) is the boundary class; consumers convert via parent-class swap, not via dual acceptance.
...
- `test_library_branches_filter_respects_related_queryset_boundary` — declares a `BranchFilter` with `RelatedFilter(ShelfFilter, queryset=Shelf.objects.filter(topic="permanent collection"))` (uses the existing `Shelf.topic` text field — corrected per M1 of [`docs/feedback.md`][feedback], which flagged the rev1 `is_archived` field as nonexistent on the `Shelf` model); queries `{ allLibraryBranches(filter: { shelves: { code: { iContains: "A" } } }) { ... } }`; asserts shelves outside the "permanent collection" topic never appear regardless of the nested filter clause.
...
4. `sets.py` ships `FilterSetMetaclass` and `FilterSet` per Layers 3 + 4 of [Decision 3](#decision-3--six-layer-lazy-resolution-pipeline). `FilterSet` subclasses `django_filters.BaseFilterSet` per [Decision 5](#decision-5--django-filter-as-the-foundation) and carries `Meta.model`, `Meta.fields`, `get_filters`, `filter_queryset`, `check_permissions`, `_apply_related_queryset_constraints` on top of the upstream's `declared_filters` / `base_filters` / `.filters` / `.form` / `.qs` / `Filter.method` machinery.
5. `factories.py` ships `FilterArgumentsFactory` (Layer 5 BFS, with module-globals materialization via `materialize_input_class`), `get_filterset_class`, `_dynamic_filterset_cache` (Layer 6), and `FILTER_DEFAULTS` (FK/PK → `GlobalIDFilter` conditional on Relay-target shape per [Decision 4](#decision-4--upstream-primitives-parity-floor)).
...
13. [`examples/fakeshop/apps/library/schema.py`][fakeshop-library-schema] grows `Meta.filterset_class = filters.BranchFilter` (and the matching key on `ShelfType` / `BookType` / `LoanType` / `PatronType` / `GenreType`) on the corresponding `DjangoType` classes; root resolvers annotate `filter:` via `filter_input_type(filters.<Name>Filter)` per [Decision 11](#decision-11--filter_input_typefilterset-consumer-helper) and call the resolved filterset's `filter_queryset(...)` before returning the queryset.
14. [`examples/fakeshop/test_query/test_library_api.py`][fakeshop-test-library] grows 8 live `/graphql/` HTTP tests per the [Test plan](#test-plan) (scalar / choice-enum / non-Relay-FK-scalar / Relay-M2M-GlobalID / reverse-FK / logical / optimizer-cooperation / related-queryset-boundary-via-`Shelf.topic` / cross-module-absolute-path-`RelatedFilter`). The Relay-vs-scalar split pins [Decision 4](#decision-4--upstream-primitives-parity-floor)'s conditional `FILTER_DEFAULTS` end-to-end.
