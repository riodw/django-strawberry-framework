...
- [ ] Slice 1: migrate `extensions=` construction sites to the singleton-factory form (per [Decision 3](#decision-3--slice-1-adopts-the-singleton-factory-extensions-form))
  - [ ] Rewrite each `extensions=[DjangoOptimizerExtension()]` (instance) and `extensions=[DjangoOptimizerExtension]` (class) construction site to a module-level singleton plus a factory: `_optimizer = DjangoOptimizerExtension(...)` then `extensions=[lambda: _optimizer]`. This preserves the instance-bound [Plan cache][glossary-plan-cache] (same singleton per request under 0.316.0's `get_extensions`) AND drops the `Schema.__init__` instance-form `DeprecationWarning`. Do NOT use the bare class or a constructing-`lambda` (re-instantiated per request → cold cache, both modes).
  - [ ] Code sites: the package test schemas ([`tests/optimizer/test_extension.py`][test-extension] (~19), [`tests/optimizer/test_field_meta.py`][test-field-meta], [`tests/optimizer/test_relay_id_projection.py`][test-relay-id-projection], [`tests/test_list_field.py`][test-list-field], [`tests/types/test_generic_foreign_key.py`][test-generic-fk], [`examples/fakeshop/test_query/test_multi_db.py`][fakeshop-test-multi-db]) — one module-level `_optimizer` per file; the example schema [`examples/fakeshop/config/schema.py`][fakeshop-config-schema] (currently the bare class — a cold-cache regression under 0.316.0) and [`TODAY.md`][today] (class form) both move to the singleton-factory.
  - [ ] Consumer-doc snippets: rewrite the `extensions=[DjangoOptimizerExtension()]` schema-construction snippets in [`docs/README.md`][docs-readme] and [`docs/GLOSSARY.md`][glossary] to the singleton-factory form, with a one-line "module-level singleton wrapped in a factory — preserves the instance-bound [Plan cache][glossary-plan-cache], no deprecation warning" note.
...
**Decision for `0.0.9`: migrate the instance form to the module-level-singleton factory `extensions=[lambda: _optimizer]`.** It is the modernize-without-regression form — preserves the instance-bound plan cache AND removes the (live, "will be removed") deprecation warning, with **no plan-cache relocation required**. Slice 1 rewrites each `extensions=[DjangoOptimizerExtension()]` construction site to declare a module-level `_optimizer = DjangoOptimizerExtension(...)` (one per module; pass `strictness=` here if needed) and pass `extensions=[lambda: _optimizer]`: the consumer-doc snippets ([`docs/README.md`][docs-readme], [`docs/GLOSSARY.md`][glossary]), the example schema ([`config/schema.py`][fakeshop-config-schema]), and the package test schemas (~19 sites in [`tests/optimizer/test_extension.py`][test-extension], plus [`test_field_meta.py`][test-field-meta] / [`test_relay_id_projection.py`][test-relay-id-projection] / [`test_list_field.py`][test-list-field] / [`test_generic_foreign_key.py`][test-generic-fk] / [`test_multi_db.py`][fakeshop-test-multi-db]). The bare instance stays correct but deprecated; the bare class / constructing-`lambda` are rejected (cold cache per request).
...
**Reaching a finalized registry (cold CLI invocation).** A bare `manage.py inspect_django_type BookType` in a cold process registers nothing (no module imported `BookType`) → unresolvable; and even the dotted form imports only the target's own module, which registers that one type but leaves the registry **unfinalized** (`finalize_django_types()` needs *every* relation target registered, so importing one module is not enough). So the command takes an optional **`--schema <dotted_path>`** (e.g. `config.schema`) that it `import_string`s **first** — importing the project schema registers all `DjangoType`s and runs the module's `finalize_django_types()` — before resolving the target type. This mirrors how [`export_schema.py`][export-schema-cmd] is self-triggering via its schema argument. Without `--schema`, the command works only when the registry is already populated + finalized in-process (the happy-path tests, where pytest has imported `config.schema`); the unfinalized-`CommandError` names `--schema` as the fix. (Whether `--schema` defaults from a settings key is deferred — the package adds settings keys only when a feature needs them, per [`AGENTS.md`][agents]; for `0.0.9` it is an explicit argument.)
...
| --- | --- | --- | --- |
| 1 — `extensions=` singleton-factory migration | [`docs/README.md`][docs-readme], [`docs/GLOSSARY.md`][glossary], [`examples/fakeshop/config/schema.py`][fakeshop-config-schema], [`TODAY.md`][today], the ~24 package test-schema sites ([`tests/optimizer/test_extension.py`][test-extension] + 4 others), [`CHANGELOG.md`][changelog] | 1 (a no-`DeprecationWarning` assertion); existing optimizer suite is the behavior-preserving regression guard | `+45 / -28` |
...
- [`examples/fakeshop/tests/test_inspect_django_type.py`][fakeshop-tests-inspect] (new) — in-process `call_command` against real finalized fakeshop types:
  - `test_inspect_by_registered_name` — `call_command("inspect_django_type", "BookType")`; capture stdout; assert every selected `BookType` field appears with its resolved scalar and nullability — including the **non-Relay** pk `id` → `Int!` (BookType declares no `interfaces`, so the pk is a plain scalar, NOT `GlobalID!`) — plus `title` → `String!`, `subtitle` → `String`, `circulation_status` → choice enum, `genres` → list relation.
  ...
  - `test_inspect_with_schema_option` — `call_command("inspect_django_type", "BookType", "--schema", "config.schema")`; asserts the `--schema` import path resolves and finalizes the registry (pins the cold-invocation contract — see [Decision 4](#decision-4--inspect_django_type-command-shape-and-argument-resolution) *Reaching a finalized registry*).
...
2. Every `extensions=[DjangoOptimizerExtension()]` (instance) and `extensions=[DjangoOptimizerExtension]` (class) construction site — the package test schemas, the example `config/schema.py`, and the consumer docs — is migrated to the module-level-singleton factory `_optimizer = DjangoOptimizerExtension(...); extensions=[lambda: _optimizer]` per [Decision 3](#decision-3--slice-1-adopts-the-singleton-factory-extensions-form), preserving the instance-bound [Plan cache][glossary-plan-cache].
3. The consumer-doc snippets in [`docs/README.md`][docs-readme] and [`docs/GLOSSARY.md`][glossary] show the singleton-factory form with the one-line rationale; [`examples/fakeshop/config/schema.py`][fakeshop-config-schema] and [`TODAY.md`][today] (previously the bare class — a cold-cache regression under 0.316.0) are migrated too.
