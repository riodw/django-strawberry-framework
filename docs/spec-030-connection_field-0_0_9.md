...
- [`Meta.connection`][glossary-metaconnection] — the net-new type-level Relay connection options key this card ships; `0.0.9` accepts `{"total_count": bool}`, requires [`Meta.interfaces`][glossary-metainterfaces] to include `relay.Node`, and drives the per-target connection class's `totalCount` shape.
...
- [ ] Slice 2: `DjangoConnectionField` factory + synthesized-signature argument injection + composition pipeline + consumer-resolver contract + optimizer cooperation point + sync/async (per [Decision 5](#decision-5--factory-function-mechanism-meta-only-derivation) / [Decision 6](#decision-6--sidecar-derived-arguments-via-a-synthesized-resolver-signature) / [Decision 7](#decision-7--composition-pipeline-visibilityfilterorderdefault-orderoptimizer) / [Decision 11](#decision-11--the-connection-field-owns-its-optimizer-cooperation-point))
  - [ ] `DjangoConnectionField(target_type, *, resolver=None, description=None, deprecation_reason=None, directives=())` PascalCase factory (Meta-only derivation — no `filters=` / `order=` / `total_count=` kwargs, per [Decision 5](#decision-5--factory-function-mechanism-meta-only-derivation)) running the four [`DjangoListField`][glossary-djangolistfield]-style guards (`isclass` → `issubclass(DjangoType)` → own-class `definition.origin is target_type` → callable resolver) plus a Relay-Node-shaped guard that checks the **declared-interfaces tuple** (`any(issubclass(i, relay.Node) for i in definition.interfaces)`), NOT the MRO-based `implements_relay_node(target_type)` — the factory runs before `finalize_django_types()` injects `relay.Node` into `__bases__`, so the MRO predicate is `False` for every legitimate target at construction time (per [Decision 5](#decision-5--factory-function-mechanism-meta-only-derivation)). A non-Relay target raises [`ConfigurationError`][glossary-configurationerror]. Returns `relay.connection(_connection_type_for(target_type), resolver=<synthesized>, description=…, …)`.
...
- [`docs/GLOSSARY.md`][glossary] already has `## DjangoConnectionField`, `## DjangoConnection`, `## Meta.connection`, and `## Connection-aware optimizer planning` headings (all `planned for 0.0.9`). Slice 5 flips the first three to `shipped (0.0.9)` and leaves the connection-aware entry planned for the sibling [`WIP-ALPHA-033-0.0.9`][kanban] card.
...
The Relay-Node-shaped guard at this **construction-time** call site checks the **declared-interfaces tuple** stored on the definition — `any(issubclass(i, relay.Node) for i in definition.interfaces)`, the same `issubclass(i, relay.Node)` half [Decision 8](#decision-8--metaconnection-opt-in-key-stored-on-the-definition)'s `_validate_connection` uses — **not** the MRO-based `implements_relay_node(target_type)` (`issubclass(target_type, relay.Node)`) predicate. The factory runs at class-body time, before [`finalize_django_types()`][glossary-finalize_django_types] Phase 2.5 (`apply_interfaces`) injects `relay.Node` into `target_type.__bases__`, so the MRO predicate would be `False` for every legitimately-declared connection target here; the stored `definition.interfaces` tuple is the only Relay-Node signal available at construction time. A non-Relay target raises [`ConfigurationError`][glossary-configurationerror].
...
- **Optimizer cooperation scope.** Preferred answer per [Decision 11](#decision-11--the-connection-field-owns-its-optimizer-cooperation-point): the field owns the cooperation point via the extracted helper, which runs and publishes a plan before the slice; ALL `edges { node }` planning (root scalar/FK projection included) is [`WIP-ALPHA-033-0.0.9`][kanban]'s walker-awareness change — the derived plan is empty in `0.0.9` — guarded by a strictness `"raise"` test. Fallback: if `033` slips past the joint cut, the documented constraint stands and the strictness test keeps the gap visible — connection fields are still correct (filter / order / pagination / `totalCount` all work) and the cooperation seam is in place, so `033` lights up optimization with no `connection.py` change.
