...
- [ ] Slice 2: the encode seam — strategy-parameterized `resolve_typename` injection + the four encoders + the default flip to `model` (per [Decision 3](#decision-3--the-encode-seam-a-strategy-parameterized-resolve_typename-default) / [Decision 4](#decision-4--four-strategies-model-type-typemodel-callable-and-an-unchanged-node_id-portion) / [Decision 9](#decision-9--changing-the-default-to-model-is-a-breaking-wire-format-change-acceptable-pre-100) / [Decision 10](#decision-10--resolve_typename-injection-via-the-__func__-identity-test-at-phase-25))
  ...
  - [ ] An `install_globalid_typename_resolver(type_cls, definition)` step (called from [`finalize_django_types`][glossary-finalize_django_types] Phase 2.5, alongside `install_relay_node_resolvers`) runs the `existing.__func__ is relay.Node.resolve_typename.__func__` override test (MRO-aware, the same the four `resolve_*` defaults use) **before** installing: a consumer override → effective strategy **`custom`**, install nothing (and if the type also declares an explicit `Meta.globalid_strategy`, raise [`ConfigurationError`][glossary-configurationerror] — the both-declared conflict, [Decision 10](#decision-10--resolve_typename-injection-via-the-__func__-identity-test-at-phase-25)); no override → resolve via `_resolve_globalid_strategy`, install the package closure for `model` / `type+model` / `callable` (the `callable` closure raises [`ConfigurationError`][glossary-configurationerror] on a non-`str` / empty return), leave Strawberry's default for `type`. It **records the resolved effective strategy** (`model` / `type` / `type+model` / `callable` / `custom`) on the [`DjangoTypeDefinition`][definition] (a finalization-set field, since the post-install `__func__` can no longer tell a consumer override from the framework closure).
...
- A `decode_global_id` input that is malformed base64 or does not decode to a `"<type_name>:<node_id>"` shape → [`ConfigurationError`][glossary-configurationerror] (Strawberry's `GlobalIDValueError` / `ValueError` from `from_base64` is caught and re-raised, [Decision 8](#decision-8--decode-routes-through-djangos-app-registry-then-the-framework-registry-to-the-primary-type)).
...
`decode_global_id(gid: relay.GlobalID | str)` accepts a Strawberry [`relay.GlobalID`][glossary-relay-node-integration] instance or its base64 string form (which it `from_base64`-decodes); it never accepts a raw, un-encoded `"app_label.modelname:pk"` payload ([`docs/feedback.md`][feedback] P3). A malformed base64 string or one that does not decode to a `"<type_name>:<node_id>"` shape — which Strawberry surfaces as `GlobalIDValueError` / `ValueError` from `from_base64` — is caught and re-raised as [`ConfigurationError`][glossary-configurationerror], so every `decode_global_id` failure (malformed input, unresolvable label, strategy-forbidden shape) presents the one decode-failure exception type rather than leaking Strawberry's internal error class ([`docs/feedback.md`][feedback] P2). It splits the decoded `(type_name, node_id)` and runs a **two-step resolve-then-enforce** dispatch.
...
**Step 2 — enforce the candidate's effective strategy permits the payload shape** ([`docs/feedback.md`][feedback] P1). Read the candidate's **recorded effective strategy** (stamped on the [`DjangoTypeDefinition`][definition] at finalization, [Decision 10](#decision-10--resolve_typename-injection-via-the-__func__-identity-test-at-phase-25)) and reject a shape the strategy does not emit:
...
An `install_globalid_typename_resolver(type_cls, definition)` step runs in [`finalize_django_types`][glossary-finalize_django_types] Phase 2.5, alongside `install_relay_node_resolvers`, for every Relay-Node-shaped type. It runs once per type, **before** installing anything:
...
A sibling **model-label-routing audit** (P1-b) runs after every type's effective strategy is recorded — see [Decision 8](#decision-8--decode-routes-through-djangos-app-registry-then-the-framework-registry-to-the-primary-type).
...
| 2 — encode seam: `resolve_typename` injection + four encoders + override→`custom` + model-label-routing audit + default flip | [`django_strawberry_framework/types/relay.py`][relay] (encode helper + `install_globalid_typename_resolver` + non-`str` callable guard), [`django_strawberry_framework/types/definition.py`][definition] (recorded effective-strategy field), [`django_strawberry_framework/types/finalizer.py`][finalizer] (Phase 2.5 install + the model-label-routing audit), [`tests/types/test_relay_interfaces.py`][test-relay-interfaces] (extend) | ~14 (each strategy's type-name slot; override→`custom` + recording; both-declared `ConfigurationError`; non-`str` callable `ConfigurationError`; model-label-routing audit `ConfigurationError`; default flip → `model`; `type` reproduces pre-`0.0.9` payload) | `+230 / -6` |
