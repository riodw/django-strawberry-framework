...
4. Pin the cursor contract as a Relay-spec conformance suite — `first: 0`, overrun `first`, stale `after` does not error, `first`+`last` rejection, four-field `pageInfo` correctness including unrequested `hasNextPage`, backward pagination — against both root and synthesized relation connections, without changing the delegated Strawberry mechanics and without asserting keyset-cursor stability the offset implementation deliberately defers (Slice 4, [Decision 9](#decision-9--cursor-mechanics-stay-delegated-to-strawberry-this-card-pins-the-conformance-contract)).
...
- [`Meta.relation_shapes`][glossary-djangotype] that is not a `dict[str, str]`, carries a value outside `{"list", "connection", "both"}`, names an unknown / non-relation / single-valued field, names a **consumer-authored** relation (overrides own the field's shape), is declared on a non-Relay-Node type, or explicitly requests a connection over a non-Node-shaped target → [`ConfigurationError`][glossary-configurationerror] (shape, key-name, and consumer-authored failures at type creation; target-shape failures at finalization, where relation targets are settled).
...
- **Live primary home** (per the [`test_query/README.md`][fakeshop-test-query-readme] coverage rule; runs against the already-shipped `allLibraryGenresConnection`, so it has no dependency on the Slice-6 activation): `test_first_zero`, `test_first_overrun`, `test_stale_after_cursor_no_error` (pins ONLY that the query succeeds after a row deletion — no skip/duplicate/next-row assertion, Revision 2 P1), `test_first_and_last_rejected` (re-affirmation), `test_page_info_four_fields`, `test_has_next_page_correct_when_unrequested`, `test_backward_pagination_last_before`.
...
- `test_node_malformed_id_live` — a malformed / unresolvable id posted to `/graphql/` surfaces `GLOBALID_INVALID`, not a 500.
...
- `test_genres_connection_cursor_round_trip` / `test_genres_connection_total_count` — `endCursor` → `after` continuation; `totalCount` against the created-row count.
- `test_genre_books_connection_behavior` — nested reverse-M2M `booksConnection` pagination + row correctness (**behavior only**; SQL-shape assertions are [`WIP-ALPHA-033-0.0.9`][kanban]'s).
