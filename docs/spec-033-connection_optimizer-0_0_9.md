...
- [ ] Slice 2: connection-class fast path (per [Decision 5](#decision-5--connection-class-fast-path-with-annotation-presence-detection-and-a-per-parent-fallback))
  - [ ] [`connection.py::_build_relation_connection_resolver`][connection] gains only the handoff branch: when `getattr(root, "_dst_<field>_connection", None)` is a list whose members carry `_dst_row_number` / `_dst_total_count`, and the resolver's **own** call kwargs carry no `filter` / `order_by`, return an internal windowed-row wrapper. It does **not** return a prebuilt connection instance; Strawberry's `ConnectionExtension` still calls the generated connection class's `resolve_connection`. If sidecar kwargs are present, the `to_attr` is ignored and the shipped pipeline runs so a future planner/argument desync cannot serve unfiltered wrong data.
...
- probe `getattr(root, "_dst_<field>_connection", None)`;
...
When the wrapper is absent, the generated connection class delegates to the shipped path — correctness never depends on the plan having fired (no optimizer installed, strictness off, a fallback shape from [Decision 6](#decision-6--fallback-shapes-sidecar-input-divergent-aliases-hints-and-scalar-only-connections), a consumer's own prefetch: all degrade to today's behavior).
...
Three nested-connection shapes are deliberately **not** window-planned in `0.0.9`; each resolves per-parent through the shipped pipeline (and is therefore visible to strictness as unplanned):
...
| --- | --- | --- | --- |
| 1 — plan-side foundation: helpers consolidation, synthesis metadata, argument-aware aliases, union context publish, walker recognition, windowed `Prefetch` | [`optimizer/walker.py`][walker] (helpers in + `_plan_connection_relation` + argument-preserving alias classification), [`optimizer/plans.py`][plans] (`window_partition_for_prefetch`, `apply_window_pagination`, order-rule hoist), [`optimizer/extension.py`][extension] (helper imports + union publish), [`connection.py`][connection] (order-rule import-back), [`types/definition.py`][definition] (`relation_connections` slot), [`types/finalizer.py`][finalizer] (slot write), [`tests/optimizer/test_walker.py`][test-opt-walker] + [`tests/optimizer/test_plans.py`][test-opt-plans] + [`tests/optimizer/test_extension.py`][test-opt-extension] (extend) | ~24 (recognition via definition slot; windowed-prefetch plan content incl. `to_attr` / annotations / slice filters / deterministic order; reverse-FK + forward/reverse-M2M partition helper incl. missing-`related_name` reverse query name; shared-child M2M proof; malformed slice fallback; scalar-only planning; first/after, last-only reverse, literal + variable args; recursion two-level; fallback non-planning ×3; divergent-alias wrong-plan absence; connector + ordering columns in `only()`; subquery-wrap survival for `only()` and child `select_related`; `"both"` + consumer accessor prefetch coexistence; union-publish no-regression; helper-move no-regression) | `+520 / -60` |
| 2 — connection-class fast path | [`connection.py`][connection] (windowed-row wrapper + generated connection-class consumption), [`tests/test_relay_connection.py`][test-relay-connection] (extend) | ~15 (through-schema fast-path pin; fast-path single-query pin; wire parity vs pipeline path — edges/cursors/pageInfo/totalCount; cursor cross-path round-trip; last-only parity; annotated `totalCount` bypasses non-queryset guard; `first: 0` / overshot `after:` ambiguous-empty fallback parity; sidecar kwargs ignore window; annotations-missing fallback; consumer-prefetch fallback; optimizer-absent fallback) | `+230 / -20` |
...
- `test_fallback_not_planned_sidecar_input` / `..._divergent_aliases` / `..._skip_hint` — no window, no `planned_resolver_keys` entry; the divergent-alias test also asserts the wrong `Prefetch` is absent after argument-aware merge/classification.
...
4. The walker plans every recognized nested connection as a windowed `Prefetch` under `_dst_<field>_connection` — child plan with visibility / projections / recursion, deterministic order shared with the pipeline via the hoisted helper, relation-kind-correct partition key, slice window from resolved arguments capped by `relay_max_results`, `_dst_row_number` / `_dst_total_count` annotations, resolver keys recorded — and leaves the three [Decision 6](#decision-6--fallback-shapes-sidecar-input-divergent-aliases-hints-and-scalar-only-connections) fallback shapes unplanned. Argument-aware alias handling, scalar-only connection planning, malformed-slice error locality, shared-child M2M partitioning, subquery-wrap survival, union context publication, and `"both"` + consumer prefetch coexistence are pinned. [`tests/optimizer/test_walker.py`][test-opt-walker] / [`test_plans.py`][test-opt-plans] / [`test_extension.py`][test-opt-extension] cover the slice.

**Slice 2 — connection-class fast path**

5. The synthesized relation connection resolver returns an internal windowed-row wrapper, and the generated connection class serves edges / cursors / `pageInfo` / `totalCount` from the window annotations, with wire parity against the pipeline path, cursor cross-path compatibility, last-only parity, annotated `totalCount` bypassing the non-queryset count guard, through-schema proof under Strawberry's `ConnectionExtension`, sidecar kwargs refusing the window, and per-parent fallback for ambiguous empty windows ([Decision 5](#decision-5--connection-class-fast-path-with-annotation-presence-detection-and-a-per-parent-fallback)). One window query per relation per request; zero per-parent queries on unambiguous fast-path pages. [`tests/test_relay_connection.py`][test-relay-connection] covers the slice.
