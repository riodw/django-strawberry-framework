...
- [`Meta.connection`][glossary-metaconnection] — the target-type `total_count` opt-in; a windowed nested connection serves `totalCount` from the `_dst_total_count` annotation, including metadata-anchor rows for zero-edge windows, instead of a per-parent `COUNT` ([Decision 4](#decision-4--windowed-prefetch-planning-under-a-package-reserved-to_attr) / [Decision 5](#decision-5--connection-class-fast-path-with-annotation-presence-detection-and-a-per-parent-fallback)).
...
- [ ] Slice 1: plan-side foundation — synthesis metadata, argument-aware alias handling, walker recognition, relation-kind-correct windowed-`Prefetch` planning (per [Decision 3](#decision-3--walker-recognition-is-definition-metadata-driven-not-name-pattern-guessing) / [Decision 4](#decision-4--windowed-prefetch-planning-under-a-package-reserved-to_attr) / [Decision 9](#decision-9--the-edgesnode-selection-helpers-consolidate-into-the-walker))
  ...
  - [ ] [`optimizer/plans.py`][plans] gains the window helpers — `window_partition_for_prefetch(field)` deriving the relation-kind-specific parent partition key, and `apply_window_pagination(queryset, *, partition_by, order_by, offset, limit, reverse, include_metadata_anchor)` annotating `_dst_row_number` (`RowNumber()` partitioned by the parent key), `_dst_total_count` (`Count(1)` partitioned the same way), and `_dst_edge_in_window` (true only for rows that should become edges) before filtering to the requested edge window plus, when `pageInfo` / `totalCount` need zero-row metadata, one metadata anchor row per parent — the mechanism port of `/Users/riordenweber/projects/strawberry-django-main/strawberry_django/pagination.py::apply_window_pagination` (itself based on Django's own approach in <https://github.com/django/django/pull/15957>) with the package's zero-edge metadata correction.
...
- [ ] Slice 2: connection-class fast path (per [Decision 5](#decision-5--connection-class-fast-path-with-annotation-presence-detection-and-a-per-parent-fallback))
  - [ ] [`connection.py::_build_relation_connection_resolver`][connection] gains only the handoff branch: when `getattr(root, "_dst_<field>_connection", None)` is a list whose members carry `_dst_row_number` / `_dst_total_count`, return an internal windowed-row wrapper that exposes edge rows separately from metadata-anchor rows. It does **not** return a prebuilt connection instance; Strawberry's `ConnectionExtension` still calls the generated connection class's `resolve_connection`.
  - [ ] [`connection.py::DjangoConnection.resolve_connection`][connection] and the generated `<TypeName>Connection.resolve_connection` path detect the internal wrapper after the existing `first`+`last` guard, build edges via the generated edge class's `resolve_edge(..., cursor=<zero_based_offset>)`, derive `pageInfo` from wrapper metadata, attach `totalCount` (when the target's [`Meta.connection`][glossary-metaconnection] opted in and the field is selected) from `_dst_total_count`, and skip the per-parent pipeline entirely. Rows absent, annotations missing, or wrapper absent → the shipped per-parent pipeline runs unchanged (the fallback is the pre-card behavior, never an error).
...
- [ ] Slice 4: strictness wiring for connection paths (per [Decision 8](#decision-8--strictness-mode-wiring-for-connection-paths))
  - [ ] [`connection.py::_build_relation_connection_resolver`][connection] consults the [`_context.py`][context] sentinels before running the per-parent pipeline: when `DST_OPTIMIZER_PLANNED` is stashed (an optimizer ran with strictness active), the connection field's resolver key is absent from it, and the fast-path `to_attr` is absent on `root`, fire the strictness contract — `OptimizerError` under `"raise"`, a logged warning under `"warn"` — reusing the [`types/resolvers.py::_check_n1`][resolvers] machinery (parameterized for the `to_attr` probe and the generated field name) rather than duplicating it. Nested fallback connection pipeline runs publish optimizer sentinels with scoped save/restore semantics, so a child connection plan cannot overwrite the parent resolver's planned set. The deliberately-ABSENT comment block in the resolver docstring (the [`spec-032`][spec-032] pre-`033` posture) is removed in the same change.
...
2. **Consume the window.** The synthesized relation connection resolver returns annotated rows through an internal wrapper, and the generated connection class serves `edges` (offset cursors from `_dst_row_number`), `pageInfo`, and `totalCount` (from `_dst_total_count`, when the target opted in) directly from that wrapper — including zero-edge windows via a metadata-anchor row — one window query per relation per request, zero per-parent queries — falling back to the shipped per-parent pipeline whenever the window is absent (Slice 2, [Decision 5](#decision-5--connection-class-fast-path-with-annotation-presence-detection-and-a-per-parent-fallback)).
...
- **Partition key**: [`optimizer/plans.py`][plans] owns `window_partition_for_prefetch(field)`, which returns the parent-side key Django's prefetch attach uses, not a vague "related connector." Reverse FK partitions by the child table's FK column to the parent. Forward and reverse M2M partition by the through table's parent-side FK selected through Django's prefetch join; child pk is explicitly wrong because the same child can belong to multiple parents and would otherwise get one global page. Unsupported relation kinds raise a package-internal planning error and fall back unplanned rather than guessing.
- **Window**: `_dst_row_number = RowNumber() OVER (PARTITION BY <parent key> ORDER BY <deterministic order>)`, `_dst_total_count = Count(1) OVER (PARTITION BY <parent key>)`, and `_dst_edge_in_window = <row belongs to requested edge range>` with filters for the edge window — the `plans.py` helper ported from upstream's `apply_window_pagination`, including the reversed-row-number branch for `last`-only backward pagination.
- **Zero-edge metadata**: when `pageInfo` or `totalCount` is selected, the window query also admits one metadata-anchor row per parent (`_dst_edge_in_window = False`) so `first: 0` over a non-empty relation and "after the end" pages still have `_dst_total_count` / row-number metadata without per-parent fallback. The generated connection class filters anchors out of `edges`; it uses them only for count and pageInfo calculations. Parents with no related rows still produce an empty list and therefore `totalCount = 0`.
...
- when the value is a list whose rows carry `_dst_row_number` / `_dst_total_count`, wrap it in an internal `_WindowedConnectionRows` list/sentinel that separates edge rows (`_dst_edge_in_window = True`) from metadata-anchor rows and stores enough metadata to answer `pageInfo` / `totalCount`;
...
- edges are built through the generated edge class's `resolve_edge(..., cursor=<zero_based_offset>)`, where the cursor offset is `_dst_row_number - 1`; the cursor prefix stays owned by Strawberry's edge type;
- `pageInfo` is derived from the edge rows plus the metadata anchor when one exists, so empty edge pages over non-empty relations preserve the shipped root contract (`first: 0` still reports a next page when rows exist, and cursors stay `None` when no edge exists);
- `totalCount` (when the target's [`Meta.connection`][glossary-metaconnection] opted in and the field is selected) is read from `_dst_total_count` on any annotated edge or metadata row; only a parent with no related rows resolves `0` from an empty wrapper;
...
When the wrapper is absent, the generated connection class delegates to the shipped path — correctness never depends on the plan having fired (no optimizer installed, strictness off, a fallback shape from [Decision 6](#decision-6--fallback-shapes-sidecar-input-divergent-aliases-hints-pageinfo-only), a consumer's own prefetch: all degrade to today's behavior).
...
- Pagination-argument consistency is structural: the window was computed from the *same selection's* resolved arguments `resolve_connection` receives, so cursors and `pageInfo` derived from row numbers agree with what the pipeline would have produced.
...
Fallback nested connections still call the root connection pipeline per parent. [`optimizer/extension.py::apply_connection_optimization`][extension] therefore publishes `DST_OPTIMIZER_PLAN`, `DST_OPTIMIZER_PLANNED`, and sibling sentinels with scoped save/restore semantics when a connection pipeline runs inside an already-active optimization context. A child fallback connection can warn and continue without replacing the parent plan's sentinels for later sibling resolvers.
...
- **Scoped publication preserves parent strictness**: nested fallback pipelines are real optimizer runs, but their sentinels are child-scoped. The parent resolver's planned set must still be visible after the nested connection returns, especially in `"warn"` mode where execution continues.
...
| --- | --- | --- | --- |
| 1 — plan-side foundation: helpers consolidation, synthesis metadata, argument-aware aliases, walker recognition, windowed `Prefetch` | [`optimizer/walker.py`][walker] (helpers in + `_plan_connection_relation` + argument-preserving alias classification), [`optimizer/plans.py`][plans] (`window_partition_for_prefetch`, `apply_window_pagination`, metadata anchor, order-rule hoist), [`optimizer/extension.py`][extension] (import shuffle only), [`connection.py`][connection] (order-rule import-back), [`types/definition.py`][definition] (`relation_connections` slot), [`types/finalizer.py`][finalizer] (slot write), [`tests/optimizer/test_walker.py`][test-opt-walker] + [`tests/optimizer/test_plans.py`][test-opt-plans] (extend) | ~20 (recognition via definition slot; windowed-prefetch plan content incl. `to_attr` / annotations / slice filters / deterministic order; reverse-FK + forward/reverse-M2M partition helper; shared-child M2M proof; first/after, last-only reverse, literal + variable args; recursion two-level; fallback non-planning ×4; divergent-alias wrong-plan absence; connector + ordering columns in `only()`; `"both"` + consumer accessor prefetch coexistence; helper-move no-regression) | `+470 / -60` |
| 2 — connection-class fast path | [`connection.py`][connection] (windowed-row wrapper + generated connection-class consumption), [`tests/test_relay_connection.py`][test-relay-connection] (extend) | ~14 (through-schema fast-path pin; fast-path single-query pin; wire parity vs pipeline path — edges/cursors/pageInfo/totalCount; cursor cross-path round-trip; `first: 0` / after-end metadata correctness; metadata-anchor rows excluded from edges; annotations-missing fallback; consumer-prefetch fallback; optimizer-absent fallback) | `+220 / -20` |
| 3 — plan-cache key hygiene | [`optimizer/extension.py`][extension] (`_build_cache_key` pagination-variable collection with response-depth-preserving fragment traversal), [`tests/optimizer/test_extension.py`][test-opt-extension] (extend) | ~7 (nested `first: $n` distinct plans per value; root `first: $n` shared plan; root-in-fragment negative; mixed; fragment-carried nested args; superset rule pin; B1 hit-path no-regression) | `+110 / -5` |
| 4 — strictness wiring | [`connection.py`][connection] (sentinel consultation), [`types/resolvers.py`][resolvers] (`_check_n1` parameterization), [`optimizer/extension.py`][extension] (scoped publish/restore for nested connection pipeline sentinels), [`tests/test_relay_connection.py`][test-relay-connection] + [`tests/optimizer/test_extension.py`][test-opt-extension] (extend) | ~8 (planned→silent; window-served→silent; unplanned `"raise"`→`OptimizerError`; `"warn"`→log; warn-mode nested fallback does not clobber parent planned siblings; `"off"`/no-optimizer→no-op; sidecar-fallback flags; list-relation `_check_n1` no-regression) | `+105 / -15` |
...
- **Identical-argument aliases merge; divergent ones fall back.** The walker preserves enough per-response-key argument state to compare resolved pagination arguments before window planning; divergent aliases assert the wrong `Prefetch` and wrong `planned_resolver_keys` entry are absent, not merely that the query still resolves ([Decision 6](#decision-6--fallback-shapes-sidecar-input-divergent-aliases-hints-pageinfo-only)).
...
- **`first: 0`** — edge rows are empty, but the metadata anchor keeps `totalCount` and `pageInfo` aligned with the shipped root contract (`totalCount` is the pre-slice count, `hasNextPage` reflects available rows, cursors are `None`). Pinned.
- **Parents with no related rows** — empty `to_attr` list and no metadata anchor; `totalCount` is `0`, both pageInfo booleans are `False`, and no fallback is triggered (the empty list IS the window result).
...
- `test_window_partition_for_reverse_fk_forward_m2m_reverse_m2m` — the helper returns the parent-side partition key Django prefetch attach uses for every supported relation kind.
...
- `test_fallback_not_planned_sidecar_input` / `..._divergent_aliases` / `..._skip_hint` / `..._pageinfo_only` — no window, no `planned_resolver_keys` entry; the divergent-alias test also asserts the wrong `Prefetch` is absent after argument-aware merge/classification.
...
- `test_apply_window_pagination_unit` (plans.py) — annotation names, partition, range filters, reverse, `_dst_edge_in_window`, metadata-anchor rows; `test_deterministic_order_helper_hoist_parity` — the hoisted rule answers identically to the previous `connection.py` implementation.
...
- `test_fast_path_total_count_from_annotation_no_query` / `test_fast_path_first_zero_total_count_and_pageinfo_from_metadata_anchor` / `test_fast_path_after_end_total_count_and_pageinfo_from_metadata_anchor`.
- `test_fast_path_metadata_anchor_not_returned_as_edge`.
...
- `test_strictness_warn_nested_fallback_restores_parent_plan_context` — a warn-mode fallback nested connection followed by a parent-level planned relation sibling does not warn or raise for the sibling.
...
4. The walker plans every recognized nested connection as a windowed `Prefetch` under `_dst_<field>_connection` — child plan with visibility / projections / recursion, deterministic order shared with the pipeline via the hoisted helper, relation-kind-correct partition key, slice window from resolved arguments capped by `relay_max_results`, `_dst_row_number` / `_dst_total_count` / `_dst_edge_in_window` annotations, metadata-anchor rows when needed, resolver keys recorded — and leaves the four [Decision 6](#decision-6--fallback-shapes-sidecar-input-divergent-aliases-hints-pageinfo-only) fallback shapes unplanned. Argument-aware alias handling, shared-child M2M partitioning, and `"both"` + consumer prefetch coexistence are pinned. [`tests/optimizer/test_walker.py`][test-opt-walker] / [`test_plans.py`][test-opt-plans] cover the slice.

**Slice 2 — connection-class fast path**

5. The synthesized relation connection resolver returns an internal windowed-row wrapper, and the generated connection class serves edges / cursors / `pageInfo` / `totalCount` from the window annotations, with wire parity against the pipeline path, cursor cross-path compatibility, metadata-anchor correctness for zero-edge windows, through-schema proof under Strawberry's `ConnectionExtension`, and silent fallback when the window is absent ([Decision 5](#decision-5--connection-class-fast-path-with-annotation-presence-detection-and-a-per-parent-fallback)). One window query per relation per request; zero per-parent queries on the fast path. [`tests/test_relay_connection.py`][test-relay-connection] covers the slice.

**Slice 3 — cache keys**

6. Nested pagination variables key the plan cache; root pagination variables do not, including root connections selected through fragments; nested connections selected through fragments do key; the superset collection rule and the B1 properties (hit/miss, LRU, immutability) are pinned ([Decision 7](#decision-7--plan-cache-key-hygiene-nested-pagination-variables-hash-root-pagination-arguments-do-not)). [`tests/optimizer/test_extension.py`][test-opt-extension] covers the slice.

**Slice 4 — strictness**

7. An unplanned, unserved nested-connection access fires `OptimizerError` / a warning per the mode through the parameterized `_check_n1`; planned, window-served, strictness-off, and optimizer-absent paths are silent; nested fallback connection pipelines restore the parent strictness context after publishing child sentinels; the list-relation strictness suite passes unmodified ([Decision 8](#decision-8--strictness-mode-wiring-for-connection-paths)).
