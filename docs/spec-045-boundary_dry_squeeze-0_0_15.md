...
([Decision 11](#decision-11--lone-card-at-0015--slice-5-owns-the-version-cut)).
...
- [ ] **Slice 5 — Docs fold-in + `0.0.15` cut + card wrap**
...
- Card `WIP-ALPHA-044-0.0.14`
  ...
  the `0.0.14` joint cut; this card's `0.0.15` work begins after that cut
  lands ([Risks](#risks-and-open-questions)).
...
already installs. `[project].dependencies` is untouched; the runtime guards
...
### Decision 11 — Lone card at `0.0.15` — Slice 5 owns the version cut

Per the Step 3 scan, this card is the **only** non-Done card at `0.0.15`
(its board neighbors are `0.0.14` — the [joint version
cut][glossary-joint-version-cut] card `044` owns that line — and the `0.1.x`
beta queue). So this spec mirrors the lone-card shape (spec-038 Decision 14,
...
- **Sequencing behind spec-044**: card 044 owns the `0.0.14` cut and its
  TODO anchors sit on the version-quintet sites. This card must not start
  Slice 5 (nor place its own quintet anchors) until that cut lands.
  Preferred answer: begin Slices 1–2 immediately (they touch none of the
  quintet); hold Slice 5 behind the `0.0.14` release. Fallback: if `0.0.14`
  stalls, the maintainer may re-order the cuts explicitly.
...
- The beta-release cleanup card (now `TODO-BETA-046-0.1.0` after the
  renumber) — this card's squeeze does not absorb its verification scope.
...
- [ ] Slice 5 shipped: version quintet at `0.0.15`, GLOSSARY flips,
