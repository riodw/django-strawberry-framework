...
Slice O2 — Selection-tree walker. New module `optimizer/walker.py`. A pure function `plan_optimizations(info, model) -> OptimizationPlan` that walks `info.selected_fields[0].selections` recursively, mapping each relation selection to a `Prefetch` chain and each scalar selection to an `only()` column. Tested with synthetic `info` objects (no Strawberry execution required) so the walker is unit-testable in isolation.
