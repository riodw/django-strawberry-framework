...
## User-facing API
The shipped consumer surface in `0.0.5` is still the `class DjangoType` + `class Meta` API. No new public exports are added.
...
Generated defaults must not clobber user-authored `resolve_id_attr`, `resolve_id`, `resolve_node`, or `resolve_nodes`. The override discriminator is the `__func__` identity check from Decision 3, not a simple `cls.__dict__` check.
...
Internal helper surface sketch:
...
The exact public method signatures attached to the class must match Strawberry's `relay.Node` expectations. The helper names above are internal implementation anchors; they are not new public exports.
...
Required invariants:
- Selecting Relay `id` preserves enough model primary-key data for `resolve_id`.
- `only()` projection includes the concrete primary-key attname when Relay `id` is selected.
...
Implementation consequence: keep the primary key in `DjangoTypeDefinition.field_map` even when the normal scalar `id` annotation is suppressed for Relay node types. The suppression is GraphQL-annotation-only; optimizer metadata still treats the primary key as a connector column.

### Decision 8: registry implications
...
This is the primary implementation plan for `0.0.5`. The slice is small enough to implement as a single PR but easier to review as five commits. Each commit cites the exact file:line touched.
...
### Optimizer / projection coverage (extend existing optimizer tests)
- Selecting Relay `id` on a node type includes the model primary-key column in `only()` projection even though the scalar `id` annotation is suppressed.
- Relation traversal through Relay-declared types still preserves connector columns.
- `resolve_id` uses the loaded primary-key value without triggering an avoidable lazy load when the optimizer already selected it.
...
4. Tests in `tests/types/test_relay_interfaces.py` (new), and the extensions to `tests/types/test_definition_order_schema.py`, `tests/test_registry.py`, optimizer/projection coverage, and `examples/fakeshop/test_query/test_library_api.py` listed in the Test plan all pass.
