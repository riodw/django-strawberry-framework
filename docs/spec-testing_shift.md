...
## Implemented outcome
The example project now has a real `library` app at `examples/fakeshop/library/`. Its models are `Branch`, `Shelf`, `Genre`, `Book`, `Patron`, `MembershipCard`, and `Loan`, covering forward FK, reverse FK, forward OneToOne, reverse OneToOne, forward M2M, reverse M2M, a choice field, and a nullable scalar field.
The fakeshop project was flattened to `examples/fakeshop/settings.py`, `schema.py`, `urls.py`, `wsgi.py`, with `products/` and `library/` as sibling app packages. `pytest.ini` sets `DJANGO_SETTINGS_MODULE = settings` and `pythonpath = examples/fakeshop`.
The project schema in `examples/fakeshop/schema.py` imports each app's `Query`, composes the top-level `Query`, calls `finalize_django_types()` once after all example `DjangoType`s are imported, then constructs `strawberry.Schema(query=Query, extensions=[DjangoOptimizerExtension()])`.
...
The autouse `_reload_project_schema_for_acceptance_tests` fixture is load-bearing. Package tests clear the global registry for isolation; the HTTP suite therefore clears the registry, reloads `library.schema`, reloads the project `schema`, reloads `urls`, and clears URL caches so cached schema modules never hold stale `DjangoType` classes.
...
Package tests now use real example-project models from `products.models` and `library.models` instead of a test-only cardinality fixture app. That coupling is intentional: those example model surfaces are part of the framework test substrate.
## Resolved risks and decisions
The app name and placement are settled: `library` under the flattened fakeshop project.
The schema finalization seam is settled: one project-level `finalize_django_types()` call in `examples/fakeshop/schema.py`.
