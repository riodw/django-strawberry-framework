...
| [`test_products_visibility_api.py`][test-products-visibility-api] | Generated relations enforce target visibility themselves; sync + async mounts, holder schema; optimizer-less nested connection costs `1 + N`; Relay id-only page in one query; cascading-hook target blocks FK-id elision; a PLANNED relation is scoped by the plan alone (list and connection vocabularies, anonymous and staff), the resolver having stood down. |
