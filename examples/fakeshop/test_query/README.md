...
| [`test_uploads_api.py`][test-uploads-api] | `Upload` over multipart: file / image output objects, filesystem-path opt-in, model + form write paths, `updateMediaSpecimen` omit / replace / explicit `null` on the required file and its optional partial-input `Upload`, storage degradation (unpathable backend, vanished file, corrupt image, `SuspiciousFileOperation` not swallowed). |
