...
What moved: the extension under test is the one `config.urls` serves, reached via `sys.modules["config.schema"]` at call time (the conftest rebuilds that module before every test; a module-level import binds a discarded instance and reads zeros forever, passing). The obvious document (`allLibraryBooks { shelf { code } }`) measured `misses=2, size=0`: `BookType` / `ShelfType` declare `get_queryset`, and a plan w/ a hook in the selection is `cacheable=False`. Throwaway types had no hooks so the package row never met this. SQL-identity clause added: a stale reused plan would change the emitted SQL. Don't copy the stand-in's wiring: a plain `strawberry.Schema` gives `lambda: ext` no per-operation isolation, so every operation shares one extension instance's per-operation state and execution context; `config/schema.py`'s `DjangoSchema` isolates both on the shared instance, which is what makes the singleton factory safe.

Failability: `extensions=[lambda: _optimizer]` → fresh instance per request → row fails.

### 2. SQL beats the plan object: the hint pair

`LoanType.Meta.optimizer_hints = {"book": OptimizerHint.prefetch_related(), "patron": OptimizerHint(skip=True)}` ships in [`apps/library/schema.py`][lib-schema]. Two package rows redeclared the same hints on a throwaway `ItemType` and read `ctx.dst_optimizer_plan`.
...
def test_sessionless_login_refuses_with_the_error_naming_both_middleware_stacks():
    ...
    with override_settings(**_SESSIONLESS_PASS_THROUGH):
        response = _post_graphql(
            _LOGIN, client=Client(), variables={"u": "staff_1", "p": TEST_USER_PASSWORD}
        )
...
Deleted this sweep, w/ the sibling that covered them: `tests/test_error_policy.py` opt-out + value-completion rows (→ `/ep-off/`, `/ep/` mounts in [`test_error_policy_api.py`][test-error-policy-api]); `tests/extensions/test_debug.py` empty-lists + fresh-per-operation rows; `tests/test_schema.py` four `execute_*` rows (success half everywhere; error half pinned exactly by the policy rows); `tests/test_schema.py::test_get_extensions_sync_and_async` (cardinality anti-pattern; install-position row pins both policies by name); `::test_get_extensions_with_custom_factory_dedup` (→ `/ep-factory/` row in [`test_error_policy_api.py`][test-error-policy-api]; package identity stays in `test_a_callable_policy_entry_cannot_become_the_masking_authority`); `::test_a_factory_configured_bound_is_the_one_a_request_is_held_to` (→ `/rp-entry-rows/` in [`test_resource_policy_api.py`][test-resource-policy-api]); `::test_a_resolver_cannot_disarm_enforcement_by_emptying_the_extension_list` and `::test_a_resolver_cannot_nominate_a_wider_policy_by_replacing_the_extension_list` (→ `/rp-authority/` widen / nominate); `::test_a_retained_policy_argument_cannot_widen_a_later_request` (→ `/rp-retained/`); `tests/test_relay_node_field.py` typed node / mismatch / duplicates / empty (zero-query clause folded into `test_nodes_duplicates_and_empty_live`); `tests/test_relay_node_field.py` model-label / hidden / missing / malformed (3 ids) / uncoercible single+hole / typed `DjangoNodesField` resolve+mismatch / per-table query-count pair / malformed mid-batch / async `node`+`nodes` / hostile-seal trio / cascade node+batch (→ [`test_library_api.py`][test-library-api]); `tests/test_connection.py` negative / over-cap / malformed-cursor / first+last-through-schema (keep the classmethod-direct `test_first_and_last_raises_graphql_error`); `tests/test_strawberry_patches.py` two `parse_json` rows after folding `b"3.14"`, `b"false"`, `b'["not", "objects"]'` into the live parametrize lists, then the null-param / scalar-param / deep-body / deep-param rows (live twins in [`test_products_api.py`][test-products-api]); eight `tests/mutations/test_resolvers.py` rows, two `tests/mutations/test_permissions.py` rows, nine `tests/forms/test_resolvers.py` rows, one `tests/auth/test_queries.py` row, one `tests/auth/test_mutations.py` row (its non-redundant `PermissionDenied` arm went live, see 5); `tests/types/test_resolvers.py` file/image trio + many-side visibility row; `tests/test_scalars.py` two `strawberry_config` round-trips, serializer/parser happy-path stand-ins, the GraphQL-expressible string-rejection family, and `test_upload_field_resolves_under_strawberry_config_schema` (→ [`test_scalars_api.py`][test-scalars-api] / [`test_uploads_api.py`][test-uploads-api]); `tests/test_list_field.py` non-nullable-outer row; `tests/test_permissions.py` nested-cascade row; `tests/test_relay_connection.py` `first_zero`; `examples/fakeshop/apps/kanban/tests/test_mutations.py` whole module, 11 rows (→ [`test_kanban_mutations_api.py`][test-kanban-mutations-api], which folds every clause the in-process rows owned: `card.transitions…exists()`, `item.is_complete`, `attempt.outcome_id is None`, `attempt.ended_at`, `Decision(card__isnull=True)`, and `__all__` on an unknown lookup).

Kept, on purpose: `tests/types/test_resolvers.py::test_runtime_path_from_info_strips_list_indexes_and_keeps_aliases` has no wire shape (optimizer path tuple). Reverse-O2O custom visibility is live in [`test_products_visibility_api.py`][test-products-visibility-api].
...
Fixture gaps: none open. The protected-delete gap is closed: `products.ItemHold` carries a `PROTECT` and a `RESTRICT` reference to `Item`, and `test_products_api.py::test_delete_item_refused_by_a_hold_returns_the_protected_envelope` is the live row. The custom-pk gap is closed: `PatronProfile` carries a relation primary key and `test_library_shapes_api.py::test_patron_profile_relay_id_is_built_from_the_relation_primary_key_column` is the live row for the shape `tests/optimizer/test_relay_id_projection.py::test_relay_id_with_custom_pk_attname_avoids_lazy_load` stands in for. The mixed-case gap is closed: `Distributor` / `Consignment` carry a mixed-case scalar (`displayName`), forward key (`distributorRef`) and reverse `related_name` (`consignmentItems`), and the `test_library_shapes_api.py` rows `test_distributor_mixed_case_scalar_projects_its_own_column`, `test_consignment_mixed_case_forward_key_joins_its_target_in_one_statement` and `test_distributor_mixed_case_reverse_relation_loads_through_one_prefetch` replaced the two unmanaged-model rows `tests/optimizer/test_extension.py` carried for the shape.
...
- **Throwaway-schema internals.** Type-name `globalid_strategy`, multi-type-over-one-model `__typename` dispatch, `_stamp_node_type` isolation, custom `relay.NodeID` coercion/decode, construction- / finalize-time guards, consumer `resolve_nodes` override contracts, `SyncMisuseError.original_error` (not a wire field). [`tests/test_relay_node_field.py`][tests-relay-node-field]; live sibling [`test_library_api.py`][test-library-api].
- **No WebSocket mount.** Fakeshop has no `config/asgi.py`. [`tests/test_routers.py`][tests-routers] and [`tests/test_consumers.py`][tests-consumers] stay package-side (Channels scope, `WebsocketCommunicator`, actor-revalidation checkpoints). HTTP sibling [`test_transport_api.py`][test-transport-api].
...
- **Async.** [`test_list_field_async_api.py`][test-list-field-async-api], [`test_relations_async_api.py`][test-relations-async-api], [`test_products_api.py`][test-products-api] form-mutation colour: `django.test.AsyncClient` vs an `AsyncDjangoGraphQLView` mount. Async rows: `@pytest.mark.django_db(transaction=True)`, declare the helper exemption, rely on [`tests/conftest.py`][tests-conftest] to close per-task SQLite connections (else `ResourceWarning` = error here).
...
| Suite | Owns |
|---|---|
| [`test_library_api.py`][test-library-api] | Acceptance app end to end: FK / reverse FK / O2O / M2M traversal, choice enums, nullable scalars, optimizer SQL shape + hints (non-sentinel `skip=True`, reverse-O2O `card` JOIN in the root SQL, flag-gated `select_related()` book hint still prefetching past `BookType.get_queryset`), shipped plan cache reused across HTTP requests, consumer relation overrides, `filter:` / `orderBy:`, row-preserving correlated `EXISTS` for deep relational predicates, mixed direct/relational `OR` oracle over the seeded loan graph, flag-gated `allLibraryLoansConnection` proof, GlobalID rejection codes, shipped `node` / `nodes` / typed `genre(id:)` refetch (malformed / uncoercible / hidden / missing / mixed-batch holes, single-node missing row null with hidden-vs-missing query cost parity, per-table `nodes` query counts, typed `DjangoNodesField` on `/graphql-test/`, async colour on `/graphql-async/`), genre-connection pagination + `totalCount` gating + filter/order/slice composition, post-`FilterSet` seal on the shipped genre connection (every defect class refused by name w/ SQL capture, `super()` pass-through admitted), relation-seeded `booksConnection` `pageInfo` + `last`/`before`, nested `booksConnection` argument guards (`first` overrun clamping, stale `after` cursor no-error, `first` + `last` mutual exclusivity rejection), two to-many `orderBy` terms each w/ own aggregate, descending to-many `MAX(` SQL on `allLibraryBooks` plus async to-many `orderBy` on `allLibraryGenresViaListField` over `/graphql-async/`; custom-input model writes (`genres` replace / clear vs omit / `null` / wrong-type / uncoercible member, choice-enum unwrap on create + update), raw-pk M2M attach + nonexistent member on `createShelf`; serializer M2M `pk__in` batching (top-level `altBranches` and nested shelves) plus unauthorized `updateBookGenresViaSerializer` never querying `library_book_genres` while the granted path does snapshot it, after the permission read; consumer `Prefetch` children over the proxy-targeted relation `BranchNote.branch` -> `ProxyBranch` admitted for the proxy AND the concrete `Branch`, refused over an unrelated table, its unmasked seal wording readable under error-policy debug pass-through; flag-gated secondary `NullabilityOverrideBookType` Node emitting `library.book:<pk>` so `node(id:)` refetches as `BookType`; `ShelfType.books` binds to primary `BookType`, not that secondary; a target hook returning a sliced queryset refused with the typed `sliced` defect on a plain list relation before any child query runs, and its nested-connection control refused by the per-parent fallback's own read-surface seal after the parent query, which is what proves the walker still admits the shape the degrade path exists for. |
| [`test_library_inheritance_api.py`][test-library-inheritance-api] | The venue surface's model shapes: inherited `Venue` columns projected through the `venue_ptr` join in one statement (and through both joins for `SelfServeDesk`), unselected inherited columns kept out of the projection, `orderBy: openedOn` sorting by the parent table in both directions, desks hidden with their closed `Venue` through the parent-link cascade (transitively for the kiosk chain); `repairticket` / `venuebadge` / `venuesponsor` resolving through the default accessors of relations declared without `related_name`, one statement each; the nullable `lead_ticket` cycle cascading one way (unled venue kept by the `IS NULL` disjunct, venue with a withdrawn lead ticket hidden, `leadTicket { venue }` walking back); `allLibraryOpenVenues` served from the `OpenVenue` default manager against `allLibraryVenues` as control; a sign on a city-less branch hidden because the cascade composes the `VisibleBranch` target from its default manager while `sign.branch` still loads it; `CirculationDesk` publishing `children` / `profile` under a cascade scoped to `branch` / `shelf` (served despite its generic key, restricted-branch desk hidden from anonymous, shown to staff); `archiveGenres` absent from `BookInput` / `BookPartialInput` beside the editable `genres`; `MediaSpecimenInput` optional through `blank` (`optionalAttachment`) and through `null` (`spareImage`) separately, `attachment` still `Upload!`. |
| [`test_library_shapes_api.py`][test-library-shapes-api] | The publishing surface's relation shapes, one planner rule each: a Relay id minted from a relation primary key (`PatronProfile.patron`, one statement, no patron read), `<word>_<digit>` names projecting their real columns (`Edition.isbn_10` on a scalar, `Edition.publisher_2` on a forward key), two `strawberry.field(name=...)` names whose normalized spellings collide each returning their own column, forward-key id elision when the target key is a text column not named `id` (`Printing.edition` answered from `edition_id`) against its non-key control that joins `library_edition`, elision refused through a `to_field` key (`PatronProfile.favorite_genre` stores the genre name) and refused into a custom `relay.NodeID` (`PublisherType.house_code`, the id decoding to the house code), a digit-boundary reverse accessor paging two parents off one `ROW_NUMBER() OVER (PARTITION BY publisher_2_id)` statement, and a child default manager returning `.distinct()` leaving `annotationsConnection` unplanned - no window anywhere, one `library_annotation` statement per parent, pages still correct; mixed-case Django names resolving by their published spelling (`Distributor.displayName` projecting its own column, the forward key `Consignment.distributorRef` one `JOIN`, its reverse `consignmentItems` one prefetch on `distributorRef_id`, both relation counts at one and three parents). |
| [`test_products_api.py`][test-products-api] | Canonical consumer app: connection reads, all three mutation flavours, `DjangoModelPermission`, per-field filter permission gate, own-pk GlobalID filtering, `RelatedFilter` traversal, not-found-before-auth on hidden rows, private-row refetch after an authorized write, form stale-field revalidation, per-operation argument map by introspection, empty-`name` `full_clean` envelope, unresolvable relation id, form- / model- / serializer-mutation async colour on `/graphql-async/`, GET object / malformed `variables` params, dangling non-null FK, sync strict-UTF-8 rows, product-table statement total on cascade query count, `deleteItem` refused by an `ItemHold`'s `PROTECT` / `RESTRICT` reference, explicit `itemId: null` on the `blank=True, null=False` hold FK keyed to `itemId`, the DEBUG pass-through idiom. |
| [`test_products_visibility_api.py`][test-products-visibility-api] | Generated relations enforce target visibility themselves; sync + async mounts, holder schema; optimizer-less nested connection and raw-list reverse-FK each cost `1 + N`; reverse-O2O custom-visibility holder (planned and unplanned match; async colour); Relay id-only page in one query and `{ id name }` with decoded Relay ids clean under `strictness="raise"`; cascading-hook target blocks FK-id elision; a PLANNED relation is scoped by the plan alone (list and connection vocabularies, anonymous and staff), the resolver having stood down; and the converse, a relation the walker never planned on an execution whose `strictness="raise"` is armed anyway - the target loaded by a consumer `select_related`, re-checked per parent (non-null refusal for anonymous, served for staff, and a nullable holder key collapsing to `null` for the hidden row only). |
...
| [`test_scalars_api.py`][test-scalars-api], [`test_scalars_filter_api.py`][test-scalars-filter-api] | One specimen row per scalar wire format; `ScalarSpecimenType.id` as `Int` under `BigAutoField`; override-specimen SDL (`quantity: Float`, `note: String!`) plus resolved values; scalar filtering / ordering / related querysets, the declared `priceSpan` `RangeFilter` applied over the wire with its per-filterset scoped nested input names, out-of-range `__in` member drop, `BigInt` bool / float / malformed decimal-string literal rejection, canonical `BigInt` input literals including int64 bounds. |
| [`test_uploads_api.py`][test-uploads-api] | `Upload` over multipart: file / image output objects, filesystem-path opt-in, model + form write paths, `updateMediaSpecimen` omit / replace / explicit `null` on the required file and its optional partial-input `Upload`, its raw-pk `id:` (non-Relay primary; uncoercible / out-of-range / missing pk each the not-found `FieldError`), storage degradation (unpathable backend, vanished file, corrupt image, `SuspiciousFileOperation` not swallowed). |
| [`test_input_shapes_api.py`][test-input-shapes-api] | Relation-id and payload-slot shapes of generated write inputs: a forward key is `ID!` to a Relay primary (`ItemInput.categoryId`) and the raw `Int!` to a plain one (`BookInput.shelfId`, no `shelf` field), a many-to-many is an optional `[ID!]` to a Relay primary (`BookInput.genres`) and `[Int!]` to a plain one (`createShelf`'s `altBranches`), a Relay primary's payload (`CreateBookViaCustomInputPayload`) carries a nullable `node` with no `result` and a plain primary's (`createShelf`) a nullable `result` with no `node`, each beside `errors: [FieldError!]!`. |
| [`test_auth_api.py`][test-auth-api] | `login` / `logout` / `register` / `me` on the shipped `AllowAny` surface, plus a `/graphql-test/` holder arming optimizer `strictness="raise"` over the same mutations: session rotation branches, collapsed failure envelope incl. backend `PermissionDenied`, password validators, non-string `username` rejected at validation, sessionless middleware stack refused by name. |
| [`test_keyset_api.py`][test-keyset-api] | `Meta.cursor_field` keyset pagination: value cursors surviving inserts / deletes, nested uniform seeks, tamper rejection, permission-aware replay, cursor survives `SECRET_KEY` rotation via `SECRET_KEY_FALLBACKS`, nested window partitioned and counted over the viewer's rows, opaque deterministic cursors. |
| [`test_connection_pagination_api.py`][test-connection-pagination-api] | Pagination error containment on connections; `Meta.connection` `totalCount` opt-in gating + inherited connection SDL description; sidecar `filter:` / `orderBy:` argument presence on shipped connections; malformed post-`OrderSet` result matrix over both pipelines on the field shape each belongs to - `apply_sync` on the shipped `allLibraryGenresConnection` (sync public resolver) over `/graphql/`, each row = exact rejection + `library_genre` query cost, and `apply_async` on a test-local `GenreType` connection field whose `async def` consumer resolver is the only shape that selects the async pipeline, served over a module-owned `/graphql-async/` mount, each row = exact rejection + no raw exception text + a sentinel proving the override ran - each with a `super()` pass-through positive control; the async arm mirrors the sync shapes (evaluated, materialized-list, none, projection, wrong-model, sliced, combined, routing-rewritten-in-place, malformed-deferred-filter) and answers the sync-only `awaitable-in-sync` row with two twins, `non-awaitable` and `residual-awaitable`, the second also reading the refused awaitable's disposal (closed coroutine, body never run); the whole async matrix is mirrored for the `FilterSet` arm (`GenreFilter.apply_async`) on a request carrying a real `filter:` argument, the same defect shapes, sentinel, disposal read and typed messages, with a pass-through control asserting the filtered page. |
| [`test_list_field_api.py`][test-list-field-api] | Sync `DjangoListField` argument surface: ordered offset pages, visibility before order, cap + error matrix, converter naming, the effective-ordering precedence behind the positive-offset guard (random model default rejected, superseded dormant random accepted, each with its control), the unreadable-leaf rejections (`RawSQL`, a bare `Func`, a `Subquery`, and a fragment boxed inside a predicate's `__in` sequence) against their conditional-order control, malformed post-`OrderSet` result matrix (each row = exact rejection + query cost), hostile normalized-term rejection, legacy-reference byte-parity oracle under the same field name, consumer-context isolation for list field + ordered connection, the policy-authority boundary (a sibling field writing every policy object a resolver can reach, and the request after it). |
| [`test_list_field_async_api.py`][test-list-field-async-api] | Async colour of the same: safe queryset completion, optimizer preservation, iterable cleanup including a deadline the resolver ends across its own await, closing a source it never advanced, the offset guard's effective-ordering precedence witnessed at the compiler (an executor thread's connection is invisible to `CaptureQueriesContext`), async-only malformed shapes (non-awaitable result, residual awaitable), published-name reporting, post-`OrderSet` routing pin, consumer-context rows, the policy-authority boundary written across an await, and the nested synchronous operation an async request can start - the one shape where the ambient event loop and the GraphQL executor disagree, refused on the wire rather than answered with a generic completion failure. |
...
| [`test_multi_db.py`][test-multi-db] | Behind `FAKESHOP_SHARDED=1`: resolver isolation on `shard_b`, correlated `EXISTS` pinned to `queryset.db`, post-`OrderSet` routing attestation (four list-field rejections + the accepted identity-preserving row, plus the connection-field alias rejection), multi-DB debug capture. |
...
| [`test_error_policy_api.py`][test-error-policy-api] | Production error policy over the wire, including an entry that resolves to the masking authority refusing the request with the configuration code instead of masking it. |
| [`test_extension_isolation_api.py`][test-extension-isolation-api] | What one operation's extensions may be answered from: every write spelling aimed at the box an accepted slotted entry is held in plus a write to a stateful accepted factory (bound and masking both still enforced on the NEXT request, over the sync and async views), a consumer extension's own operation leaving the request its published optimizer plan, a factory resolving to an enforcement authority refusing every request - valid, aliased and malformed alike, and under an `operationName` that is absent, punctuated, empty or non-ASCII, over the sync and async views - with the stable configuration code in a JSON envelope that echoes nothing back, while its pre-parse token and depth ceiling still rejects an oversized or deeply nested document under any of those names, a factory resolving to anything but an ordinary extension instance (one that raises, returns a non-extension or a class, returns an object whose `__class__` claims an ordinary extension or an authority, or returns an authority subclass or hybrid) refusing every request with the configuration code and the reason in the package log only, and a worker that outlived its request bounded and publishing for itself over the async mount. |
...
| [`test_resource_policy_api.py`][test-resource-policy-api] | Every `ResourcePolicy` bound, against mounts each narrowing one family of bounds; malformed document keeps the parser's own diagnostic / is rejected on size first; the pre-parse token bound charged over the whole request document with its named-operation control; an entry-declared `max_list_rows` as the bound the request is held to; a many-side relation whose manager returns a `QuerySet` subclass, bounded on both transports with Django's own manager as the control; sync/async parity row; streamed multipart rows. |
| [`test_debug_extension_api.py`][test-debug-extension-api], [`test_debug_toolbar_api.py`][test-debug-toolbar-api] | `DjangoDebugExtension` on a probe mount; `DebugToolbarMiddleware` across GraphQL / panel / pass-through routes under `DEBUG=True` override: bridge script + handle on the IDE page, named-operation panel payload (`TemplatesPanel` skip, `RedirectsPanel` `title: null`), `Content-Length` refresh after `CommonMiddleware`, introspection skip, `Content-Encoding` early-out via an inner stamp middleware, untagged JSON holder view left untouched. |
| [`test_mutation_atomicity.py`][test-mutation-atomicity] | Generated mutation's transaction spans response completion: unserializable payload → rollback, not commit behind `data: null`; async colour of the same on `/graphql-async/`. |
| [`test_client_api.py`][test-client-api] | Request-driving half of the `TestClient` family's coverage. |
| [`test_kanban_api.py`][test-kanban-api], [`test_kanban_mutations_api.py`][test-kanban-mutations-api], [`test_glossary_api.py`][test-glossary-api] | Docs-as-data apps' read + write surfaces; the board is also the one example graph whose child model declares a single non-pk `Meta.ordering` column, so the nested `decisionsConnection` window is where the deterministic `ORDER BY <order column>, pk` is read off captured SQL for a two-parent batch. |
...
[tests-consumers]: ../../../tests/test_consumers.py
[tests-dir]: ../../../tests/
[tests-relay-node-field]: ../../../tests/test_relay_node_field.py
[tests-routers]: ../../../tests/test_routers.py
...
[test-input-shapes-api]: test_input_shapes_api.py
[test-keyset-api]: test_keyset_api.py
[test-library-api]: test_library_api.py
[test-library-inheritance-api]: test_library_inheritance_api.py
[test-library-shapes-api]: test_library_shapes_api.py
