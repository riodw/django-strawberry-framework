...
The project schema lives at `../config/schema.py`. It imports each app's `Query` (from all six apps — `library`, `products`, `scalars`, `kanban`, `glossary`, `accounts`) and the `Mutation`s contributed by `products`, `scalars`, `library`, and `accounts`, composes them into the top-level `Query` and `Mutation`, calls `finalize_django_types()`, then constructs `DjangoSchema(query=Query, mutation=Mutation, config=strawberry_config(), extensions=[lambda: _optimizer])` over a module-level `_optimizer = DjangoOptimizerExtension()` singleton. `DjangoSchema` rather than plain `strawberry.Schema` because the schema carries generated mutations, whose write transactions must span GraphQL response completion — the write pipeline refuses to run under a plain `Schema`.
