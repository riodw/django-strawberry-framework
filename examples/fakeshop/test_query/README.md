...
Fixture gaps (rung 2 work, one fakeshop change each; adding a model/migration is outside this sweep): one `on_delete=PROTECT` FK (`tests/mutations/test_resolvers.py::test_delete_refused_by_protected_reference_is_envelope_not_graphql_error`). The custom-pk gap is closed: `PatronProfile` carries a relation primary key and `test_library_shapes_api.py::test_patron_profile_relay_id_is_built_from_the_relation_primary_key_column` is the live row for the shape `tests/optimizer/test_relay_id_projection.py::test_relay_id_with_custom_pk_attname_avoids_lazy_load` stands in for.
