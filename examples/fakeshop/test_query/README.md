...
Two more cross-cutting suites round out the tier. `test_client_api.py` is the request-driving half of `testing/client.py`'s coverage: it posts real operations through the spec-043 `TestClient` / `AsyncTestClient` / `GraphQLTestCase` / `GraphQLTransactionTestCase` family (async multipart upload, `login()` brackets, the `GRAPHQL_URL` / per-call endpoint rungs against a real view), while the DB-free mechanics stay in the package tier at `../../../tests/testing/test_client.py`. `test_mutation_atomicity.py` pins the BETA-055 response-completion transaction contract: a generated mutation's transaction now stays open through graphql-core payload serialization, so a completion failure (an unserializable payload) rolls the write back instead of committing behind `data: null`. These began as `xfail(strict=True)` regressions; BETA-055 has landed, they XPASSed, and the markers were removed — the assertions now encode the shipped contract.
