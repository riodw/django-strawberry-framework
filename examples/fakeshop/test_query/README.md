...
| [`test_keyset_api.py`][test-keyset-api] | `Meta.cursor_field` keyset pagination: value cursors surviving inserts / deletes, nested uniform seeks, tamper rejection, permission-aware replay, cursor survives `SECRET_KEY` rotation via `SECRET_KEY_FALLBACKS`, nested window partitioned and counted over the viewer's rows. |
