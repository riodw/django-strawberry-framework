...
| [`test_list_field_async_api.py`][test-list-field-async-api] | Async colour of the same: safe queryset completion, optimizer preservation, iterable cleanup including a deadline rejection that closes a source it never advanced, the offset guard's effective-ordering precedence, async-only malformed shapes (non-awaitable result, residual awaitable), published-name reporting, post-`OrderSet` routing pin, consumer-context rows. |
