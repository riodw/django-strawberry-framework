...
## Revision 2 (2026-07-20) -- adversarial-spot-check corrections

Revision 1's totals and coverage were spot-checked and confirmed exact (378
items, one row per `(card, order)` pair, distribution summing to 378). The
same spot-check found *systematic classification errors* that Revision 2 folds
in. Nothing in the DB changed; only this document's proposed mapping did.

Changes applied:

- **Structural (map to EXISTING `kanban_section` keys, do not mint
  duplicates).** The DB already ships nine section keys: `scope`,
  `definition_of_done`, `foundation_seam`, `files_touched`,
  `verified_upstream`, `arch_posture`, `why_it_matters`, `dependencies_note`,
  `other`. Revision 1 invented `file_reference` and `dependency`; both are now
  folded into their existing homes:
  - every file-path item -> existing **`files_touched`** (was `file_reference`)
  - every dependency item -> existing **`dependencies_note`** (was `dependency`)
- **High-confidence errata:** `uv.lock` note rows (12/5, 15/18) ->
  `files_touched`; the `testing/relay.py` path row (32/55, was `test_plan`) ->
  `files_touched`; the "test hygiene" / "internal test/doc cleanup" positioning
  rows (11/0, 13/0, 14/0, were `test_plan`) -> `why_it_matters`; the
  upstream-parity line 43/1 (was `test_plan`) -> `verified_upstream`; 56/2 (was
  `definition_of_done`) -> `why_it_matters`, which empties `definition_of_done`
  entirely (dropped from the proposal); 32/11 (was `dependency`) -> `scope`;
  32/35-39 and 40/1 (were `note`) -> `dependencies_note`.
- **Medium-confidence errata (flagged inline with `[medium]`):** 16/2
  (`scope` -> `note`), 19/0 (`scope` -> `verified_upstream`), 54/5
  (`note` -> `open_question`), 43/2 (`test_plan` -> `scope`), 13/1
  (`test_plan` -> `scope`).
- **`note` residual re-run against under-used existing keys.** Revision 1
  dumped ~257 items into `note`. Reading the actual item texts surfaced clear
  homes in four existing keys: `verified_upstream` (every "both upstreams
  ship..." / "graphene-django ships..." / "neither upstream..." parity line),
  `why_it_matters` (positioning one-liners such as "developer experience.",
  "release card.", "security-relevant..."), `foundation_seam` (the
  forward-reserved architectural-seam slots, 10/19-20, 15/20), and
  `arch_posture` (borrowing-posture / design-stance rows, 15/19, 1/8).
  Genuinely miscellaneous rows stay `note`.
- **Header fix.** Revision 1's key list omitted `file_reference` (the
  2nd-largest bucket) and listed a `risk` key with 0 items. The Revision 2
  distribution below lists exactly the keys the mapping uses and sums to 378.

## Proposed section keys

Seven of the target keys already exist in `kanban_section` (no new `Section`
row needed):

| key | label | DB order | status |
|---|---|---|---|
| files_touched | Files likely touched | 3 | existing |
| verified_upstream | Verified in upstream | 4 | existing |
| arch_posture | Architectural posture | 5 | existing |
| why_it_matters | Why it matters | 6 | existing |
| dependencies_note | Dependencies | 7 | existing |
| scope | Scope | 0 | existing |
| foundation_seam | Foundation-slice seam | 2 | existing |

Four keys are genuinely **net-new** (would require a new `Section` row;
proposed label + `order` appended after the current max `order` of 8):

| key | proposed label | proposed order | status |
|---|---|---|---|
| test_plan | Test plan | 9 | net-new |
| decision | Decision | 10 | net-new |
| open_question | Open question | 11 | net-new |
| note | Note | 12 | net-new (residual) |

`definition_of_done` already exists in the DB but the corrected mapping leaves
it with 0 items, so it is not a target of this proposal. `other` (order 8) is
the source section, not a target.

## Proposed distribution

| proposed section | items | key status |
|---|---|---|
| note | 207 | net-new |
| files_touched | 84 | existing |
| verified_upstream | 29 | existing |
| scope | 16 | existing |
| why_it_matters | 15 | existing |
| dependencies_note | 7 | existing |
| decision | 5 | net-new |
| open_question | 5 | net-new |
| test_plan | 5 | net-new |
| foundation_seam | 3 | existing |
| arch_posture | 2 | existing |
| **total** | **378** | |
...
|---|---|---|---|---|
| 1 | DjangoType core foundation | 0 | `DjangoObjectType` (graphene-django) / `@strawberry_django.type` (strawberry-gra | verified_upstream |
| 1 | DjangoType core foundation | 1 | core foundational subsystem: `DjangoType` base, Meta validation, scalar/relation | note |
| 1 | DjangoType core foundation | 2 | `django_strawberry_framework/types/base.py` | files_touched |
| 1 | DjangoType core foundation | 3 | `django_strawberry_framework/types/converters.py` | files_touched |
| 1 | DjangoType core foundation | 4 | `django_strawberry_framework/types/resolvers.py` | files_touched |
| 1 | DjangoType core foundation | 5 | `tests/types/test_base.py` | files_touched |
| 1 | DjangoType core foundation | 6 | `tests/types/test_converters.py` | files_touched |
| 1 | DjangoType core foundation | 7 | `tests/types/test_resolvers.py` | files_touched |
| 1 | DjangoType core foundation | 8 | The public shape is intentionally narrow and explicit. | arch_posture |
...
| 2 | Optimizer O1-O6 foundation | 0 | strawberry-graphql-django ships a heavy optimizer extension; graphene-django has | verified_upstream |
| 2 | Optimizer O1-O6 foundation | 1 | heavy optimizer extension: relation resolvers, selection-tree walker, root-gated | note |
| 2 | Optimizer O1-O6 foundation | 2 | `django_strawberry_framework/optimizer/extension.py` | files_touched |
| 2 | Optimizer O1-O6 foundation | 3 | `django_strawberry_framework/optimizer/walker.py` | files_touched |
| 2 | Optimizer O1-O6 foundation | 4 | `django_strawberry_framework/optimizer/plans.py` | files_touched |
| 2 | Optimizer O1-O6 foundation | 5 | `tests/optimizer/test_extension.py` | files_touched |
| 2 | Optimizer O1-O6 foundation | 6 | `tests/optimizer/test_walker.py` | files_touched |
| 2 | Optimizer O1-O6 foundation | 7 | `tests/optimizer/test_plans.py` | files_touched |
...
| 4 | Optimizer beyond slices B1-B8 | 2 | `django_strawberry_framework/optimizer/extension.py` | files_touched |
| 4 | Optimizer beyond slices B1-B8 | 3 | `django_strawberry_framework/optimizer/hints.py` | files_touched |
| 4 | Optimizer beyond slices B1-B8 | 4 | `django_strawberry_framework/optimizer/field_meta.py` | files_touched |
| 4 | Optimizer beyond slices B1-B8 | 5 | `django_strawberry_framework/optimizer/plans.py` | files_touched |
| 4 | Optimizer beyond slices B1-B8 | 6 | `tests/optimizer/test_extension.py` | files_touched |
| 4 | Optimizer beyond slices B1-B8 | 7 | `tests/optimizer/test_hints.py` | files_touched |
| 4 | Optimizer beyond slices B1-B8 | 8 | `tests/optimizer/test_field_meta.py` | files_touched |
| 4 | Optimizer beyond slices B1-B8 | 9 | `tests/optimizer/test_plans.py` | files_touched |
...
| 6 | Documentation/status positioning for shipped Layer 2 | 0 | internal docs / status-positioning card — no upstream-parity surface. | why_it_matters |
...
| 6 | Documentation/status positioning for shipped Layer 2 | 2 | `docs/README.md` | files_touched |
| 6 | Documentation/status positioning for shipped Layer 2 | 3 | `docs/GLOSSARY.md` | files_touched |
| 6 | Documentation/status positioning for shipped Layer 2 | 4 | `docs/TREE.md` | files_touched |
...
| 7 | 0.0.4 onboarding docs and spec consolidation | 0 | internal docs cleanup / spec consolidation — no upstream-parity surface. | why_it_matters |
...
| 7 | 0.0.4 onboarding docs and spec consolidation | 2 | `README.md` | files_touched |
| 7 | 0.0.4 onboarding docs and spec consolidation | 3 | `docs/README.md` | files_touched |
| 7 | 0.0.4 onboarding docs and spec consolidation | 4 | `docs/GLOSSARY.md` | files_touched |
| 7 | 0.0.4 onboarding docs and spec consolidation | 5 | `docs/TREE.md` | files_touched |
| 7 | 0.0.4 onboarding docs and spec consolidation | 6 | `CHANGELOG.md` | files_touched |
...
| 10 | 0.0.4 foundation slice (definition-order independence) | 2 | `django_strawberry_framework/types/definition.py` | files_touched |
| 10 | 0.0.4 foundation slice (definition-order independence) | 3 | `django_strawberry_framework/types/relations.py` | files_touched |
| 10 | 0.0.4 foundation slice (definition-order independence) | 4 | `django_strawberry_framework/types/finalizer.py` | files_touched |
| 10 | 0.0.4 foundation slice (definition-order independence) | 5 | `django_strawberry_framework/types/base.py` | files_touched |
| 10 | 0.0.4 foundation slice (definition-order independence) | 6 | `django_strawberry_framework/types/converters.py` | files_touched |
| 10 | 0.0.4 foundation slice (definition-order independence) | 7 | `django_strawberry_framework/types/resolvers.py` | files_touched |
| 10 | 0.0.4 foundation slice (definition-order independence) | 8 | `django_strawberry_framework/registry.py` | files_touched |
| 10 | 0.0.4 foundation slice (definition-order independence) | 9 | `tests/types/test_definition_order.py` | files_touched |
| 10 | 0.0.4 foundation slice (definition-order independence) | 10 | `tests/types/test_definition_order_schema.py` | files_touched |
| 10 | 0.0.4 foundation slice (definition-order independence) | 11 | `tests/optimizer/test_definition_order.py` | files_touched |
| 10 | 0.0.4 foundation slice (definition-order independence) | 12 | `tests/test_registry.py` | files_touched |
| 10 | 0.0.4 foundation slice (definition-order independence) | 13 | `examples/fakeshop/apps/library/models.py` | files_touched |
| 10 | 0.0.4 foundation slice (definition-order independence) | 14 | `examples/fakeshop/apps/library/schema.py` | files_touched |
| 10 | 0.0.4 foundation slice (definition-order independence) | 15 | `examples/fakeshop/test_query/test_library_api.py` | files_touched |
| 10 | 0.0.4 foundation slice (definition-order independence) | 16 | `CHANGELOG.md` | files_touched |
| 10 | 0.0.4 foundation slice (definition-order independence) | 17 | `docs/SPECS/spec-010-foundation-0_0_4.md` | files_touched |
| 10 | 0.0.4 foundation slice (definition-order independence) | 18 | `docs/feedback.md` | files_touched |
| 10 | 0.0.4 foundation slice (definition-order independence) | 19 | The forward-reserved slots on `DjangoTypeDefinition` are the architectural seam  | foundation_seam |
| 10 | 0.0.4 foundation slice (definition-order independence) | 20 | The pending-resolution pattern (record at class creation, resolve at finalizatio | foundation_seam |
...
| 11 | Stale placeholder cleanup | 0 | internal test/doc cleanup. | why_it_matters |
| 11 | Stale placeholder cleanup | 1 | replace stale M2M / forward-reference skips with definition-order tests. | scope `[medium]` |
| 11 | Stale placeholder cleanup | 2 | `tests/types/test_definition_order.py` | files_touched |
| 11 | Stale placeholder cleanup | 3 | `tests/types/test_definition_order_schema.py` | files_touched |
| 11 | Stale placeholder cleanup | 4 | `tests/optimizer/test_definition_order.py` | files_touched |
...
| 12 | 0.0.4 version and release alignment | 2 | `pyproject.toml` | files_touched |
| 12 | 0.0.4 version and release alignment | 3 | `django_strawberry_framework/__init__.py` | files_touched |
| 12 | 0.0.4 version and release alignment | 4 | `tests/base/test_init.py` | files_touched |
| 12 | 0.0.4 version and release alignment | 5 | `uv.lock` | files_touched |
| 12 | 0.0.4 version and release alignment | 6 | `CHANGELOG.md` | files_touched |
| 13 | Real M2M coverage | 0 | test hygiene. | why_it_matters |
| 13 | Real M2M coverage | 1 | replace test-only M2M / cardinality fixtures with real `library` models; add pac | scope `[medium]` |
| 13 | Real M2M coverage | 2 | `examples/fakeshop/apps/library/models.py` | files_touched |
| 13 | Real M2M coverage | 3 | `examples/fakeshop/test_query/test_library_api.py` | files_touched |
| 13 | Real M2M coverage | 4 | `tests/types/test_definition_order.py` | files_touched |
| 13 | Real M2M coverage | 5 | `tests/optimizer/test_definition_order.py` | files_touched |
| 14 | Move test fixture out of example settings | 0 | test hygiene. | why_it_matters |
...
| 14 | Move test fixture out of example settings | 2 | `examples/fakeshop/config/settings.py` | files_touched |
| 14 | Move test fixture out of example settings | 3 | `examples/fakeshop/apps/library/models.py` | files_touched |
| 14 | Move test fixture out of example settings | 4 | `docs/SPECS/spec-014-testing_shift-0_0_4.md` | files_touched |
| 14 | Move test fixture out of example settings | 5 | `AGENTS.md` | files_touched |
| 14 | Move test fixture out of example settings | 6 | `docs/TREE.md` | files_touched |
| 15 | 0.0.5 Relay interfaces and Node foundation | 0 | both upstreams ship Relay Node interfaces; this shipped our 🍓-shaped Relay Node  | verified_upstream |
...
| 15 | 0.0.5 Relay interfaces and Node foundation | 2 | `django_strawberry_framework/types/base.py` | files_touched |
| 15 | 0.0.5 Relay interfaces and Node foundation | 3 | `django_strawberry_framework/types/relay.py` | files_touched |
| 15 | 0.0.5 Relay interfaces and Node foundation | 4 | `django_strawberry_framework/types/finalizer.py` | files_touched |
| 15 | 0.0.5 Relay interfaces and Node foundation | 5 | `tests/types/test_relay_interfaces.py` | files_touched |
| 15 | 0.0.5 Relay interfaces and Node foundation | 6 | `tests/types/test_definition_order_schema.py` | files_touched |
| 15 | 0.0.5 Relay interfaces and Node foundation | 7 | `tests/optimizer/test_relay_id_projection.py` | files_touched |
| 15 | 0.0.5 Relay interfaces and Node foundation | 8 | `tests/test_registry.py` | files_touched |
| 15 | 0.0.5 Relay interfaces and Node foundation | 9 | `examples/fakeshop/test_query/test_library_api.py` | files_touched |
...
| 15 | 0.0.5 Relay interfaces and Node foundation | 11 | `CHANGELOG.md` | files_touched |
| 15 | 0.0.5 Relay interfaces and Node foundation | 12 | `docs/GLOSSARY.md` | files_touched |
| 15 | 0.0.5 Relay interfaces and Node foundation | 13 | `docs/README.md` | files_touched |
| 15 | 0.0.5 Relay interfaces and Node foundation | 14 | `TODAY.md` | files_touched |
| 15 | 0.0.5 Relay interfaces and Node foundation | 15 | `pyproject.toml` | files_touched |
| 15 | 0.0.5 Relay interfaces and Node foundation | 16 | `django_strawberry_framework/__init__.py` | files_touched |
| 15 | 0.0.5 Relay interfaces and Node foundation | 17 | `tests/base/test_init.py` | files_touched |
| 15 | 0.0.5 Relay interfaces and Node foundation | 18 | `uv.lock` | files_touched |
| 15 | 0.0.5 Relay interfaces and Node foundation | 19 | Borrowed patterns from `strawberry-django` (spec "Borrowing posture", Decision 3 | arch_posture |
| 15 | 0.0.5 Relay interfaces and Node foundation | 20 | `Meta.interfaces` is the first `0.0.4`-reserved `DjangoTypeDefinition` slot that | foundation_seam |
...
| 16 | `FieldMeta` single-source-of-truth consolidation and mirror retirement | 2 | Commit `de35a62` (`refactor(types,optimizer): consolidate metadata onto DjangoTy | note `[medium]` |
| 16 | `FieldMeta` single-source-of-truth consolidation and mirror retirement | 3 | `django_strawberry_framework/types/base.py` | files_touched |
| 16 | `FieldMeta` single-source-of-truth consolidation and mirror retirement | 4 | `django_strawberry_framework/types/converters.py` | files_touched |
| 16 | `FieldMeta` single-source-of-truth consolidation and mirror retirement | 5 | `django_strawberry_framework/types/resolvers.py` | files_touched |
| 16 | `FieldMeta` single-source-of-truth consolidation and mirror retirement | 6 | `django_strawberry_framework/optimizer/walker.py` | files_touched |
| 16 | `FieldMeta` single-source-of-truth consolidation and mirror retirement | 7 | `django_strawberry_framework/optimizer/extension.py` | files_touched |
...
| 17 | Deferred scalar conversions | 0 | both upstreams ship scalar conversion for `BigIntegerField` / `JSONField` / `HSt | verified_upstream |
...
| 18 | Multiple DjangoTypes per model with `Meta.primary` | 0 | 🍓 parity-adjacent (strawberry-graphql-django has an implicit primary-type concep | verified_upstream |
...
| 19 | Consumer override semantics (scalar fields) | 0 | both upstreams support consumer-authored scalar field overrides on model-backed  | verified_upstream `[medium]` |
...
| 20 | `DjangoListField` (non-Relay list) | 0 | graphene-django ships `DjangoListField`; strawberry-graphql-django has no non-Re | verified_upstream |
| 20 | `DjangoListField` (non-Relay list) | 1 | `DjangoListField` factory: default + consumer resolver, `Manager → QuerySet` coe | note |
| 21 | `apps.py` and Django app config | 0 | both upstreams ship an `apps.py` `AppConfig` for `INSTALLED_APPS`-driven discove | verified_upstream |
| 21 | `apps.py` and Django app config | 1 | tiny `AppConfig` (two class attributes, no `ready()` body in 0.0.7) + tests. | note |
| 22 | Schema export management command | 0 | strawberry-graphql-django ships `manage.py export_schema`; graphene-django's dif | verified_upstream |
| 22 | Schema export management command | 1 | one management command (positional `schema`, `--path`, SDL via `print_schema`, ` | note |
| 23 | Multi-database cooperation contract | 0 | multi-DB is a Django capability neither upstream specifies a contract around (⚛️ | verified_upstream |
| 23 | Multi-database cooperation contract | 1 | pin the multi-DB cooperation contract (router-aware FK-id stubs, `.using()` pres | note |
| 24 | Django Trac #37064 hardening + `safe_wrap_connection_method` | 0 | defensive hardening unique to this package; neither upstream ships a Django Trac | verified_upstream |
...
| 25 | Warning-free scalar registration via `StrawberryConfig.scalar_map` | 0 | package-specific scalar-registration plumbing (`StrawberryConfig.scalar_map` via | verified_upstream |
...
| 26 | Scalar conversion end-to-end coverage in the fakeshop example | 0 | both upstreams ship scalar conversion for the full numeric / date / JSON / UUID  | verified_upstream |
...
| 27 | Filtering subsystem | 0 | both upstreams ship a FilterSet / filter surface; `django-graphene-filters` is t | verified_upstream |
...
| 30 | `DjangoConnectionField` | 1 | both upstreams ship Relay-shaped connection fields. | verified_upstream |
...
| 32 | Full Relay story (Node + Connection + Root + validation) | 11 | **Required arguments**: `first: Int`, `after: String`, `last: Int`, `before: Str | scope |
...
| 32 | Full Relay story (Node + Connection + Root + validation) | 35 | `{{card_ref:0}}` (`DjangoConnectionField`) — **hard dependency**; this card unbl | dependencies_note |
| 32 | Full Relay story (Node + Connection + Root + validation) | 36 | `{{card_ref:1}}` (Filtering subsystem) — soft dependency for the filter argument | dependencies_note |
| 32 | Full Relay story (Node + Connection + Root + validation) | 37 | `{{card_ref:2}}` (Ordering subsystem) — soft dependency for the orderBy argument | dependencies_note |
| 32 | Full Relay story (Node + Connection + Root + validation) | 38 | `{{card_ref:7}}` (Connection-aware optimizer planning) — ships in parallel; the  | dependencies_note |
| 32 | Full Relay story (Node + Connection + Root + validation) | 39 | `{{card_ref:8}}` (Permissions subsystem) — soft dependency; the Node entry point | dependencies_note |
...
| 32 | Full Relay story (Node + Connection + Root + validation) | 55 | `django_strawberry_framework/testing/relay.py` (new) — test helpers | files_touched |
...
| 33 | Connection-aware optimizer planning | 0 | gated on `{{card_ref:0}}` / Relay decisions. | dependencies_note |
| 33 | Connection-aware optimizer planning | 1 | strawberry-graphql-django plans connection selections natively; graphene-django  | verified_upstream |
...
| 34 | Permissions subsystem | 0 | for the fakeshop example and real usage. | why_it_matters |
| 34 | Permissions subsystem | 1 | django-graphene-filters ships rich cascade + per-field permissions; strawberry-g | verified_upstream |
| 34 | Permissions subsystem | 2 | permissions/visibility is security-relevant and blocks the fakeshop real-usage s | why_it_matters |
...
| 36 | Mutations + auto-generated Input types | 0 | mutations are the single largest unscoped gap vs strawberry-graphql-django (crea | verified_upstream |
...
| 37 | Upload scalar and file / image field mapping | 0 | strawberry-graphql-django maps `FileField` / `ImageField` to `Upload` (input) an | verified_upstream |
...
| 38 | Form-based mutations (Django Forms / ModelForms) | 0 | graphene-django ships `DjangoFormMutation` / `DjangoModelFormMutation`. | verified_upstream |
...
| 39 | DRF serializer mutations (`SerializerMutation`) | 0 | graphene-django ships `SerializerMutation`; the highest-leverage write-side feat | verified_upstream |
...
| 40 | Auth mutations (login / logout / register) | 0 | strawberry-graphql-django ships a small auth-mutations module. | verified_upstream |
| 40 | Auth mutations (login / logout / register) | 1 | depends on `{{card_ref:0}}`. | dependencies_note |
| 40 | Auth mutations (login / logout / register) | 2 | new `auth/` module (`login` / `logout` / `register` + `current_user` query helpe | note |
| 41 | Channels ASGI router (migration aid) | 0 | small slice; explicit migration aid. | why_it_matters |
| 41 | Channels ASGI router (migration aid) | 1 | strawberry-graphql-django ships a Channels `ProtocolTypeRouter` helper; graphene | verified_upstream |
| 41 | Channels ASGI router (migration aid) | 2 | small `routers.py` (~30 lines) with a soft `channels` dependency; tests for both | note |
| 42 | Debug-toolbar middleware | 0 | developer experience. | why_it_matters |
| 42 | Debug-toolbar middleware | 1 | strawberry-graphql-django ships a debug-toolbar middleware; graphene-django ship | verified_upstream |
| 42 | Debug-toolbar middleware | 2 | subclass django-debug-toolbar's middleware with two injection paths (GraphiQL HT | note |
| 43 | Test client helper | 0 | developer experience. | why_it_matters |
| 43 | Test client helper | 1 | both upstreams ship a GraphQL test client / mixin. | verified_upstream |
| 43 | Test client helper | 2 | `test/client.py` (sync + async `TestClient`, a `GraphQLTestMixin`, two `(Mixin,  | scope `[medium]` |
| 44 | Response-extensions debug middleware | 0 | developer experience. | why_it_matters |
| 44 | Response-extensions debug middleware | 1 | graphene-django ships an in-response `DjangoDebug` SQL/exception subsystem; stra | verified_upstream |
...
| 47 | Beta release (cleanup, verification, alpha → beta) | 0 | release card. | why_it_matters |
...
| 47 | Beta release (cleanup, verification, alpha → beta) | 2 | release-blocking. | why_it_matters |
| 47 | Beta release (cleanup, verification, alpha → beta) | 3 | final card in the Alpha queue; gates the alpha → beta milestone. | why_it_matters |
...
| 49 | `Meta.search_fields` support | 1 | `django-graphene-filters` exposes `Meta.search_fields = ("name", "description",  | verified_upstream |
...
| 50 | Postgres full-text search filter primitives | 1 | The only cookbook filter-surface gap found in the 0.0.7 DRY-cycle kwarg-parity a | verified_upstream |
...
| 54 | Stable choice enum naming override | 5 | Decide whether this belongs in the consumer-overrides spec or a small choice-enu | open_question `[medium]` |
...
| 56 | Product-catalog Layer 3 HTTP GraphQL tests | 2 | The library app already has live `/graphql/` acceptance tests under `examples/fa | why_it_matters |
...
| 57 | Mutation transactions and idempotency | 1 | Neither graphene-django nor strawberry-graphql-django ships mutation idempotency | verified_upstream |
...
The signed-off Revision 2 mapping was applied to `examples/fakeshop/db.sqlite3`.
This report is now a historical record.

- **Migration:** `0015_seed_reclassification_sections.py` seeded the four
  net-new `Section` rows (`test_plan`/9, `decision`/10, `open_question`/11,
  `note`/12), each with its `UUIDModel` side-row.
- **Command:** `manage.py reclassify_other_items --mapping
  section-other-mapping.json --compact` moved the 378 mapped items via
  `.update()`. Reported: "Moved 378 item(s); renumbered 0 `other` order(s)."
- **Verification gates:** (2) per-target distribution matched the proposal table
  exactly (`note` 207, `files_touched` 84, `verified_upstream` 29, `scope` 16,
  `why_it_matters` 15, `dependencies_note` 7, `decision` 5, `open_question` 5,
  `test_plan` 5, `foundation_seam` 3, `arch_posture` 2); (3) no mapped uuid
  remained in `other`; (4) no `(card, section)` had duplicate orders; (5) the
  four new `Section` rows exist with intact one-hot `UUIDModel` links.
- **Concurrent-writer note:** `other` held 380 items at execution time, not the
  378 in this mapping. The two extra rows are a new card 64 (items 0 and 1),
  added by a parallel session after Revision 2 was signed off. They are outside
  this signed-off mapping and were correctly left in `other` (gate 1 shows
  `other` count = 2, all 378 mapped items relocated). Reclassifying card 64 is a
  separate decision.
- **Exports:** `KANBAN.md`, `KANBAN.html`, `KANBAN.json` regenerated; all three
  builders plus `build_glossary_md.py` pass `--check`.
