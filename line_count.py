...
def main() -> None:
    """Print line count tables for the package, tests, examples, and the entire project."""
    project_root = Path(__file__).parent

    # Table 1: django_strawberry_framework package only
    pkg_dir = project_root / "django_strawberry_framework"
    if pkg_dir.is_dir():
        pkg_results = scan_directory(pkg_dir)
        print_table("django_strawberry_framework/", pkg_results)

    # Table 2: package tests (gates 100% coverage)
    tests_dir = project_root / "tests"
    if tests_dir.is_dir():
        tests_results = scan_directory(tests_dir)
        print_table("tests/", tests_results)

    # Table 3: example project
    examples_dir = project_root / "examples"
    if examples_dir.is_dir():
        examples_results = scan_directory(examples_dir)
        print_table("examples/", examples_results)

    # Table 4: entire project
