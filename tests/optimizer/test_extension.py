...
# ---------------------------------------------------------------------------
# Mixed-case Django field names
# ---------------------------------------------------------------------------


@contextlib.contextmanager
def _unmanaged_tables(*model_classes):
    """Create the ``managed = False`` test models' tables; drop them on exit, in reverse."""
    from django.db import connection
    ...
    try:
        ...
        with connection.schema_editor() as editor:
            for model_cls in reversed(model_classes):
                editor.delete_model(model_cls)


@pytest.mark.django_db(transaction=True)
def test_mixed_case_model_field_selection_projects_its_real_column():
    """A mixed-case scalar keys the field map by its Django name and projects that column.

    Django accepts ``isPublic`` as a field name and Strawberry exposes it as
    ``isPublic``; reversing the selection gives ``is_public``, which names no
    column. The canonical map is keyed by the raw Django name, so the walker's
    reverse lookup misses and its forward resolution projects ``.only("isPublic")``
    in the one query. No fakeshop model carries a mixed-case field, so the synthetic
    ``managed = False`` model stands in.
    """
    from django.db import connection, models
    from django.test.utils import CaptureQueriesContext

    class MixedCaseFlag(models.Model):
        isPublic = models.BooleanField(default=False)  # noqa: N815
    ...
    class MixedCaseFlagType(DjangoType):
        class Meta:
            model = MixedCaseFlag
    ...
    @strawberry.type
    class Query:
        flags: list[MixedCaseFlagType] = DjangoListField(MixedCaseFlagType)

    finalize_django_types()
    extension = DjangoOptimizerExtension()
    schema = DjangoSchema(query=Query, extensions=[lambda: extension])

    with _unmanaged_tables(MixedCaseFlag):
        row = MixedCaseFlag.objects.create(isPublic=True, name="a")
        with CaptureQueriesContext(connection) as captured:
            result = schema.execute_sync("{ flags { id name isPublic } }")

    assert result.errors is None, result.errors
    assert result.data == {"flags": [{"id": row.pk, "name": "a", "isPublic": True}]}
    assert len(captured.captured_queries) == 1, captured.captured_queries
    assert '"isPublic"' in captured.captured_queries[0]["sql"]
    definition = registry.get_definition(MixedCaseFlagType)
    assert sorted(definition.field_map) == sorted(f.name for f in definition.selected_fields)


@pytest.mark.django_db(transaction=True)
def test_mixed_case_relation_selections_plan_through_their_django_names():
    """Mixed-case forward-FK and reverse-FK names plan a JOIN and a prefetch, not an error.

    ``ownerRef`` (forward FK) and its ``related_name`` ``petItems`` (reverse FK)
    reverse to ``owner_ref`` / ``pet_items``, which name no relation. Keyed by the
    Django name, the field map resolves both through the walker's forward
    resolution: the pet list reads its owner through one ``select_related`` query
    and the owner list loads its pets through one prefetch, at two owner
    cardinalities.
    """
    from django.db import connection, models
    from django.test.utils import CaptureQueriesContext

    class MixedCaseOwner(models.Model):
        displayName = models.TextField()  # noqa: N815

        class Meta:
            app_label = "products"
            managed = False

    class MixedCasePet(models.Model):
        name = models.TextField()
        ownerRef = models.ForeignKey(  # noqa: N815
            MixedCaseOwner,
            related_name="petItems",
    ...
    class MixedCaseOwnerType(DjangoType):
        class Meta:
            model = MixedCaseOwner
            fields = ("id", "displayName", "petItems")

    class MixedCasePetType(DjangoType):
        class Meta:
            model = MixedCasePet
            fields = ("id", "name", "ownerRef")

    @strawberry.type
    class Query:
        owners: list[MixedCaseOwnerType] = DjangoListField(MixedCaseOwnerType)
        pets: list[MixedCasePetType] = DjangoListField(MixedCasePetType)
    ...
    counts = {}
    with _unmanaged_tables(MixedCaseOwner, MixedCasePet):
        for owner_count in (1, 3):
            MixedCasePet.objects.all().delete()
            MixedCaseOwner.objects.all().delete()
            for index in range(owner_count):
                owner = MixedCaseOwner.objects.create(displayName=f"o{index}")
                MixedCasePet.objects.create(name=f"p{index}", ownerRef=owner)
            with CaptureQueriesContext(connection) as pets_queries:
                pets = schema.execute_sync("{ pets { name ownerRef { displayName } } }")
            with CaptureQueriesContext(connection) as owners_queries:
                owners = schema.execute_sync("{ owners { displayName petItems { name } } }")
            assert pets.errors is None, pets.errors
            assert owners.errors is None, owners.errors
            assert sorted(p["ownerRef"]["displayName"] for p in pets.data["pets"]) == [
                f"o{index}" for index in range(owner_count)
            ]
            assert sorted(o["petItems"][0]["name"] for o in owners.data["owners"]) == [
                f"p{index}" for index in range(owner_count)
            ]
            counts[owner_count] = (
                len(pets_queries.captured_queries),
                len(owners_queries.captured_queries),
            )
            assert '"displayName"' in pets_queries.captured_queries[0]["sql"]
            assert "JOIN" in pets_queries.captured_queries[0]["sql"].upper()
            assert '"ownerRef_id"' in owners_queries.captured_queries[1]["sql"]

    assert counts == {1: (1, 2), 3: (1, 2)}
    for type_cls in (MixedCaseOwnerType, MixedCasePetType):
